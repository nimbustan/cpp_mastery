/**
 * @file 05_compilation_pipeline.cpp
 * @brief EN: C++ Compilation Pipeline — From Source to Executable
 * TR: C++ Derleme Hattı — Kaynak Koddan Çalıştırılabilir Dosyaya
 *
 * @details
 * [THEORY]
 *
 ...
 * TR: C++ derleme süreci, insan tarafından okunabilir kaynağı makine koduna dört ayrı aşamada
 * dönüştürür. Bu hattı anlamak, bağlayıcı hatalarını teşhis etmek, derleme sürelerini yönetmek
 * ve otomotiv ECU'ları için sağlam yazılım geliştirmek açısından kritiktir.
 *
 * EN: ── Stage 1: PREPROCESSING ── The preprocessor (cpp) runs before actual compilation. It
 * handles: • #include — textual inclusion of header files • #define / #ifdef / #ifndef — macro
 * expansion & conditional compilation • #pragma — compiler-specific directives • __FILE__,
 * __LINE__, __func__ — predefined macros In automotive, conditional compilation selects
 * HW-specific code paths (e.g., ECU_VARIANT_A vs ECU_VARIANT_B) from one codebase.
 * TR: ── Aşama 1: ÖN İŞLEME ── Ön işlemci (cpp), gerçek derlemeden önce çalışır: • #include —
 * başlık dosyalarının metin olarak dahil edilmesi • #define / #ifdef / #ifndef — makro açılımı
 * ve koşullu derleme • #pragma — derleyiciye özel direktifler • __FILE__, __LINE__, __func__ —
 * önceden tanımlı makrolar Otomotivde koşullu derleme, tek kod tabanından donanıma özel yolları
 * seçer.
 *
 * EN: ── Stage 2: COMPILATION ── The compiler (cc1plus) translates each preprocessed translation
 ...
 * the same types.
 * TR: ── Aşama 2: DERLEME ── Derleyici, ön işlenmiş her çeviri birimini (TU) nesne dosyasına
 * (.o) dönüştürür. Her .cpp dosyası BİR çeviri birimidir. Şablonlar her TU'da ayrı ayrı
 * örneklenir; bu, aynı şablonun birden fazla TU'da kullanılması durumunda kod şişmesine yol
 * açabilir.
 *
 * EN: ── Stage 3: LINKING ── The linker (ld) merges object files and resolves symbol references.
 ...
 * TR: ── Aşama 3: BAĞLAMA ── Bağlayıcı (ld), nesne dosyalarını birleştirir ve sembol
 * referanslarını çözümler. Tek Tanım Kuralı (ODR): her kullanılan varlığın tüm TU'lar boyunca
 * tam olarak bir tanımı olmalıdır. `extern` bir sembolü tanımlamadan bildirir. `inline`
 * fonksiyonlar/değişkenler ODR'den muaftır — bağlayıcı tek bir kopya seçer. C++17 inline
 * değişkenler, başlık-yalnız tanımları mümkün kılar. Dosya/ad-alanı kapsamında `static`, iç
 * bağlantı sağlar.
 *
 * EN: ── Stage 4: LOADING ── The OS loader maps the executable into memory, resolves dynamic
 ...
 * TR: ── Aşama 4: YÜKLEME ── İşletim sistemi yükleyicisi çalıştırılabilir dosyayı belleğe eşler,
 * dinamik kütüphane sembollerini çözümler, yığını kurar ve kontrolü _start → main()'e aktarır.
 * Otomotivde ECU yazılımı genellikle doğrudan ROM'a yazılır; "yükleyici" kodu çalıştırma
 * adreslerine kopyalayan önyükleyicidir.
 *
 * EN: ── Header-Only Libraries ── Placing all code in headers is convenient (no separate .cpp to
 ...
 * TR: ── Başlık-Yalnız Kütüphaneler ── Tüm kodu başlık dosyalarına koymak pratiktir ancak
 * derleme süresini ve ikili boyutunu artırabilir; başlığı dahil eden her TU, inline olmayan
 * tanımların kendi kopyasını alır.
 *
 * [CPPREF DEPTH: One Definition Rule (ODR) — The Silent Killer of Large Projects / CPPREF
 * DERİNLİK: Tek Tanım Kuralı (ODR) — Büyük Projelerin Sessiz Katili]
 * =============================================================================
 ...
 *
 * TR: Tek Tanım Kuralı her varlığın tüm Çeviri Birimlerinde tam olarak bir tanımı olmasını
 * gerektirir. ODR ihlali tanımsız davranıştır — ve derleyicinin uyarı vermesi zorunlu DEĞİLDİR!
 * Inline fonksiyonlar her TU'da birebir aynı tanıma sahip olmalıdır; aksi halde bağlayıcı
 * sessizce birini seçer. Başlık-yalnız kütüphaneler inline/template ODR muafiyetine dayanır.
 * C++17'de `inline` bir değişken veya fonksiyonda "birden fazla tanıma izin ver" demektir — "bu
 * çağrıyı satır içi yap" değil. LTO bazı ODR ihlallerini yakalayabilir. C++20 Modülleri dâhil
 * etme modelini tamamen kaldırarak modül-dışa-aktarılan varlıklar için ODR ihlallerini yapısal
...
// TR: ECU_DEBUG ayrıntılı tanılamayı etkinleştirir; üretim derlemelerinde kaldırılır.
...
// TR: __FILE__, __LINE__, __func__ önceden tanımlı makrolarını kullanan günlük makrosu.
...
// TR: Ad-alanı kapsamında 'static', iç bağlantı sağlar — değişken yalnızca bu çeviri biriminde
// bulunur. Başka bir TU, ODR ihlali olmadan kendi kopyasını tanımlayabilir.

static int s_ecu_boot_count = 0;

// EN: Each TU that includes such a definition gets its own independent copy.
// TR: Böyle bir tanımı içeren her TU, kendi bağımsız kopyasını alır.
...
// implementations.
// TR: Her benzersiz şablon örneklemesi kendi kod kopyasını üretir. SensorBuffer<float, 64> ve
// SensorBuffer<double, 64> birlikte kullanılırsa, derleyici İKİ tam sınıf gerçeklemesi üretir.

template <typename T, std::size_t Capacity>
class SensorBuffer {
public:
    // EN: Push a sensor reading into the circular buffer.
    // TR: Dairesel tampona bir sensör okuma değeri ekler.
    void push(T value) {
    ...
    // TR: En son okuma değerini döndürür.
    T latest() const {
        return (write_idx_ > 0)
    ...
    // TR: Kaç öğe eklendiğini döndürür (Capacity'yi aşabilir).
    std::size_t total_writes() const { return write_idx_; }

    // EN: Return the fixed capacity of this buffer.
    // TR: Bu tamponun sabit kapasitesini döndürür.
...
// (single-TU demo).
// TR: 'extern' anahtar kelimesi, başka yerde tanımlanan bir değişkeni bildirir. Çok TU'lu bir
// projede tanım tam olarak bir .cpp dosyasında olur. Burada tanımı hemen sağlıyoruz (tek TU
// demosu).
...
int main() {
    ...
    std::cout << "║  TR: Kaynak Koddan Çalışan İkili Dosyaya                ║\n";
    std::cout << "╚══════════════════════════════════════════════════════════╝\n\n";
    ...
    // TR: Derleme zamanında ECU_DEBUG'ın aktif olup olmadığını gösterir.
    ...
    // TR: Üretimde, #undef ECU_DEBUG bu bloğun tamamını kaldırır.
#endif
    ...
    // TR: ECU_LOG, çağrı noktasında __FILE__, __LINE__, __func__ değerlerini açar.
    ...
    // TR: Bu değerler, depolama maliyeti olmayan derleme zamanı sabitleridir.
    ...
    // TR: Sabitler kullanarak bir RPM aralığı denetimi simüle eder.
    ...
    // TR: s_ecu_boot_count iç bağlantıya sahiptir — yalnızca bu TU'da bulunur. Çok dosyalı bir
    // projede her TU kendi bağımsız kopyasına sahip olur.
    ...
    // TR: Farklı örneklemeler ayrı kod oluşturur — boyutları gözlemleyin.
    ...
    // TR: Örnek otomotiv sensör verisi ekler.
    ...
    // TR: Her benzersiz <T, N> kombinasyonu nesne dosyasında tam bir sınıf üretir. 4 örnekleme
    // ile push(), latest() vb. fonksiyonların 4 kopyası bulunur.
