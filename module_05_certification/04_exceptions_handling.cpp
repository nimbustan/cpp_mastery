 ...
 * =============================================================================
 * EN: CppReference expects professional C++ code to provide one of these guarantees: 1. Nothrow
 * Guarantee: Will NEVER throw (like `noexcept`). 2. Strong Guarantee: If an operation fails, the
 * state is rolled back to exactly what it was before the call (Commit or Rollback - like
 * Databases). 3. Basic Guarantee: If it fails, no memory is leaked, but the object might be in
 * an altered (but valid) state. 4. No Guarantee: Fails and corrupts/leaks memory. (Unacceptable
 * in modern C++).
 *
 * TR: CppReference, profesyonel kodun şu 4 Özel Güvenlik Seviyesinden birini vadetmesini bekler:
 * 1. Nothrow: Asla hata fırlatmaz (`noexcept`). 2. Güçlü Teminat (Strong): İşlem başarısız
 * olursa, her şey işlem öncesi haline geri döner. Hiç dokunulmamış gibi olur (Veritabanlarındaki
 * Rollback gibi). 3. Temel Teminat (Basic): Hata olursa RAM sızdırmaz, çökmeleri engeller ama
 * nesne o anki bozuk (fakat güvenli) bir halde kalabilir. 4. Garantisiz: Hata olursa RAM
 * sızdırır ve programı çökertir (Modern C++'ta Yasak!)
 *
 * @date 2026
