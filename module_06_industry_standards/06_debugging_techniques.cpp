 ...
 * ===============================================================================
 *
 * --- 1. GDB Basics / GDB Temelleri -----------------------------------------
 * EN: GDB is the GNU Debugger — the primary debugging tool on Linux. Compile with `-g` to
 * include debug symbols: g++ -g -std=c++17 -o prog 06_debugging_techniques.cpp Key commands:
 * break main    — set breakpoint at main() run           — start execution next (n)      — step
 * over to next line step (s)      — step into function call print var     — inspect variable
 * value backtrace (bt)— show call stack watch var     — break when variable changes (watchpoint)
 ...
 * 06_debugging_techniques.cpp Temel komutlar: break main    — main()'de kırılma noktası koy run 
 * — çalıştırmayı başlat next (n)      — sonraki satıra geç step (s)      — fonksiyon çağrısına
 * gir print var     — değişken değerini incele backtrace (bt)— çağrı yığınını göster watch var  
 * — değişken değişince dur (izleme noktası)
 *
 * --- 2. Core Dumps / Çekirdek Dökümleri ------------------------------------
 * EN: Enable core dumps: `ulimit -c unlimited` When a program crashes (SIGSEGV, SIGABRT), the OS
 * writes a core file. Analyze: `gdb ./program core` — see exact crash location and variables.
 * TR: Çekirdek dökümünü etkinleştir: `ulimit -c unlimited` Program çöktüğünde (SIGSEGV,
 * SIGABRT), işletim sistemi bir core dosyası yazar. Analiz: `gdb ./program core` — tam çökme
 * noktasını ve değişkenleri gör.
 *
 * --- 3. Static Analysis & Sanitizers / Statik Analiz ve Temizleyiciler -----
 * EN: Compiler warnings (-Wall -Wextra -Wpedantic) are the first line of defense.
 * Address/Thread/UB sanitizers catch bugs at runtime: -fsanitize=address   — buffer overflow,
 * use-after-free -fsanitize=undefined — signed overflow, null deref, alignment -fsanitize=thread
 * — data races in multithreaded code
 * TR: Derleyici uyarıları (-Wall -Wextra -Wpedantic) ilk savunma hattıdır. Adres/İş parçacığı/TB
 * temizleyicileri çalışma zamanında hata yakalar: -fsanitize=address   — tampon taşması, serbest
 * bırakılmış kullanımı -fsanitize=undefined — işaretli taşma, null ref, hizalama
 * -fsanitize=thread    — çok iş parçacıklı kodda veri yarışları
 *
 * --- 4. assert() vs static_assert() ----------------------------------------
 * EN: assert(expr) — runtime check; aborts if expr is false. Disabled by defining NDEBUG.
 * static_assert(expr, msg) — compile-time check; zero runtime cost. Use for type traits, sizes,
 * alignment guarantees.
 * TR: assert(ifade) — çalışma zamanı kontrolü; ifade false ise iptal eder. NDEBUG tanımlanarak
 ...
 * EN: Preconditions — validate inputs before use. Postconditions — verify outputs before
 * returning. Invariants — maintain class state consistency across all methods.
 *     [[nodiscard]] — force callers to check return values.
 * TR: Ön koşullar — kullanmadan önce girdileri doğrula. Son koşullar — dönmeden önce çıktıları
 * doğrula. Değişmezler — tüm yöntemlerde sınıf durum tutarlılığını koru.
 *     [[nodiscard]] — çağıranları dönüş değerini kontrol etmeye zorla.
 *
 * --- 6. Logging Levels / Günlük Seviyeleri ---------------------------------
 * EN: TRACE — finest granularity, every function entry/exit DEBUG — diagnostic information for
 * developers INFO  — general operational messages WARN  — potentially harmful situations ERROR —
 * error events, program can continue FATAL — severe error, program must abort
 * TR: TRACE — en ince ayrıntı, her fonksiyon giriş/çıkış DEBUG — geliştiriciler için tanılama
 * bilgisi INFO  — genel operasyonel mesajlar WARN  — potansiyel zararlı durumlar ERROR — hata
 * olayları, program devam edebilir FATAL — ağır hata, program iptal etmeli
 *
