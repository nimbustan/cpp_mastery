 ...
 * automate this. The cycle: 1) Write a FAILING test, 2) Write minimal code to pass, 3) Refactor.
 *
 ...
 * Referansı]
 * =============================================================================
 * EN: Key GTest macros from the official documentation: - `EXPECT_EQ(a, b)` / `ASSERT_EQ(a, b)`
 * — equality - `EXPECT_NE(a, b)` — not equal - `EXPECT_LT(a, b)` / `EXPECT_GT(a, b)` —
 * less/greater - `EXPECT_TRUE(cond)` / `EXPECT_FALSE(cond)` — boolean - `EXPECT_THROW(stmt,
 * ExType)` — expects exception - `EXPECT_NEAR(a, b, eps)` — floating point comparison -
 * `EXPECT_THAT(val, matcher)` — GMock matchers
 * google.github.io/googletest/reference/assertions.html
 *
 * TR: Resmi dokümantasyondan temel GTest makroları: - `EXPECT_EQ(a, b)` / `ASSERT_EQ(a, b)` —
 * eşitlik - `EXPECT_NE(a, b)` — eşit değil - `EXPECT_LT(a, b)` / `EXPECT_GT(a, b)` — küçük/büyük
 * - `EXPECT_TRUE(cond)` / `EXPECT_FALSE(cond)` — boolean - `EXPECT_THROW(stmt, ExType)` —
 * istisna beklentisi - `EXPECT_NEAR(a, b, eps)` — kayan nokta karşılaştırma - `EXPECT_THAT(val,
 * matcher)` — GMock eşleştiriciler google.github.io/googletest/reference/assertions.html
 *
 * =============================================================================
