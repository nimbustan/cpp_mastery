 ...
 * @details
 * =====================================================================
 * [THEORY: ABI and Name Mangling / TEORİ: ABI ve İsim Karıştırma]
 * =====================================================================
 *
 ...
 * ABI, derlenmiş kodun ikili düzeyde nasıl etkileştiğini tanımlar: çağrı kuralları, yazmaç
 ...
 *
 * İsim Karıştırma — Aşırı Yüklenmiş İmzaları Kodlama
 * Derleyici her fonksiyonu benzersiz bir sembole "karıştırır". Itanium ABI altında: foo(int)
 * → _Z3fooi   |   foo(int,float) → _Z3fooif Her aşırı yükleme farklı bir karıştırılmış isim
 * alır; bağlayıcı doğru olanı çözümler.
 *
 * extern "C" — Birlikte Çalışabilirlik İçin İsim Karıştırmayı Devre Dışı Bırakma
 * `extern "C" { ... }` derleyiciye C bağlantısı kullanmasını söyler: karıştırma yok. C
 * birlikte çalışma, paylaşımlı kütüphaneler ve dlopen/dlsym eklenti API'leri için gerekli.
 *
 * Derleyici ABI Uyumsuzluğu
 * MSVC, GCC ve Clang farklı karıştırma şemaları kullanır. Derleyici karışımı bağlayıcı
 * hatalarına neden olabilir. Otomotiv ECU yazılımında araç zinciri kilitlenmelidir.
 *
 * Araçlar: `c++filt` sembolleri çözümler. `nm` sembolleri listeler.
 *
 * Paylaşımlı Kütüphaneler: .so dosyaları için C ABI sunmak, herhangi bir derleyici sürümünün
