 ...
 * üç kritik durumda olur:
 *   1) Bellek eşlemeli donanım kayıtları (GPIO pinleri, zamanlayıcılar, ADC)
 *   2) Kesme Hizmet Rutinleri (ISR) tarafından değiştirilen değişkenler
 *   3) DMA denetleyicileri veya harici donanım ile paylaşılan değişkenler
 * `volatile` olmadan derleyici değeri CPU kaydında önbelleğe alabilir ve RAM'den asla tekrar
 * okumayabilir — donanım değişikliklerini tamamen kaçırır.
 *
 * =============================================================================
 ...
 * herhangi bir komut sınırında keser. ISR kuralları:
 *     1. KISA tut.
 *     2. Heap tahsisi yok.
 *     3. Blokleyici çağrı yok.
 *     4. Paylaşılan değişkenler için `volatile` kullan.
 *     5. Çok baytlı değerler için atomik tipler düşün.
 *
 * =============================================================================
