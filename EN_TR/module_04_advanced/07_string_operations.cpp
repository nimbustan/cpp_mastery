 ...
 *
 * TR: ───── std::string Bellek Modeli ─────
 *     std::string, şu özelliklere sahip dinamik bir char dizisidir:
 *     1. Heap tahsisli tampon (SSO eşiğinden büyük stringler için).
 *     2. size(): karakter sayısı.
 *     3. capacity(): tahsis edilmiş tampon boyutu (size'dan büyük olabilir).
 *     4. SSO (Küçük String Optimizasyonu): ≤ ~15-22 karakter stringler
 *        (uygulamaya bağlı) nesnenin KENDİSİNDE depolanr — heap tahsisi yok!
 *        Bu kısa stringleri çok hızlı yapar.
 *
 * TR: ───── std::string_view (C++17) ─────
 * Stringe sahiplik almayan, salt okunur bir görünüm. Kopyalamadan başkasının verisine işaret
 * eden bir "pencere" gibi.
 *  ✓ Tahsis yok, kopya yok — sadece pointer + boyut
 *  ✓ std::string, const char*, string sabitleri görüntüleyebilir
 *  ✓ Sadece string OKUYAN fonksiyon parametreleri için mükemmel
 *  ✗ TEHLİKE: Görüntülenen string, string_view'dan uzun yaşamalı! Orijinal string yok
