 ...
 *
 * İki çeşit şablon vardır:
 * 1. Fonksiyon Şablonları: `findMax<T>(T a, T b)` gibi jenerik fonksiyonlar.
 * 2. Sınıf Şablonları: `Box<T>`, `std::vector<T>` gibi jenerik sınıflar.
 *
 ...
 * temel bacağı vardır:
 * 1. Konteynerler (Containers: vector, map, set, list, deque -> Veriyi depolar)
 * 2. İteratörler  (Iterators: begin(), end(), rbegin() -> Nesne odaklı ptr)
 * 3. Algoritmalar (Algorithms: std::sort, std::find, std::transform)
 *
 ...
 *
 * Yaygın STL konteynerleri ve kullanım alanları:
 *     1. `std::vector<T>`: Dinamik dizi, bitişik bellek, hızlı rastgele erişim.
 *     2. `std::map<K,V>`: Kırmızı-siyah ağaç, sıralı anahtar-değer, O(log n).
 *     3. `std::unordered_map<K,V>`: Hash tablo, ortalama O(1) arama.
 *     4. `std::set<T>`: Sıralı benzersiz elemanlar. Üyelik testi için ideal.
 *     5. `std::deque<T>`: Çift uçlu kuyruk, her iki uçta hızlı push/pop.
 *
 * =============================================================================
 ...
 * Şişmeyi azaltmak için:
 *     1. `extern template` bildirimleri (C++11): Derleyiciye bu çeviri biriminde
 *        şablonu somutlaştırma demeyi sağlar.
 *     2. Şablon olmayan uygulamalar üzerine ince şablon sarmalayıcılar yazmak.
 *     3. Farklı tip argüman sayısını sınırlamak.
 *
