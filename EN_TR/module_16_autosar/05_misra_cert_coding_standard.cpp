 ...
 * @brief MISRA C++ 2023 & CERT C++ — Automotive Coding Standards
 *
 * @details
 * =============================================================================
 * [THEORY: MISRA C++ 2023 — Overview / TEORİ: Genel Bakış]
 * =============================================================================
 *
 ...
 *     └─────────────────────────────────┴──────────────────────────────┘
 *
 * TR: MISRA C++ 2023, C++17 için otomotiv endüstrisi kodlama standardıdır.
 *     Önceki MISRA C++ 2008 ve AUTOSAR C++ 2019'u birleştirir ve günceller.
 *
 *     Kural Kategorileri:
 *     ┌─────────────┬──────────────────────────────────────────────────┐
 *     │ Kategori    │ Açıklama                                         │
 *     ├─────────────┼──────────────────────────────────────────────────┤
 *     │ Mandatory   │ Mutlaka uyulmalı — sapma izni yok                │
 *     │ Required    │ Mutlaka uyulmalı — sapma gerekçelendirilmeli     │
 *     │ Advisory    │ Uyulması önerilir — en iyi uygulama              │
 *     └─────────────┴──────────────────────────────────────────────────┘
 *
 *     Temel MISRA C++ 2023 Kısıtlamaları:
 *     ┌─────────────────────────────────┬──────────────────────────────┐
 *     │ Özellik                         │ MISRA C++ 2023 Tutumu        │
 *     ├─────────────────────────────────┼──────────────────────────────┤
 *     │ Exceptions (throw/catch)        │ YASAK (dinamik yol yok)      │
 *     │ RTTI (dynamic_cast, typeid)     │ YASAK (çalışma zamanı yükü)  │
 *     │ Dinamik allokasyon (new/delete) │ KISITLI (yalnızca init'te)   │
 *     │ goto                            │ YASAK                        │
 *     │ C-stili dönüşümler              │ YASAK                        │
 *     │ Örtük daraltma dönüşümleri      │ YASAK                        │
 *     │ Trivial olmayan union üyeleri   │ YASAK                        │
 *     │ setjmp/longjmp                  │ YASAK                        │
 *     ├─────────────────────────────────┼──────────────────────────────┤
 *     │ constexpr / consteval           │ TEŞVİK EDİLİR                │
 *     │ auto tip çıkarımı               │ İZİNLİ (dikkatli kullanım)   │
 *     │ Range-based for                 │ TEŞVİK EDİLİR                │
 *     │ Akıllı işaretçiler (unique_ptr) │ İZİNLİ                       │
 *     │ std::optional, std::variant     │ İZİNLİ                       │
 *     │ Templates / CRTP                │ İZİNLİ                       │
 *     │ Lambda ifadeleri                │ İZİNLİ (durumsuz tercih)     │
 *     │ Structured bindings             │ İZİNLİ                       │
 *     │ if constexpr                    │ TEŞVİK EDİLİR                │
 *     └─────────────────────────────────┴──────────────────────────────┘
 *
 * =============================================================================
 * [THEORY: CERT C++ Secure Coding Standard / TEORİ: CERT C++ Güvenli Kodlama Standardı]
 * =============================================================================
 *
 ...
 *
 * TR: CERT C++, güvenlik açıklarına odaklanır:
 *     - Buffer overflow önleme (sınır kontrolü)
 *     - Integer overflow/underflow tespit
 *     - Kaynak yönetimi (RAII, sızıntı yok)
 *     - Null pointer dereference önleme
 *     - Thread güvenliği (veri yarışı önleme)
 *
 * =============================================================================
 * [THEORY: Static Analysis Tools / TEORİ: Statik Analiz Araçları]
 * =============================================================================
 *
 * EN:
 *     ┌──────────────────────┬──────────────────────────────────────────┐
 ...
 *     └──────────────────────┴──────────────────────────────────────────┘
 *
 * TR: Statik analiz araçları, kodu çalıştırmadan hataları tespit eder:
 *     ┌──────────────────────┬──────────────────────────────────────────┐
 *     │ Araç                 │ Temel Özellikler                         │
 *     ├──────────────────────┼──────────────────────────────────────────┤
 *     │ cppcheck             │ Açık kaynak, MISRA eklentisi mevcut      │
 *     │ clang-tidy           │ LLVM tabanlı, CERT/MISRA kontrolleri     │
 *     │ Parasoft C++test     │ Ticari, MISRA/CERT/AUTOSAR desteği       │
 *     │ Polyspace (MathWorks)│ Çalışma zamanı hatasız olduğunu kanıtlar │
 *     │ QA·C++ (Perforce)    │ MISRA C++ referans denetleyicisi         │
 *     │ Coverity (Synopsys)  │ Büyük ölçekli hata tespiti               │
 *     │ PVS-Studio           │ Statik analiz, MISRA desteği             │
 *     └──────────────────────┴──────────────────────────────────────────┘
 *
...
// TR: UYUMSUZ — int'ten uint8_t'ye örtülü daraltma
/*
    void bad_narrowing() {
        int value = 300;
...
// TR: UYUMLU — aralık kontrolü ile açık dönüşüm
...
// TR: UYUMSUZ: C-tarzı cast kullanımı | UYUMLU: static_cast/const_cast/reinterpret_cast kullan
...
// TR: İstisnasız hata yönetimi — dönüş kodları + std::optional
...
// TR: MISRA uyumlu sensör okuma (istisna yok)
...
inline std::string_view GearToString(GearPosition gear) {
    switch (gear) {
        ...
        // TR: MISRA: TÜM enum değerleri ele alınmalı
    }
    return "?";  // unreachable if all enums covered — TR: tüm enum değerleri kapsandıysa ulaşılamaz
}
...
// TR: UYUMSUZ: #define makro sabiti | UYUMLU: constexpr ile derleme zamanı sabiti
...
// TR: constexpr fonksiyon — derleme zamanında hesapla
...
// TR: virtual + dynamic_cast yerine CRTP kullan
...
// TR: Taşma tespitli güvenli tamsayı aritmetiği
...
// TR: Sınır kontrollü dizi erişimi
...
// TR: RAII tabanlı çevre birimi kilidi (manuel kilit açma gerekmez)
...
// TR: Null-güvenli pointer sarmalayıcı
template <typename T>
class NonNull {
    T* ptr_;
public:
...
// TR: Örnek: Motor hız denetleyicisi — uyumlu ve uyumsuz karşılaştırma
...
// TR: Tür-güvenli register erişimi (MISRA uyumlu)
...
struct MISRARule {
    std::string rule_id;
    std::string category;   // Mandatory/Required/Advisory — TR: Zorunlu/Gerekli/Tavsiye
