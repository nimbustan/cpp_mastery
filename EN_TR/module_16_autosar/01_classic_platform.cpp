 ...
 * @brief AUTOSAR Classic Platform — Layered Architecture / Katmanlı Mimari
 *
 * @details
 * =============================================================================
 * [THEORY: AUTOSAR Classic Platform — Overview / TEORİ: Genel Bakış]
 * =============================================================================
 *
 ...
 *     └──────────────────────────────────────────────────────────────┘
 *
 * TR: AUTOSAR Classic Platform, mikrodenetleyici tabanlı ECU'lar için fiili
 *     standart yazılım mimarisidir. Uygulama mantığını donanımdan ayıran
 *     katmanlı bir mimari tanımlar; taşınabilirlik, yeniden kullanılabilirlik
 *     ve standart arayüzler sağlar.
 *
 *     AUTOSAR Classic Mimarisi:
 *     ┌────────────────────────────────────────────────────────────┐
 *     │                   UYGULAMA KATMANI                         │
 *     │   ┌──────────┐  ┌──────────┐  ┌──────────┐  ┌──────────┐   │
 *     │   │  SWC_1   │  │  SWC_2   │  │  SWC_3   │  │  SWC_N   │   │
 ...
 *     │  Sanal Fonksiyon Veri Yolu — tüm SWC iletişimini soyutlar  │
 *     ├────────────────────────────────────────────────────────────┤
 *     │           BSW (Temel Yazılım)                              │
 *     │  ┌────────────────────────────────────────────────────┐    │
 *     │  │ Servis Katmanı: OS, COM, NvM, Dcm, Dem, BswM, EcuM │    │
 *     │  ├────────────────────────────────────────────────────┤    │
 *     │  │ ECU Soyutlama: CanIf, SpiIf, DioIf, AdcIf, PwmIf   │    │
 *     │  ├────────────────────────────────────────────────────┤    │
 *     │  │ MCAL: Can, Spi, Dio, Adc, Pwm, Gpt, Mcu, Port, Icu │    │
 *     │  └────────────────────────────────────────────────────┘    │
 *     ├────────────────────────────────────────────────────────────┤
 *     │           DONANIM (MCU + Çevresel Birimler)                │
 *     └────────────────────────────────────────────────────────────┘
 *
 * =============================================================================
 * [THEORY: SWC — Software Component / TEORİ: Yazılım Bileşeni]
 * =============================================================================
 *
 ...
 *     └──────────────────┴───────────────────────────────────────────┘
 *
 * TR: SWC, uygulama yazılımının atomik birimidir. Her SWC:
 *     - Bir veya daha fazla Runnable (çalıştırılabilir varlık) içerir
 *     - Yalnızca Port'lar aracılığıyla iletişim kurar (Sender/Receiver veya Client/Server)
 *     - Donanımdan bağımsızdır (eşleşen RTE'ye sahip herhangi bir ECU'da çalışır)
 *
 *     Port Türleri:
 *     ┌──────────────────┬────────────────────────────────────────────┐
 *     │ Port Türü        │ Açıklama                                   │
 *     ├──────────────────┼────────────────────────────────────────────┤
 *     │ SenderReceiver   │ Asenkron veri geçişi (sinyal-seviyesi)     │
 *     │ ClientServer     │ Senkron fonksiyon çağrısı (istek/yanıt)    │
 *     │ ModeSwitchPort   │ Mod değişikliği bildirimi (EcuM, BswM)     │
 *     │ NvDataPort       │ Kalıcı veri erişimi (NvM arayüzü)          │
 *     └──────────────────┴────────────────────────────────────────────┘
 *
 * =============================================================================
 * [THEORY: COM Stack — PDU Routing / TEORİ: COM Yığını — PDU Yönlendirme]
 * =============================================================================
 *
 ...
 *
 * TR: İletişim yığını, Protocol Data Unit'leri (PDU) uygulamadan donanıma
 *     yönlendirir:
 *
 *     COM (Sinyal ↔ PDU grubu)
 *      ↓
 *     PduR (PDU Yönlendirici — yönlendirme tablosu)
 *      ↓
 *     CanIf / LinIf / FrIf / SoAd (Arayüz katmanı)
 *      ↓
 *     CanDrv / LinDrv / FrDrv / TcpIp (Sürücü — MCAL)
 *      ↓
 *     CAN Denetleyici / LIN Transceiver / FlexRay CC / Ethernet PHY
 *
 * =============================================================================
 * [THEORY: AUTOSAR OS — OSEK/VDX Based / TEORİ: AUTOSAR OS — OSEK/VDX Tabanlı]
 * =============================================================================
 *
 ...
 *     └──────────┴─────────────────────────────────────────────────────┘
 *
 * TR: AUTOSAR OS, OSEK/VDX OS'u genisletir:
 *     - Ölçeklenebilirlik Sınıfları (SC1-SC4): temel görevlerden bellek korumaya
 *     - Görevler: Temel (preemptive olmayan) ve Genişletilmiş (olay bekleyebilir)
 *     - ISR Kategori 1 (OS çağrısı yok) ve Kategori 2 (OS çağrısına izin var)
 *     - Alarm: Görevlerin/geri çağrıların periyodik etkinleştirmesi
 *     - Schedule Table: Deterministik etkinleştirme dizisi
 *     - Counter: Donanım veya yazılım tik kaynakları
 *
 *     ┌───────────┬──────────────────────────────────────────┐
 *     │ SC Sınıfı │ Özellikler                               │
 *     ├───────────┼──────────────────────────────────────────┤
 *     │ SC1       │ Temel görevler, ISR Kat2, Alarm, Counter │
 *     │ SC2       │ SC1 + Zamanlandırma Koruması             │
 *     │ SC3       │ SC1 + Bellek Koruması                    │
 *     │ SC4       │ SC2 + SC3 (tam koruma)                   │
 *     └───────────┴──────────────────────────────────────────┘
 *
...
// TR: Port arayüzleri — Sender/Receiver ve Client/Server soyutlama
...
// TR: RTE, "Virtual Function Bus" işlevi görür. SWC'ler arası doğrudan
//     çağrı yasaktır; tüm iletişim RTE üzerinden geçer.
...
class RTE {
    ...
    // TR: Client/Server fonksiyon kaydı
...
public:
    ...
    // TR: RTE'ye sinyal yaz (gönderici tarafı)
...
// TR: Task önceliği (yüksek değer = yüksek öncelik)
...
// TR: Alarm — periyodik veya tek seferlik task aktivasyonu
struct Alarm {
    std::string name;
...
// TR: Minimal AUTOSAR OS zamanlayıcı simülasyonu
class OS {
...
public:
    ...
    // TR: Sistem tick'ini ilerlet ve alarmları kontrol et
    void Tick() {
        ++tick_counter_;
    ...
    // TR: Öncelik tabanlı zamanlama (tüm READY task'lar öncelik sırasıyla çalışır)
    void Schedule() {
        for (;;) {
            Task* best = nullptr;
            for (auto& t : tasks_) {
...
// TR: PDU — temel veri alışveriş birimi
...
// TR: PDU Router — PDU'ları COM, CanIf, LinIf, SoAd arasında yönlendirir
...
// TR: NvM — kalıcı bellek blok yöneticisi
//     Mimari: NvM → MemIf → Fee(Flash) veya Ea(EEPROM) → Fls/Eep(MCAL)
...
public:
    ...
    NvMRequestResult ReadBlock(uint16_t id) {
        ...
        // TR: NV'den RAM aynasına kopyala, CRC doğrula
...
// TR: DEM — Teşhis Olay Yöneticisi: DTC'leri depolar
...
// TR: EcuM fazları: STARTUP → UP (RUN/POST_RUN) → SHUTDOWN → SLEEP/OFF
...
public:
    void Init() {
        ...
        // TR: Init sırası: Mcu_Init, Port_Init, Dio_Init, SchM_Init, BswM_Init, ...
    ...
    void MainFunction() {
        ...
        // TR: Uyandırma kaynaklarını değerlendir, çalışma isteklerini yönet
    }
...
int main() {
    ...
    std::cout << "\n--- Demo 2: Client/Server via RTE ---\n";
    {
        ...
        // TR: Teşhis sunucusu kaydet (SWC_Diag ClearDTC servisi)
    ...
    std::cout << "\n--- Demo 4: COM Stack — PDU Routing ---\n";
    {
        ...
        // TR: COM'dan CanIf'e PDU yönlendir
        ...
        // TR: CAN bus'tan al ve COM'a yönlendir
    ...
    std::cout << "\n--- Demo 6: DEM — Diagnostic Event Manager (DTC lifecycle) ---\n";
    {
        ...
        // TR: Olay simülasyonu: C07300 3 kez fail → onaylandı, sonra iyileşir
