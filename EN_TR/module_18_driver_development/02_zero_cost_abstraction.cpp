 ...
 * @brief CRTP & Policy-Based Design — Zero-Cost Abstraction in C++
 *
 * @details
 * =============================================================================
 * [THEORY: Virtual Dispatch vs CRTP — Cost Analysis / TEORİ: Sanal Dağıtım ve CRTP — Maliyet Analizi]
 * =============================================================================
 *
 ...
 *     └──────────────────┴───────────────┴──────────────────┘
 *
 * TR: Virtual dispatch çalışma zamanı ek maliyeti oluşturur:
 *     - vtable işaretçisi: nesne başına +8 bayt (64-bit)
 *     - Dolaylı çağrı: pipeline durması, önbellek ıskası
 *     - Derleyici satır içi yapamaz
 *
 *     CRTP derleme zamanında çözerek tüm ek maliyeti sıfırlar.
 *
 ...
 *     │ vptr    │──→ vtable      │ (ptr yok)│
 *     └─────────┘   ┌──────┐     └──────────┘
 *                   │func1 │     Derleyici çözer:
 *                   │func2 │     Derived::func() satır içi
 *                   └──────┘
 *
 *     Ek maliyet karşılaştırması:
 *     ┌──────────────────┬───────────────┬──────────────────┐
 *     │ Metrik           │ Virtual       │ CRTP             │
 *     ├──────────────────┼───────────────┼──────────────────┤
 *     │ Nesne boyutu     │ +8 bayt(vptr) │ Ek yük yok       │
 *     │ Çağrı maliyeti   │ ~2-5 döngü    │ 0 (satır içi)    │
 *     │ Satır içi yapılır│ Hayır         │ Evet             │
 *     │ Çalışma zamanı   │ ✅ Esnek      │ ❌ Derleme zamanı│
 *     └──────────────────┴───────────────┴──────────────────┘
 *
 * =============================================================================
 * [THEORY: Policy-Based Design / TEORİ: Politika Tabanlı Tasarım]
 * =============================================================================
 *
 ...
 *
 * TR: Policy-based design, kaygıları bağımsız politika sınıflarına ayırır:
 *
 *     template<typename CommPolicy, typename ErrorPolicy>
 *     class Sensor : public CommPolicy, public ErrorPolicy { ... };
 *
 ...
 *     Sensor<I2C_Comm, ResetError>  sensor_i2c;
 *
 *     Her kombinasyon derlemede ayrı optimum kod üretir — çalışma zamanı
 *     ek maliyeti sıfır.
 *
 * =============================================================================
 * [THEORY: Embedded C++ Compile Flags / TEORİ: Gömülü C++ Derleme Bayrakları]
 * =============================================================================
 *
 ...
 *     └────────────────────────┴────────────────────────────────────┘
 *
 * TR: Gömülü C++ için derleme bayrakları:
 *     ┌────────────────────────┬────────────────────────────────────┐
 *     │ Bayrak                 │ Etki                               │
 *     ├────────────────────────┼────────────────────────────────────┤
 *     │ -fno-exceptions        │ C++ istisnalarını devre dışı bırak │
 *     │ -fno-rtti              │ RTTI'yi devre dışı bırak           │
 *     │ -fno-threadsafe-statics│ Statik init korum. devre dışı bırak│
 *     │ -fno-unwind-tables     │ Yığın geri sarım tablolarını kaldır│
 *     │ -Os / -Oz              │ Boyut için optimize et             │
 *     │ -ffunction-sections    │ Her fonksiyonu kendi bölümüne koy  │
 *     │ -fdata-sections        │ Her veriyi kendi bölümüne koy      │
 *     │ -Wl,--gc-sections      │ Bağlayıcı kullanılmayanı kaldırır  │
 *     └────────────────────────┴────────────────────────────────────┘
 *
...
// TR: --- Virtual versiyon (çalışma zamanı polimorfizmi) ---
...
// TR: --- CRTP versiyon (derleme zamanı polimorfizmi) ---
...
// TR: Boyut karşılaştırma yardımcısı
...
// TR: Jenerik fonksiyon — herhangi bir CRTP sensörüyle çalışır (derleme zamanı polimorfizm)
...
// TR: SPI iletişim arka ucu
class SPI_Backend {
protected:
    static uint16_t TransferRaw(uint8_t reg, [[maybe_unused]] uint8_t dummy) {
        // EN: Simulated SPI register read
        // TR: Simüle edilmiş SPI register okuma — gerçek donanımda SPI MISO hattından veri gelir
...
// TR: I2C iletişim arka ucu
class I2C_Backend {
protected:
    static uint16_t TransferRaw(uint8_t reg, [[maybe_unused]] uint8_t dummy) {
        // EN: Simulated I2C register read
        // TR: Simüle edilmiş I2C register okuma — gerçek donanımda I2C SDA hattından veri gelir
...
// TR: CRTP sensör sürücüsü — arka uç derleme zamanında seçilir
...
// TR: BMP280 basınç/sıcaklık sensörü — Bosch üretimi, I2C/SPI destekli barometrik sensör
template <typename CommBackend>
...
struct IgnoreErrorPolicy {
    static void HandleError([[maybe_unused]] std::string_view msg) {
        // EN: Silently ignore
        // TR: Sessizce yoksay — hata mesajı üretilmez, üretimde dikkatli kullanılmalı
    }
    static std::string_view ErrorPolicyName() { return "IgnoreError"; }
};

// EN: Policy-based sensor class
// TR: Politika tabanlı sensör sınıfı
template <typename CommPolicy, typename ErrorPolicy>
class Sensor {
...
// TR: Platform-specific başlatma (ölü kod derleyici tarafından elenir)
template <typename Platform>
...
// TR: Dağıtım için etiket türleri
struct SPI_tag {};
struct I2C_tag {};
struct UART_tag {};

// EN: Overloaded init functions
// TR: Aşırı yüklenmiş init fonksiyonları
...
// TR: İletişim başlatma için etiket dağıtımı kullanan sensör — CommTag türüne göre doğru InitComm seçilir
...
// TR: Register adresi tanımları (derleme zamanı)
...
// TR: Derleme zamanı çevre birimi tablosu — tüm adresler ve IRQ'lar constexpr ile doğrulanır
...
// TR: Derleme zamanında doğrula — yanlış index kullanımı derleme hatasına dönüşür
...
// TR: Tampon boyutları — derleme zamanı sabitleri, RAM kullanımını önceden belirler
...
// TR: Derleme zamanı CRC polinomu kontrolü — sıfır polinom veri bütünlüğünü bozar
