 ...
 * @brief Peripheral Driver Development — GPIO, UART, SPI, ISR, State Machine
 *
 * @details
 * =============================================================================
 * [THEORY: Register-Level Driver Architecture / TEORİ: Yazmaç Düzeyi Sürücü Mimarisi]
 * =============================================================================
 *
 ...
 *     └──────────────────────────────────────────────────┘
 *
 * TR: Bare-metal sürücüler, bellek eşlemeli (memory-mapped) register'lara volatile
 *     işaretçiler aracılığıyla erişir. Her çevre birimi sabit bir taban adresinde
 *     bir register bloğuna sahiptir.
 *     - SET: İlgili biti 1 yapar → çevre birimini etkinleştirir
 *     - CLEAR: İlgili biti 0 yapar → özelliği devre dışı bırakır
 *     - MODIFY: Çok bitli alanı maskeleyerek güvenli değiştirir
 *     Neden önemli: Yanlış bit manipülasyonu donanımı kilitleyebilir.
 *     Ne zaman: Her register erişiminde bu temel işlemler kullanılır.
 *
 * =============================================================================
 * [THEORY: Type-Safe Register Access / TEORİ: Tip Güvenli Yazmaç Erişimi]
 * =============================================================================
 *
 ...
 *
 * TR: Ham bit manipülasyonu hataya açıktır — yanlış register'a yanlış bit
 *     yazılması donanım hatasına yol açar. Tip-güvenli RegisterField şablonu
 *     taban adresi, bit konumu ve genişliği derleme zamanında kapsüller.
 *     - Ne: Şablon parametreleri ile register alanını statik olarak tanımlar
 *     - Neden: Farklı çevre birimlerinin register'larını karıştırmayı engeller
 *     - Ne zaman: Birden fazla register'a erişen karmaşık sürücülerde kullanılır
 *
 * =============================================================================
 * [THEORY: Driver State Machine / TEORİ: Sürücü Durum Makinesi]
 * =============================================================================
 *
 ...
 *
 * TR: Her sürücü bu durum makinesini izler. Geçersiz geçişler reddedilir.
 *     Durum akışı: UNINITIALIZED →Init→ INITIALIZED →Start→ RUNNING
 *     Hata/Durdurma durumunda ERROR/STOPPED'a geçilir.
 *     DeInit() ile başlangıç durumuna dönülür.
 *
 * =============================================================================
 * [THEORY: ISR Callback Mechanism / TEORİ: ISR Geri Çağırma Mekanizması]
 * =============================================================================
 *
 ...
 *
 * TR: Gömülü sistemlerde çevre birimleri donanım kesmelerine (interrupt) neden olur.
 *     ISR (Kesme Servis Rutini) hızlı olmalıdır — genellikle bir bayrak ayarlar
 *     veya halka tamponuna veri yazar, ardından ana döngüye sinyal gönderir.
 *     ISR geri çağrı deseni:
 *     1. Sürücü bir callback fonksiyon işaretçisi kaydeder
 *     2. Simüle edilmiş IRQ handler callback'i çağırır
 *     3. Callback veri depolar / bayrak ayarlar (minimum iş)
 *     4. Ana döngü bayrak / veriyi işler
 *     Neden: ISR içinde uzun işlem yapmak kesme gecikmesini artırır.
 *     Ne zaman: UART RX, ADC dönüşüm tamamlandı gibi asenkron donanım olaylarında.
 *
...
// TR: Simüle edilmiş register değişkeni üzerinde temel bit işlemleri
struct RegOps {
    ...
    // TR: Register içinde çok bitli bir alanı değiştir
...
// TR: Tip-güvenli register alanı şablonu
...
// TR: Simüle edilmiş GPIO port register bloğu (STM32 düzenini yansıtır)
struct GPIO_Regs {
    uint32_t MODER   = 0;  // Mode register (2 bits per pin) — TR: Mod yazmacı (pin başına 2 bit)
    uint32_t OTYPER  = 0;  // Output type — TR: Çıkış tipi (push-pull/open-drain)
    uint32_t OSPEEDR = 0;  // Output speed — TR: Çıkış hızı
    uint32_t PUPDR   = 0;  // Pull-up/pull-down — TR: Yukarı/aşağı çekme direnci
    uint32_t IDR     = 0;  // Input data — TR: Giriş verisi (salt okunur)
    uint32_t ODR     = 0;  // Output data — TR: Çıkış verisi
    uint32_t BSRR    = 0;  // Bit set/reset (write-only) — TR: Bit set/reset (salt yazılır)
};
...
public:
    ...
    // TR: Pin modunu yapılandır — her pin için MODER (giriş/çıkış/alternatif) ve PUPDR (pull-up/down) ayarlanır
    void ConfigurePin(uint32_t pin, PinMode mode, PullType pull = PullType::NONE) {
        if (pin >= 16) return;
        // EN: MODER: 2 bits per pin
        // TR: MODER: pin başına 2 bit — 00=giriş, 01=çıkış, 10=alt fonksiyon, 11=analog
        uint32_t pos = pin * 2;
    ...
    void WritePin(uint32_t pin, bool state) {
        ...
        // TR: BSRR'yi ODR'ye uygula (donanım davranışını simüle eder) — atomik set/reset
        ApplyBSRR();
    }
    ...
    void TogglePin(uint32_t pin) {
        ...
        // TR: Geri döngü simülasyonu — çıkış pinlerinin değeri IDR'den okunabilir
    }
...
private:
    void ApplyBSRR() {
        ...
        // TR: Alt 16 bit = set (1 yap), üst 16 bit = reset (0 yap) — BSRR atomik pin kontrolü sağlar
        ...
        // TR: Çıkış pinleri için geri döngü — ODR değeri IDR'ye kopyalanır
        regs_.BSRR = 0;
    }
};
...
// TR: RX FIFO için basit halka tampon
template <size_t N>
class RingBuffer {
...
struct UART_Regs {
    uint32_t CR1 = 0;   // Control register 1 — TR: Kontrol yazmaçı 1
    uint32_t BRR = 0;   // Baud rate register — TR: Baud hızı yazmaçı
    uint32_t SR  = 0;   // Status register — TR: Durum yazmaçı
    uint32_t DR  = 0;   // Data register — TR: Veri yazmaçı
};

// EN: Status register bits
// TR: Durum register bitleri — TXE: gönderim tamponu boş, RXNE: alım tamponu dolu
constexpr uint32_t UART_SR_TXE  = (1u << 7);  // TX empty — TR: TX boş
constexpr uint32_t UART_SR_RXNE = (1u << 5);  // RX not empty — TR: RX boş değil
constexpr uint32_t UART_SR_TC   = (1u << 6);  // Transmission complete — TR: İletim tamamlandı

// EN: Control register bits
// TR: Kontrol register bitleri — UE: UART etkin, TE: verici etkin, RE: alıcı etkin
constexpr uint32_t UART_CR1_UE   = (1u << 0); // UART enable — TR: UART etkinleştir
constexpr uint32_t UART_CR1_TE   = (1u << 3); // Transmitter enable — TR: Verici etkinleştir
constexpr uint32_t UART_CR1_RE   = (1u << 2); // Receiver enable — TR: Alıcı etkinleştir
constexpr uint32_t UART_CR1_RXNEIE = (1u << 5); // RX interrupt enable — TR: RX kesmesi etkinleştir

// EN: Callback type for RX interrupt
// TR: RX kesme geri çağrı tipi — alınan her byte için çağrılacak fonksiyon imzası
...
public:
    ...
    // TR: Yoklamalı TX — TXE bayrağını bekle, byte yaz (gerçek HW'de çevrim döngüsü gerekir)
    bool TransmitByte(uint8_t byte) {
        if (!initialized_) return false;
        // EN: In real HW, we'd spin on !(regs_.SR & UART_SR_TXE)
        // TR: Gerçek HW'de TXE=0 iken döngüde beklenir — burada simülasyon olduğu için atlıyoruz
        regs_.DR = byte;
        regs_.SR |= UART_SR_TC;
        return true;
    }
    ...
    // TR: Byte alma simülasyonu — donanım DR'ye veri koymuş gibi davranır ve ISR tetikler
    void SimulateRxIRQ(uint8_t byte) {
        regs_.DR = byte;
        regs_.SR |= UART_SR_RXNE;
        // EN: ISR handler
        // TR: ISR işleyici — RXNE bayrağı kontrol edilir, veri FIFO'ya yazılır, callback çağrılır
        HandleRxIRQ();
    }
...
class SPIDriver {
    ...
    // TR: Simüle edilmiş kaydırma register'ı (geri döngü: MOSI → MISO) — test amaçlı
...
public:
    ...
    // TR: Tam çift yönlü transfer: tx_byte gönder, rx_byte döndür
    uint8_t Transfer(uint8_t tx_byte) {
        ...
        // TR: Geri döngü simülasyonu (MOSI → MISO, çeşitlilik için bit terslemesi) — gerçek HW'de slave cevap verir
    ...
    // TR: Çok byte'lık transfer — blok halinde veri gönderip alır, DMA ile hızlandırılabilir
...
// TR: Sürücü işlemleri için dönüş kodu (istisnalar yerine)
...
// TR: Durum makineli genel çevre birimi sürücüsü
class ManagedDriver {
...
// TR: Hafif Expected tipi (C++23 std::expected her yerde mevcut değildir)
...
public:
    ...
    // TR: Başarı yapıcısı — geçerli değer taşıyan Expected nesnesi oluşturur
    ...
    // TR: Hata yapıcısı — hata bilgisi taşıyan Expected nesnesi oluşturur (istisna yerine)
...
// TR: Örnek: Hata yönetimi ile ADC okuma
...
public:
    ...
    Expected<uint16_t, ADCError> Read(uint32_t channel) {
        ...
        // TR: Simüle edilmiş dönüşüm sonucu — gerçek HW'de ADC kanal değerine göre analog okuma yapar
...
int main() {
    ...
    std::cout << "--- Demo 1: Register Bit Manipulation ---\n";
    {
        ...
        // TR: Çok bitli alan: [5:4] bitlerini 0b11 yap — ModifyField maskeleme ile güvenli değiştirir
        reg = 0;
        reg_ops::RegOps::ModifyField(reg, 4, 2, 3);
        std::cout << "    ModifyField[5:4]=3: 0x" << std::hex << reg << std::dec << "\n";

        // EN: Type-safe RegisterField
        // TR: Tip-güvenli RegisterField — derleme zamanı bit konumu/genişliği doğrulaması sağlar
    ...
    std::cout << "\n--- Demo 3: UART Driver — TX Polling + RX ISR ---\n";
    {
        ...
        // TR: RX geri çağrı fonksiyonunu kaydet — her alınan byte'ta ISR içinden çağrılır
        int rx_count = 0;
        ...
        // EN: TX (polling)
        // TR: TX (yoklamalı) — TXE bayrağı kontrol edilerek byte byte gönderilir
        std::cout << "    TX: ";
        uart.TransmitString("Hi");
        std::cout << "sent 'Hi'\n";

        // EN: Simulate RX interrupts
        // TR: RX kesmelerini simüle et — donanım DR'ye veri koymuş gibi SimulateRxIRQ çağrılır
        ...
        // EN: Read from FIFO
        // TR: FIFO'dan oku — halka tamponundaki verileri sırayla çıkar (FIFO prensibi)
    ...
    std::cout << "\n--- Demo 4: SPI Driver — Full Duplex ---\n";
    {
        ...
        // EN: Single byte
        // TR: Tek byte transferi — tam çift yönlü: TX gönderilirken eşzamanlı RX alınır
        uint8_t rx = spi.Transfer(0xAA);
        std::cout << "    TX=0xAA → RX=0x" << std::hex << static_cast<int>(rx) << std::dec << "\n";

        // EN: Block transfer
        // TR: Blok transfer — birden çok byte'ı sırayla gönderip alır
    ...
    std::cout << "\n--- Demo 6: Error Handling with Expected ---\n";
    {
        ...
        // TR: Kalibrasyon öncesi okuma — kalibre edilmemiş ADC hata döndürmeli
        auto r1 = adc.Read(0);
        if (!r1.HasValue())
        ...
        // EN: Valid read
        // TR: Geçerli okuma — kalibre edilmiş ADC'den kanal 3 değeri başarıyla okunur
        auto r2 = adc.Read(3);
        if (r2.HasValue())
            std::cout << "    Read ch3: value=" << r2.Value() << "\n";

        // EN: Invalid channel
        // TR: Geçersiz kanal — MAX_CHANNELS üstü kanal numarası CHANNEL_INVALID hatası döndürür
        auto r3 = adc.Read(20);
        if (!r3.HasValue())
        ...
        // TR: Tüm geçerli kanalları oku — döngüde Expected<T,E> ile hata kontrolü yapılır
