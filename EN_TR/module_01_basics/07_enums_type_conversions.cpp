 ...
 *
 * TR: Klasik C-tarzı `enum` üyelerini kapsam dışına sızdırır ve `int`'e dolaylı olarak
 *     dönüşür. Bu isim çakışmasına ve hatalara neden olur.
 *     Modern `enum class` (C++11):
 *     1. Kapsamlı isimler: `Color::Red` (sadece `Red` değil).
 *     2. int'e dolaylı dönüşüm YOK — `static_cast` kullanmalısınız.
 *     3. Alt tür belirlenebilir: `enum class Gear : uint8_t`.
 *
 * =============================================================================
 ...
 *
 * TR: C++ iki tür dönüşüm yapar:
 *     1. Dolaylı (otomatik): Derleyici sessizce tipleri genişletir/daraltır.
 *        Daraltma dönüşümleri veri kaybına yol açabilir!
 *     2. Açık (cast):
 *        a. `static_cast<T>()`: Derleme zamanı dönüşümü (EN GÜVENLİ).
 *        b. `(T)x` veya `T(x)`: C-tarzı cast (TEHLİKELİ, kontrol yok).
 *        c. `reinterpret_cast`, `const_cast`, `dynamic_cast` ileri seviye
 *           durumlar için.
 *
 * =============================================================================
 ...
 *
 * TR: `auto` (C++11) derleyiciye başlatıcıdan tipi çıkarmasını söyler.
 *     Standart kodu azaltır ama dikkatli kullanılmalıdır:
 *     1. İyi: `auto it = vec.begin();` (uzun iterator tipinden kaçınır).
 *     2. Kötü: `auto x = 42;` (int mi? unsigned mı? long mu? — netlik kaybolur).
 *     3. `decltype(expr)`: Bir ifadenin tipini, onu çalıştırmadan çıkarır.
 *
...
// TR: Klasik enum üyeleri KAPSAM DIŞINA SIZAR. Bu bildirimden sonra RED, GREEN, BLUE
//     global isimler olur — sanki `const int RED = 0;` yazmışsınız gibi.
//     Sorun: Başka bir enum da RED tanımlarsa, DERLEME HATASI alırsınız (isim çakışması).
//     Ayrıca klasik enum int'e dolaylı dönüşür, yani `int x = RED;` sessizce derlenir.
//     Bu nedenlerle modern C++'ta `enum class` tercih edin.
...
//     `: uint8_t` ALT TÜR belirtir — her numaralandırıcı tek bayt (0-255) olarak saklanır.
//     Bu olmadan derleyici varsayılanı seçer (genellikle int).
//     Gömülü/otomotiv kodda, bellek düzeni kontrolü ve protokol serileştirmesi
//     (örn. CAN çerçeve baytları) için alt türü belirtmek gereklidir.
...
// TR: Tür takma adları, ilkel türlere anlamlı isimler verir. Her yere
//     `unsigned long` yazmak yerine `SensorId` yazarsınız — kod kendi kendini belgeler.
//     `typedef` eski C tarzı sözdizimidir. Çalışır ama "tersinden" okunur.
...
// TR: `using` (C++11) modern karşılıktır — atama gibi soldan sağa okunur.
//     `using MilliVolts = int;` demek "MilliVolts bir int'TIR" demektir.
//     Ayrıca şablon takma adlarını DESTEKLEYEn TEK sözdizimidir:
//         template<typename T> using Vec = std::vector<T>;  // typedef bunu yapamaz!
//     Tüm yeni kodda `using` tercih edin.
...
int main() {
  ...
  // TR: Sadece `GREEN` yazabiliriz — önek gerekmez. Bu "sızma" davranışıdır:
  //     GREEN, düz bir tamsayı sabiti gibi TÜM KAPSAMDA görünürdür.
  //     Başka bir enum'da Green olsaydı, isim çakışması hatası alırdınız.
  ...
  // TR: Klasik enum SESSİZCE int'e dönüşür — cast gerekmez!
  //     Bu uyarısız derlenir: derleyici GREEN'i sadece `1` olarak işler.
  //     Bu tehlikelidir çünkü yanlışlıkla renkleri vites pozisyonlarıyla
  //     karşılaştırabilirsiniz ve derleyici mantıksal hatayı yakalaMAZ.
  ...
  // TR: enum class üyeleri KAPSAMLIDIR — sadece "Drive" değil, GearPosition::Drive
  //     yazmalısınız. Bu, farklı enum'lar arasındaki isim çakışmasını önler.
  ...
  // TR: Klasik enum'dan farklı olarak, enum class int'e DOLAYLI dönüşmez.
  //     Aşağıdaki satır DERLEME HATASI verir:
  //         int gearNum = gear;   // HATA: 'GearPosition' → 'int' dönüştürülemez
  //     Açıkça dönüştürmek için static_cast<> KULLANMALISINIZ. Bu tasarım gereğidir —
  //     ilgisiz enum türlerinin tamsayılarla yanlışlıkla karışmasını önler.
  ...
  // TR: Alt tür uint8_t'dir (bildirimde belirttik).
  //     static_cast ile uint8_t'ye dönüştürmek ham bayt değerini verir.
  ...
  // TR: enum class switch ifadeleri için idealdir — bir durumu ele almayı
  //     unutursanız derleyici uyarı verebilir (-Wswitch ile).
  ...
  // TR: `unsigned long egtId = 1001UL;` yerine `SensorId egtId = 1001UL;` yazıyoruz.
  //     Derlenen kod BİREBİR AYNIDIR — tür takma adları yeni tür oluşturmaz, sadece yeni isim.
  //     Ama okunabilirlik artışı önemlidir: bu değişkenin sadece "herhangi bir unsigned long"
  //     değil, bir sensör kimliği tuttuğunu hemen anlarsınız.
  ...
  // TR: `auto` derleyiciye der ki: "sağ taraftaki değere bak ve türü kendin bul".
  //     13.8 bir double literal'dır     → voltage `double` olur.
  //     5 bir int literal'dır           → ecuCount `int` olur.
  //     std::string("...") bir string   → vinStr `std::string` olur.
  //     auto "tipsiz" demek DEĞİLDİR — tür derleme zamanında sabittir, sadece çıkarılır.
  auto voltage = 13.8;
  auto ecuCount = 5;
  ...
  // TR: DİKKAT: `auto x = "hello";` türü `const char*` çıkarır, `std::string` DEĞİL!
  //     std::string istiyorsanız: `auto s = std::string("hello");` yazın
  //     veya string literal son eki kullanın: `auto s = "hello"s;` (C++14, `using namespace std::literals;` gerektirir)
  ...
  // TR: GENİŞLETME (güvenli): int → double. Derleyici küçük türü otomatik olarak
  //     büyük türe yükseltir. 3500, veri kaybı olmadan 3500.0 olur.
  //     Bu, double'ın tüm int değerlerini temsil edebilmesinden kaynaklanır (2^53'e kadar).
  //     Genişletme dönüşümleri: bool→int, int→long, int→double, float→double.
  int rpm = 3500;
  ...
  // TR: DARALTMA (TEHLİKELİ!): double → int. Kesirli kısım kesilir.
  //     (kesilir, yuvarlanMAZ). 2.75, 3 değil 2 olur.
  //     Derleyici -Wconversion ile uyarı verebilir. C++11 küme parantezi başlatmada
  //     daraltma DERLEME HATASIDIR: `int x{2.75};` → hata!
  //     Daraltmayı açık ve bilinçli yapmak için her zaman static_cast kullanın.
  ...
  // TR: bool → int: `true` 1 olur, `false` 0 olur. Bu C++ standardı tarafından
  //     garanti edilir. Tersi de çalışır: sıfır olmayan herhangi bir int `true` olur,
  //     sıfır `false` olur. Bu, koşullarda yoğun olarak kullanılır:
  //         if (absFlag) { ... }  // çalışır çünkü sıfır olmayan = true
  bool absActive = true;
  ...
  // TR: `static_cast<T>(expr)` EN GÜVENLİ C++ cast'idir. Derleyici derleme
  //     zamanında dönüşümün geçerli olup olmadığını kontrol eder. Geçerli değilse
  //     (örn. ilgisiz bir sınıfa pointer cast'i), sessiz UB değil DERLEME HATASI alırsınız.
  //     Sayısal türler arasında bilinçli dönüşüm yapacağınızda static_cast kullanın.
  ...
  // TR: KLASİK TUZAK — Tamsayı bölmesi kesir kısmını atar! İKİ operand da int
  //     olduğunda, C++ TAMSAYI bölmesi yapar: 15000/7 = 2142 (2142.857... değil).
  //     Kesirli kısım sessizce atılır. Bu, C/C++ kodundaki en yaygın hatalardan biridir.
  ...
  // TR: ÇÖZÜM — Bölmeden önce EN AZ BİR operandı double'a cast edin.
  //     Bu, kayan noktalı bölmeyi zorlar ve kesirli kısmı korur.
  //     `static_cast<double>(15000) / 7` → `15000.0 / 7` → `2142.857...`
  double avgSegment =
  ...
  // TR: enum class'ı temel türüne dönüştürme. GearPosition::Sport değeri 4'tür.
  //     Önce uint8_t'ye (bildirdiğimiz alt tür), sonra yazdırma için int'e cast ediyoruz.
  //     Neden çift cast? Çünkü std::cout, uint8_t'yi sayı 4 olarak değil, karakter
  //     (ASCII 4) olarak işler. Dıştaki static_cast<int> onu yazdırılabilir sayıya çevirir.
  ...
  // TR: C-tarzı cast `(int)x` C dilinden miras kalmıştır. Basit görünür ama
  //     EN TEHLİKELİ cast'tir çünkü derleyici SIRALI OLARAK birden fazla cast dener:
  //         1. const_cast       (const'ı kaldırır — yazarsanız UB olabilir)
  //         2. static_cast      (normal tür dönüşümü)
  //         3. reinterpret_cast (ham bit yeniden yorumlama — neredeyse her zaman UB)
  //     Derleyicinin hangisini seçtiği üzerinde HİÇBİR KONTROLÜNÜZ YOKTUR. Derlenen
  //     ilkini seçer — sonuç tanımsız davranış olsa bile.
  //     Modern C++'ta HER ZAMAN isimli cast'leri (static_cast, vb.) kullanın.
  //     İsimli cast'ler grep/IDE ile aranabilir ve NİYETİNİZİ açıkça belirtir.
