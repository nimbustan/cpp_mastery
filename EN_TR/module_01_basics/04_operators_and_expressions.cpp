 ...
 *
 * TR: C++ zengin bir operatör setine sahiptir:
 * 1. Aritmetik       (+, -, *, /, %)
 * 2. Karşılaştırma   (==, !=, <, >, <=, >=)
 * 3. Mantıksal       (&&, ||, !)
 * 4. Bitsel          (&, |, ^, ~, <<, >>)
 * 5. Atama           (=, +=, -=, *=, /=, %=, <<=, >>=, &=, |=, ^=)
 * 6. Üçlü (Ternary)  (koşul ? doğru : yanlış)
 * 7. Artırma/Azaltma (++x ön, x++ son, --x, x--)
 *
 * =============================================================================
 * [CPPREF DEPTH: Operator Precedence & Associativity /
 *  CPPREF DERİNLİK: Operatör Önceliği ve İlişkilendirme Yönü]
 * =============================================================================
 ...
 *
 * TR: Operatörler katı bir "Öncelik Tablosu"na uyar (yüksek → düşük):
 *  1. `()`              (Parantez)       — En yüksek
 *  2. `* / %`                            — Soldan Sağa (L→R)
 *  3. `+ -`                              — L→R
 *  4. `<< >>`                            — L→R
 *  5. `< <= > >=`                        — L→R
...
int main() {
  ...
  // std::bitset<8> değerleri ikili (binary) formatta yazdırır.
