 ...
 * @brief std::filesystem & Binary I/O — Dosya Sistemi ve İkili G/Ç
 *
 * @details
 * =============================================================================
 * [THEORY: std::filesystem (C++17) / TEORİ: std::filesystem (C++17)]
 * =============================================================================
 *
 ...
 *
 * TR: C++17 <filesystem> ile tam, taşınabilir, platformlar arası dosya
 *     ve dizin işlemleri API'si geldi. Artık platforma özel koda gerek yok!
 *
 *     Temel tipler:
 *     ┌─────────────────────────┬────────────────────────────────────────────┐
 *     │ Tip                     │ Amaç                                       │
 *     ├─────────────────────────┼────────────────────────────────────────────┤
 *     │ std::filesystem::path   │ Dosya/dizin yolu nesnesi (operator/ ile)   │
 *     │ directory_entry         │ Tek dizin öğesi (dosya, dizin, sembolik)   │
 *     │ directory_iterator      │ Düz dizin taraması                         │
 *     │ recursive_dir_iterator  │ Alt dizinleri dahil özyinelemeli tarama    │
 *     │ file_status             │ Tür bilgisi (regular, directory, symlink)  │
 *     │ space_info              │ Disk kapasitesi, boş alan, kullanılabilir  │
 *     └─────────────────────────┴────────────────────────────────────────────┘
 *
 *     Temel fonksiyonlar:
 *     - exists(), is_regular_file(), is_directory(), is_symlink()
 ...
 *
 * =============================================================================
 * [THEORY: Binary I/O / TEORİ: İkili G/Ç]
 * =============================================================================
 *
 ...
 *
 * TR: İkili G/Ç ham baytları yazar (metin biçimlendirmesi yok):
 *
 *     Metin G/Ç vs İkili G/Ç:
 *     ┌──────────────┬───────────────────────┬──────────────────────────────┐
 *     │ Özellik      │ Metin G/Ç             │ İkili G/Ç                    │
 *     ├──────────────┼───────────────────────┼──────────────────────────────┤
 *     │ Mod          │ ios::in/out           │ ios::binary                  │
 *     │ Fonksiyonlar │ operator<< / >>       │ write() / read()             │
 *     │ Veri         │ Okunabilir metin      │ Ham baytlar (bellekteki gibi)│
 *     │ Satır sonu   │ Platforma göre dönüşür│ Aynen korunur                │
 *     │ Boyut        │ Değişken              │ Sabit (sizeof struct)        │
 *     │ Hız          │ Yavaş (biçimlendirme) │ Hızlı (doğrudan kopyalama)   │
 *     │ Kullanım     │ Yapılandırma, log, CSV│ Görüntü, veritabanı, FW      │
 *     └──────────────┴───────────────────────┴──────────────────────────────┘
 *
 *     İkili G/Ç deseni:
 *     ofstream out("data.bin", ios::binary);
 ...
 *     in.read(reinterpret_cast<char*>(&data), sizeof(data));
 *
 *     DİKKAT: Struct'ın ikili G/Ç ile taşınabilirliği sorunlu:
 *     - Farklı makineler farklı endianness kullanabilir
 *     - Struct padding derleyiciler arasında farklılık gösterebilir
 *     - Platformlar arası için __attribute__((packed)) veya #pragma pack kullanın
 *
...
void demo_directory_operations() {
    ...
    // TR: Test dosyaları oluştur — ofstream ile dosyaya metin yazılır
...
// TR: İkili serileştirme için paketlenmiş struct
#pragma pack(push, 1)
struct SensorRecord {
    uint32_t sensor_id;
    float temperature;
...
void demo_binary_io() {
...

    {
        ...
        // TR: Önce sayıyı yaz, sonra kayıtları
    ...
    // TR: Veri eşleşmesini doğrula — yazdığımız ve okuduğumuz verilerin aynı olduğunu kontrol et
...
void demo_filesystem_error_handling() {
    ...
    // TR: Yöntem 1: istisnalar (varsayılan)
    try {
        auto sz = fs::file_size("/nonexistent/path/file.txt");
        std::cout << "    Size: " << sz << "\n";  // never reached — TR: buraya ulaşılamaz
    ...
    // TR: Yöntem 2: error_code (istisna yok — gömülü için tercih edilen)
    std::error_code ec;
...
void demo_practical_patterns() {
    ...
    // TR: Kalıp 1: Belirli uzantılı tüm dosyaları bul
    ...
    // TR: Kalıp 2: Yazmadan önce çıktı dizininin var olduğundan emin ol
    ...
    // TR: Kalıp 3: Kapsam korumalısı ile geçici dosya
