# Clean Code Reference for C++ — C++ İçin Temiz Kod Referansı
...
### TR: Yaygın C++ İsimlendirme Stilleri
- Sınıf/Struct: PascalCase (`SensorManager`)
- Fonksiyon: camelCase veya snake_case
- Sabit: kPascal veya UPPER_SNAKE
- Private üye: sondaki alt çizgi (`count_`)

### Important Rules / Önemli Kurallar
1. **Names should reveal intent** — İsimler amacı açıklamalı
   - BAD:  `int d;  // elapsed time in days`
   - GOOD: `int elapsedDays;`
2. **Avoid abbreviations** — Kısaltmalardan kaçın
   - BAD:  `int tpc;  // temperature processing count`
   - GOOD: `int temperatureProcessingCount;`
3. **One letter names only for loops** — Tek harfli isimler sadece döngülerde
   - OK: `for (int i = 0; i < n; ++i)`
4. **Bool names should be questions** — Bool isimleri soru olmalı
   - `bool isActive;`, `bool hasPermission;`, `bool shouldRetry;`

---

## 2. Function Design / Fonksiyon Tasarımı
...
### TR: Fonksiyon Uzunluğu ve Karmaşıklığı
- Fonksiyon başına satır: ≤ 20 (maks 40)
- Parametreler: ≤ 3 (maks 5)

### Rules / Kurallar

1. **Do ONE thing** — Fonksiyon BİR şey yapsın
   ```cpp
...
2. **Use early return to reduce nesting** — Erken dönüş ile yuvalamayı azalt
   ```cpp
   // BAD: Deep nesting
...
3. **Parameter passing rules** — Parametre geçirme kuralları
   ```cpp
...
## 3. Code Smells to Avoid / Kaçınılacak Kod Kokuları
...
### TR: Yaygın C++ Kod Kokuları
- Ham `new`/`delete` → `make_unique`/`make_shared` kullan
- C-tarzı dönüşüm → `static_cast` kullan
- `#define` sabitleri → `constexpr` kullan
- Sihirli sayılar → isimli sabitler kullan

---

## 4. Header File Best Practices / Başlık Dosyası En İyi Uygulamaları
...
## 5. clang-format Configuration / clang-format Yapılandırması

### Recommended `.clang-format` file / Önerilen `.clang-format` dosyası
...
### Most Important Rules / En Önemli Kurallar
...
## 7. Modern C++ Idioms Checklist / Modern C++ Deyimleri Kontrol Listesi

### Before Committing / Commit Etmeden Önce
