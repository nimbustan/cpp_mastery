...
void openDatabaseModern(const std::string &address) {
  if (address.empty()) {
    ...
    // TR: Mimar kuralı: Sadece objeyi fırlat
...
int main() {
  ...
  // TR: Gelen -1 değerini `if` içine almayı unuttum. Program ZOMBİ olarak devam ediyor!
  openDatabaseOldWay( "");

  // 2. SAFETY OF EXCEPTIONS (Exception Güvenliği, Fail-Fast Yaklasimi)
  try {
    // EN: This will immediately blast an error and JUMP to catch!
    // TR: Bu fonksiyon anında fırlatma yapar ve Alt satırları okumadan 'catch' bloğuna DÜŞER!
    openDatabaseModern("");
