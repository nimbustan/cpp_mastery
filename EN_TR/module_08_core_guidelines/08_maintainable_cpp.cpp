 ...
 * @brief Maintainable C++ in Big Projects — Büyük Projelerde Sürdürülebilir C++
 *
 * @details
 * =============================================================================
 * [THEORY: What Makes C++ Hard to Maintain? / TEORİ: C++ Bakımını Zorlaştıran Nedir?]
 * =============================================================================
 *
 ...
 *
 * TR: Büyük C++ projeleri (100K+ satır) benzersiz zorluklarla karşılaşır:
 *     1. DERLEME SÜRELERİ: Tek bir başlık değişikliği binlerce dosyanın yeniden derlenmesini tetikler
 *     2. BAĞIMLILIK KARMAŞASI: Dairesel include'lar, sıkı bağlama, geçişken bağımlılıklar
 *     3. YAPI KARMAŞIKLIĞI: Platforma özel kod, koşullu derleme (#ifdef çorbası)
 *     4. ÖRTÜK DAVRANIŞ: Örtük dönüşümler, copy vs move, ADL sürprizleri
 *     5. ABI KIRILGANLIĞI: Sınıf düzenini değiştirmek ikili uyumluluğu bozar
 *
 *     Temel stratejiler:
 *     - Net bağımlılık grafikleriyle modüler mimari
 *     - Include'ları azaltmak için ileri bildirimler (forward declarations)
 *     - API sınırlarında PIMPL
 *     - CI'da statik analiz (clang-tidy, cppcheck)
 *     - C++'a özel kod gözden geçirme kontrol listeleri
 *
 * =============================================================================
 * [THEORY: Modular Architecture / TEORİ: Modüler Mimari]
 * =============================================================================
 *
 ...
 *
 * TR: Kodu net bağımlılık kurallarıyla katmanlara düzenle:
 *
 *     ┌─────────────────────────────────────────────────────┐
 *     │                Uygulama Katmanı                     │
 *     │   (main.cpp, CLI, GUI — her şeye bağımlı)           │
 *     ├─────────────────────────────────────────────────────┤
 *     │                  Servis Katmanı                     │
 *     │   (İş mantığı, orkestrasyon)                        │
 *     ├─────────────────────────────────────────────────────┤
 *     │                  Alan Katmanı                       │
 *     │   (Temel tipler, arayüzler — minimum bağımlılık)    │
 *     ├─────────────────────────────────────────────────────┤
 *     │               Altyapı Katmanı                       │
 *     │   (Veritabanı, ağ, dosya G/Ç — adaptörler)          │
 *     ├─────────────────────────────────────────────────────┤
 *     │               Platform Katmanı                      │
 *     │   (İşletim sistemi soyutlamaları, gömülü için HAL)  │
 *     └─────────────────────────────────────────────────────┘
 *
 *     Kurallar:
 *     - Bağımlılıklar SADECE AŞAĞIYA doğru akar (yukarı çağrı yok)
 *     - Katman sınırlarında arayüzler (soyut sınıflar) kullan
 *     - Alan katmanının SIFIR harici bağımlılığı olmalı
 *
 ...
 *
 * TR: BAĞIMLILIK TERSİNE ÇEVİRME İLKESİ (DIP):
...
// TR: ALAN KATMANI — Saf arayüz, uygulama detayı yok
...
// TR: ALTYAPI KATMANI — Somut uygulama
 ...
 *     └─────────────────────────┴──────────┴────────────────────────────┘
 *
 * TR: INCLUDE HİJYENİ — Büyük projelerde hızlı derleme için kurallar:
 *     1. Mümkün olduğunda İLERİ BİLDİRİM kullan (#include yerine)
 *     2. KULLANDIĞINI INCLUDE ET (IWYU)
 *     3. #pragma once veya include korumaları kullan
 *     4. Başlık içeriklerini MİNİMİZE et
 */
 ...
 *
 * TR: Statik analiz, kodu ÇALIŞTIRMADAN hataları yakalar.
 *     C++ projeleri için temel araçlar: clang-tidy, cppcheck, derleyici uyarıları.
 */

// EN: Example .clang-tidy configuration
// TR: Örnek .clang-tidy yapılandırması
/*
    Checks: >
      -*,
      bugprone-*,
 ...
 *
 * TR: C++'a Özgü Kod İnceleme Kontrol Listesi (yukarıda).
 */
 ...
 *
 * TR: Büyük C++ projelerini yeniden yapılandırma stratejileri:
 *     1. BOĞUCU İNCİR KALIPBI: Eski kodu yeni arayüzle sar, kademeli geçiş
 *     2. ARAYÜZ ÇIKAR: Somut sınıfın üstüne soyut temel sınıf ekle
 *     3. PIMPL GEÇİŞİ: Özel üyeleri Impl struct'ına taşı
 *     4. DERLEME ZAMANI DİKİŞİ: Test için şablon tabanlı bağımlılık enjeksiyonu
 */

// EN: Example of compile-time seam for testability
// TR: Test edilebilirlik için derleme zamanı dikişi örneği
...
// TR: Testlerde: çağrıları izleyen mock repository kullan
...
public:
    ...
    // EN: Test helpers / TR: Test yardımcıları
...
int main() {
    ...
    // ─── Demo 2: Compile-Time Seam (Testing) ────────────────────────────
    {
        ...
        // TR: Üretim: gerçek repository kullanır
        ...
        // TR: Test: mock repository kullanır (çağrıları izler)
