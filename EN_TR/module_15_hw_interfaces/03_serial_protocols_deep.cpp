 ...
 * @brief SPI/I2C/UART — Register-Level Deep Dive & Advanced Patterns
 *
 * @details
 * =============================================================================
 * [THEORY: Serial Protocol Register-Level Programming / TEORİ: Seri Protokol Yazmaç Düzeyi Programlama]
 * =============================================================================
 *
 ...
 *     └─────────┴────────┴────────────────────────────────────────┘
 *
 * TR: Bu dosya modül 14'teki genel bakıştan daha derine iner.
 *     Yazmaç seviyesinde çevre birimi başlatma, hata tespiti, kurtarma
 *     ve DMA entegrasyonu ele alınır.
 *
 *     SPI Yazmaç Haritası (STM32):
 *     ┌─────────┬────────┬────────────────────────────────────────┐
 *     │ Ofset   │ Ad     │ Temel Alanlar                          │
 *     ├─────────┼────────┼────────────────────────────────────────┤
 *     │ 0x00    │ CR1    │ MSTR, CPOL, CPHA, BR[2:0], SPE, DFF,   │
 *     │         │        │ SSM, SSI, LSBFIRST, CRCEN, BIDIMODE    │
 *     │ 0x04    │ CR2    │ TXEIE, RXNEIE, ERRIE, TXDMAEN,RXDMAEN  │
 *     │ 0x08    │ SR     │ TXE, RXNE, BSY, OVR, MODF, CRCERR      │
 *     │ 0x0C    │ DR     │ Veri yazmacı (8/16-bit)                │
 *     │ 0x14    │ CRCPR  │ CRC polinom yazmacı                    │
 *     └─────────┴────────┴────────────────────────────────────────┘
 *
 *     I2C Yazmaç Haritası (STM32):
 *     ┌─────────┬────────┬────────────────────────────────────────┐
 *     │ Ofset   │ Ad     │ Temel Alanlar                          │
 *     ├─────────┼────────┼────────────────────────────────────────┤
 *     │ 0x00    │ CR1    │ START, STOP, ACK, PE, SWRST            │
 *     │ 0x04    │ CR2    │ FREQ[5:0], DMAEN, ITEVTEN, ITERREN     │
 *     │ 0x08    │ OAR1   │ ADDMODE, ADD[9:0]                      │
 *     │ 0x14    │ SR1    │ SB, ADDR, BTF, TXE, RXNE, AF, ARLO     │
 *     │ 0x18    │ SR2    │ BUSY, MSL, TRA                         │
 *     │ 0x1C    │ CCR    │ F/S, CCR[11:0] — f_PCLK/(2×f_SCL)      │
 *     └─────────┴────────┴────────────────────────────────────────┘
 *
 *     UART Yazmaç Haritası (STM32):
 *     ┌─────────┬────────┬────────────────────────────────────────┐
 *     │ Ofset   │ Ad     │ Temel Alanlar                          │
 *     ├─────────┼────────┼────────────────────────────────────────┤
 *     │ 0x00    │ SR     │ TXE, TC, RXNE, ORE, FE, PE, NF         │
 *     │ 0x04    │ DR     │ Veri yazmacı (9-bit)                   │
 *     │ 0x08    │ BRR    │ DIV_Mantissa[11:0], Fraction[3:0]      │
 *     │ 0x0C    │ CR1    │ UE, TE, RE, RXNEIE, TXEIE, PCE, M      │
 *     │ 0x10    │ CR2    │ STOP[1:0], CLKEN, CPOL, CPHA           │
 *     │ 0x14    │ CR3    │ DMAT, DMAR, HDSEL, SCEN, EIE           │
 *     └─────────┴────────┴────────────────────────────────────────┘
 *
 *     DMA Entegrasyonu: Çevre birimi ↔ bellek transferini CPU müdahalesi
 *     olmadan gerçekleştirir — yüksek hızlı SPI Flash okumaları için gerekli.
 *
...
namespace spi_regs {
    // CR1 bit positions — TR: CR1 bit pozisyonları
    constexpr uint32_t CPHA      = 0;
    constexpr uint32_t CPOL      = 1;
    constexpr uint32_t MSTR      = 2;
    constexpr uint32_t BR_SHIFT  = 3;  // BR[2:0] — TR: Baud hızı böleni (3 bit)
    ...
    constexpr uint32_t DFF       = 11; // 0=8-bit, 1=16-bit — TR: veri çerçeve formatı

    // SR bit positions — TR: SR (Durum Yazmaçı) bit pozisyonları
    ...
    // CR2 bit positions — TR: CR2 bit pozisyonları
...
class SPIPeripheral {
    uint32_t CR1_ = 0;
    uint32_t CR2_ = 0;
    uint32_t SR_  = (1U << spi_regs::TXE);  // TXE = 1 at reset — TR: sıfırlamada TXE=1
    uint32_t DR_  = 0;
...
public:
    // EN: Register write
    // TR: Yazmaç yazma — doğrudan CR1'e değer atar (HAL katmanı yerine)
    ...
    // TR: Veri yazmacına yaz → aktarımı başlatır (full-duplex: aynı anda gönder/al)
    void write_DR(uint8_t data) {
        if (!(CR1_ & (1U << spi_regs::SPE))) return;  // SPI not enabled — TR: SPI etkin değil

        SR_ &= ~(1U << spi_regs::TXE);  // TXE = 0 during transfer — TR: aktarım sırasında
        SR_ |= (1U << spi_regs::BSY);     // BSY = 1 — TR: meşgul

        // EN: Simulate full-duplex transfer
        // TR: Tam çift yönlü aktarımı simüle et — MOSI'dan gönderirken MISO'dan alır
        uint8_t rx = slave_handler_ ? slave_handler_(data) : 0xFF;

        rx_fifo_.push(rx);
        SR_ |= (1U << spi_regs::RXNE);   // RXNE = 1 — TR: alınan veri var
        SR_ |= (1U << spi_regs::TXE);     // TXE = 1 (ready for next) — TR: sonraki veriye hazır
        SR_ &= ~(1U << spi_regs::BSY);    // BSY = 0 — TR: aktarım bitti
    }

    // EN: Read data register
    // TR: Veri yazmacını oku — RXNE sıfırlanır, FIFO'dan veri çekilir
    uint8_t read_DR() {
        if (rx_fifo_.empty()) return 0;
        uint8_t data = rx_fifo_.front();
        rx_fifo_.pop();
        if (rx_fifo_.empty())
            SR_ &= ~(1U << spi_regs::RXNE);  // RXNE = 0 — TR: alınan veri okundu
        return data;
    }
...
namespace i2c_regs {
    // CR1 — EN: Control Register 1 — TR: Kontrol Yazmacı 1
    constexpr uint32_t PE       = 0;
    constexpr uint32_t START    = 8;
    constexpr uint32_t STOP     = 9;
    constexpr uint32_t ACK      = 10;
    // SR1 — EN: Status Register 1 — TR: Durum Yazmacı 1
    constexpr uint32_t SB       = 0;   // Start bit generated — TR: Start biti üretildi
    constexpr uint32_t ADDR     = 1;   // Address sent — TR: Adres gönderildi
    constexpr uint32_t BTF      = 2;   // Byte transfer finished — TR: Bayt aktarımı bitti
    constexpr uint32_t TXE      = 7;   // TX empty — TR: Gönder tamponu boş
    constexpr uint32_t RXNE     = 6;   // RX not empty — TR: Alınan veri var
    constexpr uint32_t AF       = 10;  // Acknowledge failure — TR: Onay hatası (NACK)
    constexpr uint32_t ARLO     = 9;   // Arbitration lost — TR: Arbitrasyon kaybı
    constexpr uint32_t BERR     = 8;   // Bus error — TR: Bus hatası
}
...
class I2CPeripheral {
    ...
    // TR: Bus üzerindeki simüle edilmiş slave cihazlar (her biri 7-bit adres + yazmaç haritası)
    struct I2CSlave {
        uint8_t address;   // 7-bit — TR: 7-bit I2C adresi
...
public:
    ...
    // TR: Çevre birimini etkinleştir (PE=1 — bus erişimi başlar)
    void enable() { CR1_ |= (1U << i2c_regs::PE); }

    // EN: Generate START condition
    // TR: START koşulu üret — SDA, SCL yüksekken düşer (bus sahipliği alınır)
    bool start() {
    ...
    // TR: Adres baytı gönder (7-bit adres + R/W biti — 0=yaz, 1=oku)
    bool send_address(uint8_t addr_rw) {
        ...
        if (current_slave_ >= 0) {
            SR1_ |= (1U << i2c_regs::ADDR);  // address matched — TR: adres eşleşti
            SR1_ |= (1U << i2c_regs::TXE);
            return true;
        } else {
            SR1_ |= (1U << i2c_regs::AF);  // NACK — TR: cihaz bulunamadı
            return false;
        }
    }

    // EN: Write data byte
    // TR: Veri baytı yaz — ilk bayt yazmaç adresi, sonrakiler veri
    bool write_data(uint8_t data) {
        if (current_slave_ < 0) return false;

        if (byte_count_ == 0) {
            current_reg_ = data;  // first byte = register address — TR: ilk bayt = yazmaç adresi
        } else {
    ...
    // EN: Read data byte
    // TR: Veri baytı oku — slave'in yazmaç adresinden sıralı okuma yapar
    ...
    // TR: STOP koşulu üret — SDA, SCL yüksekken yükselir (bus serbest bırakılır)
    void stop() {
    ...
    // EN: Error detection
    // TR: Hata tespiti — NACK/arbitrasyon/bus hatası bayrakları SR1'den okunur
    struct I2CError {
        bool nack = false;
    ...
    void set_clock(uint32_t pclk_mhz, uint32_t target_khz) {
        ...
        // TR: CCR hesabı (standart mod) — saat bölücü değer
        // CCR = f_PCLK / (2 × f_SCL) — TR: saat bölücü formülü
...
namespace uart_regs {
    // SR — EN: Status Register — TR: Durum Yazmacı
    constexpr uint32_t PE_FLAG  = 0;   // Parity error — TR: Parite hatası
    constexpr uint32_t FE       = 1;   // Framing error — TR: Çerçeve hatası
    constexpr uint32_t NF       = 2;   // Noise flag — TR: Gürültü bayrağı
    constexpr uint32_t ORE      = 3;   // Overrun error — TR: Taşma hatası
    constexpr uint32_t IDLE     = 4;
    constexpr uint32_t RXNE     = 5;
    constexpr uint32_t TC       = 6;   // Transmission complete — TR: Gönderim tamamlandı
    constexpr uint32_t TXE      = 7;   // Transmit data register empty — TR: Gönder yazmacı boş
    // CR1 — EN: Control Register 1 — TR: Kontrol Yazmacı 1
    constexpr uint32_t UE       = 13;  // USART enable — TR: USART etkinleştir
    constexpr uint32_t TE       = 3;   // Transmitter enable — TR: Verici aç
    constexpr uint32_t RE       = 2;   // Receiver enable — TR: Alıcı aç
    constexpr uint32_t RXNEIE   = 5;   // RXNE interrupt enable — TR: RXNE kesme etkin
    constexpr uint32_t TXEIE    = 7;   // TXE interrupt enable — TR: TXE kesme etkin
    constexpr uint32_t PCE      = 10;  // Parity control enable — TR: Parite kontrolü etkin
    constexpr uint32_t M        = 12;  // Word length (0=8bit, 1=9bit) — TR: Kelime uzunluğu
}
...
public:
    ...
    // TR: Baud hızını yapılandır — BRR = f_PCLK / (16 × baud)
    // BRR = f_PCLK / (16 × baud) — TR: baud bölücü formülü
    ...
    // EN: Transmit byte
    // TR: Bayt gönder — TXE kontrolü yap, DR'ye yaz, bağlı UART'a ilet
    bool transmit(uint8_t data) {
        if (!(CR1_ & (1U << uart_regs::UE))) return false;
        if (!(CR1_ & (1U << uart_regs::TE))) return false;

        // Wait for TXE — TR: TXE bekle (gönder tamponu boş olmalı)
        ...
        // TR: İletimi simüle et → bağlı UART'a ulaşır (TX → RX çapraz bağlantı)
        if (connected_) {
    ...
    // TR: Bayt al (dahili çağrı — karşı UART'tan gelen veri)
    ...
    // TR: Alınan baytı oku — RXNE temizlenir, kuyruktan çekilir
    ...
    // TR: Taşma hatası simüle et (RXNE hala 1 iken yeni veri gelirse veri kaybolur)
...
// TR: DMA transferini simüle eder — çevre birimi ↔ bellek arası CPU müdahalesi olmadan veri taşır
struct DMAChannel {
    ...
    // TR: Bir DMA transfer adımını simüle et — Normal modda bittiğinde durur, Circular modda başa döner
    bool transfer_step(uint8_t data_in, uint8_t& data_out) {
        ...
        if (transferred >= data_count) {
            if (mode == Mode::Normal) {
                ...
                transferred = 0;  // circular — TR: döngüsel mod, başa sar
            }
        }
        return true;
    }
...
int main() {
    ...
    // ─── Demo 1: SPI Register-Level Init ─────────────────────────────────
    {
        ...
        // TR: Adım adım yazmaç yapılandırması (STM32 HAL kalıbı — önce CR1, sonra SPE)
        uint32_t cr1 = 0;
        cr1 |= (1U << spi_regs::MSTR);      // Master mode — TR: Master modu
        cr1 |= (1U << spi_regs::CPOL);       // CPOL=1 — TR: boşta yüksek
        cr1 |= (0U << spi_regs::CPHA);       // CPHA=0 → SPI Mode 2 — TR: düşen kenarda örnekle
        cr1 |= (0b010U << spi_regs::BR_SHIFT); // f_PCLK/8 — TR: saat bölücü
        cr1 |= (1U << spi_regs::SSM);        // Software slave management — TR: yazılımsal CS
        cr1 |= (1U << spi_regs::SSI);        // Internal slave select — TR: dahili slave seçimi
        // Do NOT set SPE yet — TR: SPE'yi henüz AYARLAMA (yapılandırma bitmeden)
        ...
        // TR: SPI'yı etkinleştir (SPE biti — tüm yapılandırma bittikten sonra set edilir)
        cr1 |= (1U << spi_regs::SPE);
        spi.write_CR1(cr1);
        std::cout << "\n  Step 2: SPE=1 → SPI enabled\n\n";

        // EN: Attach a simulated slave (ADXL345 accelerometer)
        // TR: Simüle edilmiş bir slave bağla (ADXL345 ivmeölçer — DEVID=0xE5)
        spi.set_slave_handler([](uint8_t tx) -> uint8_t {
            static uint8_t reg_addr = 0;
            static bool first = true;
            if (first) { reg_addr = tx; first = false; return 0; }
            first = true;
            if (reg_addr == 0x00) return 0xE5;  // DEVID — TR: cihaz kimliği
            if (reg_addr == 0x32) return 0x12;  // DATAX0 — TR: X ekseni alt bayt
            if (reg_addr == 0x33) return 0x00;  // DATAX1 — TR: X ekseni üst bayt
            return 0xFF;
        });

        // EN: Read DEVID register (0x00 | 0x80 for read)
        // TR: DEVID yazmacını oku (0x80 = okuma komutu | 0x00 = yazmaç adresi)
        spi.write_DR(0x80);  // read command + reg 0x00 — TR: okuma komutu + yazmaç adresi
        spi.read_DR();        // discard first byte — TR: ilk bayt at (çöp)
        spi.write_DR(0x00);   // clock out data — TR: veri için saat gönder
    ...
    // ─── Demo 2: SPI Mode Comparison ────────────────────────────────────
    {
        ...
        // TR: Yaygın cihazların SPI modları ve maksimum saat hızları
    ...
    // ─── Demo 3: I2C Register-Level Transaction ─────────────────────────
    {
        ...
        // TR: Bus üzerine cihazlar ekle (TMP102 sıcaklık sensörü + MPU6050 IMU)
        i2c.add_slave(0x48, {{0x00, 0x1A}, {0x01, 0x60}});  // TMP102 temp sensor — TR: TMP102 sıcaklık sensörü
        i2c.add_slave(0x68, {{0x75, 0x68}, {0x3B, 0x12}, {0x3C, 0x34}});  // MPU6050 — TR: 6 eksenli IMU
        ...
        // TR: MPU6050'den WHO_AM_I oku (cihaz kimliği doğrulama — 0x68 beklenir)
        std::cout << "  I2C Read: MPU6050 WHO_AM_I (0x68, reg 0x75):\n";
        std::cout << "    1. START\n";
        i2c.start();

        std::cout << "    2. Send addr 0x68 + WRITE\n";
        i2c.send_address(0x68 << 1 | 0);  // write mode — TR: yazma modu (R/W=0)
        ...
        i2c.send_address(0x68 << 1 | 1);  // read mode — TR: okuma modu (R/W=1)
        ...
        // TR: TMP102 yapılandırma yazmacına yaz (adres + veri sıralı gönderilir)
        ...
        // TR: Geçersiz adrese NACK testi (bus'ta cihaz yoksa AF bayrağı set edilir)
    ...
    // ─── Demo 4: UART Register-Level Comms ──────────────────────────────
    {
        ...
        // TR: UART1 yapılandır: 115200 baud, 42 MHz PCLK (BRR otomatik hesaplanır)
        ...
        // TR: UART1'den UART2'ye "HELLO" gönder (TX → RX çapraz bağlantı)
        ...
        // EN: Read on UART2
        // TR: UART2'den oku (RXNE bayrağı kontrolü ile)
        ...
        // TR: Hata yönetimi demosu — taşma (overrun) simülasyonu ve temizleme
    ...
    // ─── Demo 5: DMA + SPI Transfer ─────────────────────────────────────
    {
        ...
        // TR: Flash ardışık baytlar döndürür (0xA0, 0xA1, ... gerçek okuma simülasyonu)
        ...
        for (uint32_t i = 0; i < 8; ++i) {
            spi.write_DR(0xFF);  // clock out dummy bytes — TR: saat üretmek için kukla bayt gönder
