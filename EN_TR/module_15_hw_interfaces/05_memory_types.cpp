 ...
 * @brief Memory Technologies — EEPROM, FLASH, FRAM, RAM, DDR, NAND, NVMe
 *
 * @details
 * =============================================================================
 * [THEORY: Embedded Memory Technologies / TEORİ: Gömülü Bellek Teknolojileri]
 * =============================================================================
 *
 ...
 *     └──────────────┴───────────┴────────────┴──────────────┴────────────┘
 *
 * TR: Gömülü/otomotiv sistemlerinde farklı hız, dayanıklılık, güç ve
 *     veri koruma özelliklerine sahip birden fazla bellek türü kullanılır.
 *
 *     Bellek Taksonomisi:
 *     ┌───────────────────────────────────────────────────────────────┐
 *     │                        BELLEK                                 │
 *     ├────────────────────┬──────────────────────────────────────────┤
 *     │   UÇUCU (Volatile) │      KALICI (Non-Volatile)               │
 *     ├────────────────────┼──────────────────────────────────────────┤
 *     │ SRAM  — MCU önbelk.│ NOR Flash  — kod çalıştırma (XIP)        │
 *     │ DRAM  — ana bellek │ NAND Flash — toplu depolama              │
 *     │ DDR4/5             │ EEPROM     — bayt düzeyinde yapılandırma │
 *     │                    │ FRAM (FeRAM) — hızlı NV, yüksek dayanık. │
 *     │                    │ MRAM       — yeni nesil NV, hızlı        │
 *     │                    │ OTP ROM  — tek seferlik programlanabilir │
 *     └────────────────────┴──────────────────────────────────────────┘
 *
 *     Temel Metrikler:
 *     ┌──────────────┬───────────┬────────────┬──────────────┬────────────┐
 *     │ Bellek       │ Okuma     │ Yazma      │ Dayanıklılık │ Saklama    │
 *     ├──────────────┼───────────┼────────────┼──────────────┼────────────┤
 *     │ SRAM         │ ~1 ns     │ ~1 ns      │ Sınırsız     │ Uçucu      │
 *     │ DRAM/DDR     │ ~10 ns    │ ~10 ns     │ Sınırsız     │ Uçucu      │
 *     │ NOR Flash    │ ~70 ns    │ ~200 μs    │ 100K döngü   │ 20+ yıl    │
 *     │ NAND Flash   │ ~25 μs    │ ~200 μs    │ 10K-100K     │ 10+ yıl    │
 *     │ EEPROM       │ ~200 ns   │ ~5 ms      │ 1M döngü     │ 40+ yıl    │
 *     │ FRAM         │ ~100 ns   │ ~100 ns    │ 10^14 döngü  │ 10+ yıl    │
 *     │ MRAM         │ ~35 ns    │ ~35 ns     │ 10^15 döngü  │ 20+ yıl    │
 *     │ NVMe SSD     │ ~10 μs    │ ~20 μs     │ 3K-100K TBW  │ Güç kes.   │
 *     └──────────────┴───────────┴────────────┴──────────────┴────────────┘
 *
 *     Flash'ta "sadece bit temizlenebilir" kuralı geçerlidir (1→0):
 *     yeniden yazmadan önce sektör/blok silme zorunludur.
 *
...
// TR: EEPROM: bayt adreslenebilir, yavaş yazma, yüksek dayanıklılık

class EEPROM {
    static constexpr uint32_t SIZE = 32768;  // 256 Kbit = 32 KB
    static constexpr int PAGE_SIZE = 64;     // page write size — TR: sayfa yazma boyutu
    static constexpr int MAX_CYCLES = 1000000;

    std::array<uint8_t, SIZE> data_;
    std::array<uint32_t, SIZE / PAGE_SIZE> page_wear_;  // write cycles per page — TR: sayfa başına yazma döngüsü
    uint32_t total_writes_ = 0;

public:
    EEPROM() {
        data_.fill(0xFF);  // erased state — TR: silinmiş durum (tüm bitler 1)
        page_wear_.fill(0);
    }

    // EN: Byte read — fast (~200ns on real HW)
    // TR: Bayt okuma — hızlı (~200ns gerçek donanımda)
    uint8_t read_byte(uint16_t addr) const {
        return data_[addr % SIZE];
    }

    // EN: Sequential read
    // TR: Sıralı okuma (adres otomatik artar)
    ...
    // TR: Bayt yazma — yavaş (~5ms gerçek donanımda, aşınmış sayfa kontrolü yapılır)
    bool write_byte(uint16_t addr, uint8_t val) {
        uint32_t page = (addr % SIZE) / PAGE_SIZE;
        if (page_wear_[page] >= MAX_CYCLES) return false;  // worn out — TR: sayfa aşınmış
        data_[addr % SIZE] = val;
        page_wear_[page]++;
        total_writes_++;
        return true;
    ...
    // TR: Sayfa yazma (tek seferde PAGE_SIZE bayta kadar — sınırı aşmak hatalı sarmaya neden olur)
...
public:
    ...
    // TR: Okuma — hızlı, XIP destekler (kod doğrudan Flash'tan çalışır)
    uint8_t read(uint32_t addr) const { return data_[addr % TOTAL_SIZE]; }

    // EN: Program (can only change 1→0, not 0→1!)
    // TR: Programla (sadece 1→0 yapılabilir, 0→1 için silme zorunlu!)
    bool program(uint32_t addr, uint8_t val) {
        addr %= TOTAL_SIZE;
        // EN: Flash physics: can only clear bits, not set them
        // TR: Flash fiziği: sadece bitleri temizleyebilir (1→0), set edemez (0→1)
        data_[addr] &= val;
        return true;
    }

    // EN: Program word (4 bytes)
    // TR: 32-bit kelime programla (4 bayt sırayla yazılır)
    ...
    // TR: Sektör silme (yeniden programlamadan önce zorunlu — tüm sektör 0xFF olur)
...
// TR: FRAM: SRAM hızında, uçucu olmayan, çok yüksek dayanıklılık
...
public:
    ...
    // TR: Toplu yazma — sayfa sınırı kısıtlaması yok (EEPROM'dan farkı)
...
public:
    ...
    // EN: Read page
    // TR: Sayfa oku (blok + sayfa numarası ile adreslenir)
    ...
    // TR: Sayfa programla (önce blok silinmiş olmalı — Flash kuralı: 1→0)
    bool program_page(uint32_t block, uint32_t page,
        ...
        for (size_t i = 0; i < std::min(data.size(), static_cast<size_t>(PAGE_SIZE)); ++i) {
            blk.data[offset + i] &= data[i];  // can only clear bits — TR: sadece bit temizlenebilir (1→0)
        }
        return true;
    }

    // EN: Block erase
    // TR: Blok silme (NAND'da minimum silme birimi bloktur — 64 sayfa = 128KB)
...
// TR: Basit aşınma dengeleme: her zaman en az silinmiş bloğa yaz
//     (bloklar arası eşit aşınma sağlanır → ömür uzar)
class WearLeveler {
    NANDFlash& flash_;
...
public:
    ...
    // TR: En düşük silme sayısına sahip geçerli bloğu bul
...
int main() {
    ...
    // ─── Demo 1: EEPROM Read/Write ──────────────────────────────────────
    {
        ...
        // TR: Kalibrasyon verisini depola (EEPROM'un tipik kullanımı)
        struct CalibData {
        ...
        // EN: Read back
        // TR: Geri oku ve doğrula (EEPROM verisi kalıcıdır)
        CalibData read_cal{};
    ...
    // ─── Demo 2: Flash Program & Erase ──────────────────────────────────
    {
        ...
        // TR: Veri programla (NOR Flash'a 32-bit kelime yazma)
        ...
        // TR: Silmeden üzerine yaz — sadece bit temizlenebilir (1→0, 0→1 olmaz!)
        ...
        // TR: Doğru sıra: önce sil sonra programla (Flash kullanmanın temel kuralı)
    ...
    // ─── Demo 3: FRAM Fast NV Storage ────────────────────────────────────
    {
        ...
        // TR: Çökme sayaçı yaz (güç kesintisine dayanıklı depolama simülasyonu)
        uint32_t crash_count = 42;
        auto* p = reinterpret_cast<const uint8_t*>(&crash_count);
        fram.write_block(0x100, p, sizeof(crash_count));

        // EN: Read back
        // TR: Geri oku (FRAM'da yazma sonrası bekleme gerekmez)
        uint32_t read_val = 0;
        for (int i = 0; i < 4; ++i)
    ...
    // ─── Demo 4: NAND Wear Leveling ─────────────────────────────────────
    {
        ...
        // TR: Aşınma dengeleme olmadan: hep blok 0'a yaz (erken bozulur)
        std::cout << "  Without wear leveling (always use block 0):\n";
        for (int i = 0; i < 5; ++i) nand.erase_block(0);
        wl.print_wear_map();

        // EN: With wear leveling: spread writes
        // TR: Aşınma dengeleme ile: yazmaları dağıt (tüm bloklar eşit aşınır)
    ...
    // ─── Demo 6: ECC & Data Integrity ────────────────────────────────────
    {
        ...
        // TR: Basit parite tabanlı ECC simülasyonu (tek bit hatası tespiti)
        ...
        // TR: Tek bit bozulmasını simüle et (ECC ile tespit edilebilir)
        auto corrupted = orig_data;
        corrupted[2] ^= 0x04;  // flip bit 2 of byte 2 — TR: 2. baytın 2. bitini çevir
