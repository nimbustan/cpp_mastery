 ...
 * @brief GPIO & Register-Level Programming — Memory-Mapped I/O, Bitfields
 *
 * @details
 * =============================================================================
 * [THEORY: Memory-Mapped I/O and Registers / TEORİ: Bellek Eşlemeli G/Ç ve Yazmaçlar]
 * =============================================================================
 *
 ...
 *     └─────────┴────────┴──────────────────────────────────────────┘
 *
 * TR: Gömülü sistemlerde donanım çevre birimleri (GPIO, UART, SPI vb.)
 *     bellek-eşlemeli yazmaçlar ile kontrol edilir. CPU bu yazmaçlara normal
 *     bellek adresleri gibi erişir.
 *
 *     Bellek Haritası (ARM Cortex-M / STM32):
 *     ┌─────────────────┬─────────────────────────────────────────┐
 *     │ Adres Aralığı   │ Çevre Birimi                            │
 *     ├─────────────────┼─────────────────────────────────────────┤
 *     │ 0x0000_0000     │ Flash bellek (kod)                      │
 *     │ 0x2000_0000     │ SRAM                                    │
 *     │ 0x4000_0000     │ APB1 çevre birimleri (UART, I2C, SPI)   │
 *     │ 0x4001_0000     │ APB2 çevre birimleri (GPIO, ADC, TIM)   │
 *     │ 0x4002_0000     │ AHB1 çevre birimleri (DMA, RCC)         │
 *     │ 0xE000_0000     │ Cortex-M sistem (NVIC, SysTick)         │
 *     └─────────────────┴─────────────────────────────────────────┘
 *
 *     Yazmaç Erişim Kuralları:
 *     1. volatile kullan — derleyici okuma/yazmaları optimize etmesin
 *     2. Bit manipülasyonu — tek tek bitleri ayarla/temizle
 *     3. Paylaşılan yazmaçlarda oku-değiştir-yaz (RMW) kullan
 *     4. Bazı yazmaçlar salt-okunur veya salt-yazılır
 *
 *     GPIO Yazmaç Seti (STM32 tipik):
 *     ┌─────────┬────────┬──────────────────────────────────────────┐
 *     │ Ofset   │ Ad     │ Açıklama                                 │
 *     ├─────────┼────────┼──────────────────────────────────────────┤
 *     │ 0x00    │ MODER  │ Mod Yazmacı (giriş/çıkış/AF/analog)      │
 *     │ 0x04    │ OTYPER │ Çıkış Tipi (push-pull / open-drain)      │
 *     │ 0x08    │ OSPEEDR│ Çıkış Hızı (düşük/orta/yüksek/çok-yük)   │
 *     │ 0x0C    │ PUPDR  │ Pull-up / Pull-down                      │
 *     │ 0x10    │ IDR    │ Giriş Veri Yazmacı (salt-okunur)         │
 *     │ 0x14    │ ODR    │ Çıkış Veri Yazmacı                       │
 *     │ 0x18    │ BSRR   │ Bit Set/Reset Yazmacı (atomik)           │
 *     │ 0x1C    │ LCKR   │ Kilit Yazmacı                            │
 *     │ 0x20    │ AFRL   │ Alternatif Fonksiyon Düşük (pin 0-7)     │
 *     │ 0x24    │ AFRH   │ Alternatif Fonksiyon Yüksek (pin 8-15)   │
 *     └─────────┴────────┴──────────────────────────────────────────┘
 *
...
// TR: Gerçek donanımda volatile pointer kullanılır: volatile uint32_t* REG = ...;
//     Burada donanımı simüle eden bir sınıf kullanıyoruz; okuma/yazma davranışını
//     taklit eder (salt-okunur, salt-yazılır, yazma maskesi desteği ile).
...
class SimRegister {
    ...
    bool read_only_;
    bool write_only_;
    uint32_t write_mask_;  // bits that can be written — TR: yazılabilir bitler
...
class GPIOPort {
    ...
    SimRegister MODER;    // offset 0x00 — TR: Mod Yazmaçı
    SimRegister OTYPER;   // offset 0x04 — TR: Çıkış Tipi Yazmaçı
    SimRegister OSPEEDR;  // offset 0x08 — TR: Çıkış Hızı Yazmaçı
    SimRegister PUPDR;    // offset 0x0C — TR: Pull-up/Pull-down Yazmaçı
    SimRegister IDR;      // offset 0x10 (read-only) — TR: Giriş Veri Yazmaçı (salt-okunur)
    SimRegister ODR;      // offset 0x14 — TR: Çıkış Veri Yazmaçı
    SimRegister BSRR;     // offset 0x18 (write-only) — TR: Bit Set/Reset (salt-yazılır)

    uint32_t external_input_ = 0;  // simulated external voltage levels
                                   // TR: simüle harici gerilim seviyeleri
...
public:
    GPIOPort(std::string name)
        : port_name_(std::move(name)),
          ...
          OSPEEDR("OSPEEDR", 0),
          PUPDR("PUPDR", 0),
          IDR("IDR", 0, true),    // read-only — TR: salt-okunur
          ODR("ODR", 0),
          BSRR("BSRR", 0, false, true)  // write-only — TR: salt-yazılır
    {}

    // EN: Configure pin mode (2 bits per pin in MODER)
    // TR: Pin modunu yapılandır (MODER'de her pin için 2 bit)
    ...
    // TR: Çıkış pinine yaz (ODR üzerinden — oku-değiştir-yaz gerektirir)
    void write_pin(int pin, bool high) {
        ...
        // TR: Çıkış pinleri için IDR'ı güncelle
        update_idr();
    }

    // EN: Atomic write via BSRR (no read-modify-write needed)
    // TR: BSRR ile atomik yazma (RMW gerektirmez — ISR-güvenli)
    ...
    // EN: Read input pin
    // TR: Giriş pinini oku (IDR'den — donanım tarafından güncellenir)
    bool read_pin(int pin) const {
        return (IDR.read() >> static_cast<uint32_t>(pin)) & 1;
    }

    // EN: Simulate external input (for input pins)
    // TR: Harici girişi simüle et (giriş pinleri için — buton/sensör yerine geçer)
...
private:
    void update_idr() {
        ...
        // TR: IDR çıkış pinlerinde ODR'yi, giriş pinlerinde harici durumu yansıtır
        uint32_t idr_val = 0;
        for (int i = 0; i < 16; ++i) {
        ...
        // TR: Simülasyonda IDR'yi zorla güncelle (gerçek donanımda IDR'yi donanım sürer)
...
// TR: Yazmaç programlamada kullanılan temel bit manipülasyonu kalıpları
namespace BitOps {
    // Set bit at position — TR: Belirtilen pozisyonda biti ayarla
    constexpr uint32_t set_bit(uint32_t reg, int bit) {
        return reg | (1U << static_cast<uint32_t>(bit));
    }
    // Clear bit at position — TR: Belirtilen pozisyonda biti temizle
    constexpr uint32_t clear_bit(uint32_t reg, int bit) {
        return reg & ~(1U << static_cast<uint32_t>(bit));
    }
    // Toggle bit — TR: Bit değiştir (XOR)
    ...
    // TR: Bit alanı ayarla: önce temizle, sonra değer yaz
    ...
    // Read bitfield — TR: Bit alanı oku
...
// TR: Herhangi bir çevre birimini kullanmadan önce RCC'de (Reset ve Saat
//     Kontrolü) saat sinyali etkinleştirilmelidir. Bunu unutmak klasik bir hatadır
//     — yazımışç 0 okur ve nedenini anlayamaz!

class RCCSimulator {
    SimRegister AHB1ENR;  // AHB1 Enable Register (GPIO clocks) — TR: GPIO saat sinyali
    SimRegister APB1ENR;  // APB1 Enable (UART, I2C, SPI) — TR: seri iletişim saatleri
    SimRegister APB2ENR;  // APB2 Enable (ADC, TIM) — TR: ADC/Zamanlayıcı saatleri

    // EN: Clock enable bit positions
    // TR: Saat etkinleştirme bit pozisyonları
...
int main() {
    ...
    // ─── Demo 1: Register Bit Manipulation ──────────────────────────────
    {
        ...
        // TR: Çoklu-bit alanı işlemleri (yazmaçtaki belirli bir alanı ayarla/oku)
        reg = 0;
    ...
    // ─── Demo 3: GPIO Output Configuration ─────────────────────────────
    {
        ...
        // TR: PA5'i çıkış olarak yapılandır: push-pull, orta hız, pull yok
        //     (STM32 Nucleo kartındaki dahili LED pini)
    ...
    // ─── Demo 4: GPIO Input (Button) ────────────────────────────────────
    {
        ...
        // TR: PC13'ü giriş olarak yapılandır, dahili pull-up ile
        //     (buton başına basılınca LOW seviyesine çeker — active-low)
        ...
        // TR: Buton durumlarını simüle et (pull-up ile serbest=HIGH, basılı=LOW)
        gpioc.set_external_input(13, true);  // not pressed (pull-up) — TR: basılmamış (pull-up=HIGH)
        std::cout << "    Button released (HIGH): " << gpioc.read_pin(13) << "\n";

        gpioc.set_external_input(13, false);  // pressed (active-low) — TR: basılı (active-low=LOW)
    ...
    // ─── Demo 5: Multiple Pin Configuration ─────────────────────────────
    {
        ...
        // TR: Tipik ECU GPIO pin ataması (otomotiv uygulaması: ADC, SPI, I2C,
        //     UART, CAN, LED — tümü aynı GPIO portu üzerinde yapılandırılır)
        struct PinConfig {
            int pin;
            GPIOMode mode;
            GPIOSpeed speed;
    ...
    // ─── Demo 6: volatile Keyword Importance ─────────────────────────────
    {
        ...
        // TR: Kavramı göster (gerçek donanım olmadan — volatile okuma/yazma
        //     derleyicinin değeri önbelleğe almasını önler)
