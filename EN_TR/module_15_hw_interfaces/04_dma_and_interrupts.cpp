 ...
 * @brief DMA Engines & Interrupt Handling — NVIC, ISR, DMA Streams
 *
 * @details
 * =============================================================================
 * [THEORY: Interrupt System (ARM Cortex-M NVIC) / TEORİ: Kesme Sistemi (ARM Cortex-M NVIC)]
 * =============================================================================
 *
 ...
 *     └─────────┴──────────────────────────────────┘
 *
 * TR: NVIC (Nested Vectored Interrupt Controller), ARM Cortex-M işlemcilerde
 *     tüm kesmeleri öncelik tabanlı yönetir:
 *     - 240'a kadar dış kesme + 16 sistem istisnası
 *     - Programlanabilir öncelik (0-255, düşük sayı = yüksek öncelik)
 *     - Öncelik gruplama: preemption önceliği + alt-öncelik
 *     - Tail-chaining: ardışık ISR'lar tam bağlam geri yüklemesi olmadan çalışır
 *     - Late-arriving: yığma (stacking) sırasında gelen yüksek öncelikli kesme önce alınır
 *
 *     Kesme Yaşam Döngüsü:
 *     ┌─────────┐   ┌───────────┐   ┌─────────┐   ┌──────────┐
 *     │ Tetikle │ → │ Beklemede │ → │ Aktif   │ → │ Dönüş    │
 *     └─────────┘   └───────────┘   └─────────┘   └──────────┘
 *        HW olay      NVIC kuyruğa   ISR çalışır   Bağlam geri yükle
 *
 *     Vektör Tablosu (Cortex-M):
 *     ┌─────────┬──────────────────────────────────┐
 *     │ Offset  │ İstisna / IRQ                    │
 *     ├─────────┼──────────────────────────────────┤
 *     │ 0x00    │ Başlangıç SP değeri              │
 *     │ 0x04    │ Reset handler                    │
 *     │ 0x08    │ NMI handler                      │
 *     │ 0x0C    │ HardFault handler                │
 *     │ 0x10    │ MemManage handler                │
 ...
 *
 * =============================================================================
 * [THEORY: DMA (Direct Memory Access) / TEORİ: DMA (Doğrudan Bellek Erişimi)]
 * =============================================================================
 *
 ...
 *
 * TR: DMA (Direct Memory Access), CPU müdahalesi olmadan veri aktarır:
 *
 *     CPU yükü karşılaştırması:
 *     ┌────────────────────────┬────────────┬─────────────┐
 *     │ Aktarım Yöntemi        │ CPU Kullan.│ Gecikme     │
 *     ├────────────────────────┼────────────┼─────────────┤
 *     │ Polling                │ %100       │ En düşük    │
 *     │ Kesme/bayt             │ ~%30-50    │ Orta        │
 *     │ DMA                    │ ~%0-5      │ Arka plan   │
 *     │ DMA + Çift buffer      │ ~%0        │ Sıfır kopya │
 *     └────────────────────────┴────────────┴─────────────┘
 *
 *     DMA Modları:
 *     - Normal: N bayt transfer et, sonra dur
 *     - Circular: N bayt sonra otomatik başa dön
 *     - Double-buffer: iki bellek bölgesi arasında ping-pong
 *
...
public:
    ...
    // TR: Kesmeyi beklemede olarak işaretle (donanım tetiklemesi simülasyonu)
    ...
    // TR: Bekleyen tüm kesmeleri önceliğe göre işle (düşük sayı = yüksek öncelik)
    void process_pending() {
        // EN: Find highest priority pending+enabled interrupt
        // TR: En yüksek öncelikli bekleyen+etkin kesmeyi bul
        int best = -1;
        ...
        for (int i = 0; i < MAX_IRQ; ++i) {
            ...
            if (irq.pending && irq.enabled && !irq.active && irq.priority < best_pri) {
                ...
                // TR: Preemption kontrolü: yalnızca aktif kesmeden yüksek öncelikliyse kesebilir
                if (active_irq_ >= 0 &&
                    irq.priority >= irqs_[static_cast<size_t>(active_irq_)].priority) {
                    continue;  // cannot preempt — TR: kesme yapamaz (düşük öncelik)
                }
                best = i;
        ...
        // TR: Tail-chaining: daha fazla bekleyen kesme var mı kontrol et
        //     (bağlam geri yüklemesi atlanarak hız kazanılır)
        process_pending();
    }
...
class DMAStream {
    ...
    // TR: Kesme callback'leri (yarı-transfer, tamamlanma, hata)
...
public:
    ...
    // TR: Bir DMA transfer adımını simüle et (her beat bir veri birimi taşır)
    bool transfer_beat() {
        ...
        // TR: Yarı-transfer kesmesi — çift buffer senaryolarında CPU'ya işleme başla sinyali
        ...
        // TR: Transfer tamamlandı — Normal modda durur, Circular modda başa döner
    ...
    // TR: Tüm transferi çalıştır (tamamlanana kadar döngü)
    void run_all() {
...
// TR: Çift buffer: DMA buffer A'yı doldururken CPU buffer B'yi işler,
//     sonra takas edilir — sıfır kopya (zero-copy) akış sağlar.
class DoubleBufferDMA {
    std::vector<uint8_t> buffer_a_;
    std::vector<uint8_t> buffer_b_;
    int active_buffer_ = 0;  // 0=A, 1=B — TR: aktif buffer indeksi
...
public:
    ...
    // TR: DMA'nın şu anda doldurduğu buffer'a eriş
    std::vector<uint8_t>& dma_buffer() {
        return active_buffer_ == 0 ? buffer_a_ : buffer_b_;
    }

    // EN: Get the buffer CPU can safely process
    // TR: CPU'nun güvenle işleyebileceği buffer'a eriş (DMA bu buffer'a dokunmaz)
    ...
    // TR: Buffer'ları takas et (DMA transfer-complete kesmesinde çağrılır)
    void swap() {
...
// TR: ISR'yı kısa tutup işi ana döngüye erteleme kalıbını gösterir
class ISRWorkQueue {
    struct WorkItem {
...
public:
    ...
    // TR: Ana döngüden çağrılır — ertelenmiş işleri işler (ağır hesaplama burada yapılır)
...
int main() {
    ...
    // ─── Demo 1: NVIC Priority & Execution Order ─────────────────────────
    {
        ...
        // TR: Tüm kesmeler aynı anda — NVIC önceliğe göre işler (0=en önce)
    ...
    // ─── Demo 2: Interrupt Preemption ────────────────────────────────────
    {
        ...
        // TR: Düşük öncelikli ISR, yüksek öncelikli kesmeyi tetikler (preemption gösterimi)
        nvic.configure_irq(10, "SPI1_IRQHandler", 6,
            [&nvic]{
                std::cout << "    → SPI1: processing (pri=6)...\n";
                // EN: While processing SPI, CAN interrupt arrives
                // TR: SPI işlenirken CAN kesmesi gelir → preemption
                nvic.set_pending(11);
                nvic.process_pending();
    ...
    // ─── Demo 3: DMA Normal Transfer ─────────────────────────────────────
    {
        ...
        // TR: UART verisinin gelişini simüle et (12 bayt "HELLO WORLD!")
    ...
    // ─── Demo 4: DMA Circular Mode (ADC Continuous) ──────────────────────
    {
        ...
        // TR: 3 tam ADC dönüşüm turunu simüle et (Circular mod: otomatik başa döner)
        std::vector<std::vector<uint8_t>> rounds = {
            {0x12, 0x34, 0x56, 0x78},  // round 1 — TR: 1. tur
            {0xA0, 0xB0, 0xC0, 0xD0},  // round 2 — TR: 2. tur
            {0x11, 0x22, 0x33, 0x44},  // round 3 — TR: 3. tur
        };
    ...
    // ─── Demo 5: Double-Buffer DMA ───────────────────────────────────────
    {
        ...
        for (int cycle = 0; cycle < 4; ++cycle) {
            ...
            // TR: DMA bir buffer'dan gönderirken CPU diğerini doldurur (zero-copy)
            // cpu_buffer() is safe for CPU, dma_buffer() is for DMA
            // TR: cpu_buffer() CPU için güvenli, dma_buffer() DMA kullanımında
            ...
            // TR: DMA bitti → buffer takası (TC kesmesinde gerçekleşir)
            db.swap();
        }
        std::cout << "  └────────┴─────────────────┴─────────────────┘\n";
        db.print_status();
    ...
    // ─── Demo 6: ISR Best Practices ──────────────────────────────────────
    {
        ...
        // TR: Gelen kesmeleri simüle et (UART, CAN, ADC — ISR'da minimal iş yapılır)
        ...
        // TR: Ana döngü ertelenmiş işleri işler (ağır hesaplama burada güvenle yapılır)
