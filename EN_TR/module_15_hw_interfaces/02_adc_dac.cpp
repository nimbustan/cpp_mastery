 ...
 * @brief ADC & DAC — Analog-to-Digital / Digital-to-Analog Conversion
 *
 * @details
 * =============================================================================
 * [THEORY: ADC (Analog-to-Digital Converter) / TEORİ: ADC (Analog-Dijital Dönüştürücü)]
 * =============================================================================
 *
 ...
 *
 * TR: ADC sürekli analog gerilimleri ayrık dijital değerlere çevirir.
 *     Temel parametreler: Çözünürlük (bit sayısı), Örnekleme Hızı, Referans
 *     Gerilim (Vref), Giriş Aralığı (0V-Vref tek uçlu veya ±Vref farksal).
 *
 *     ADC Türleri:
 *     ┌──────────────────┬──────────┬───────────────┬────────────────────────┐
 *     │ Mimari           │ Hız      │ Çözünürlük    │ Kullanım Alanı         │
 *     ├──────────────────┼──────────┼───────────────┼────────────────────────┤
 *     │ SAR              │ 1-5 MSPS │ 8-18 bit      │ MCU genel amaçlı       │
 *     │ Sigma-Delta (ΣΔ) │ <100 SPS │ 16-24 bit     │ Hassas ölçüm           │
 *     │ Pipeline         │ >100 MSPS│ 8-16 bit      │ Video, radar           │
 *     │ Flash            │ >1 GSPS  │ 4-8 bit       │ Osiloskop, telekom     │
 *     └──────────────────┴──────────┴───────────────┴────────────────────────┘
 *
 *     Dönüşüm Formülü: Digital = (V_in / V_ref) × (2^n - 1)
 *                       V_in = Digital × V_ref / (2^n - 1)
 *
 * =============================================================================
 * [THEORY: DAC (Digital-to-Analog Converter) / TEORİ: DAC (Dijital-Analog Dönüştürücü)]
 * =============================================================================
 *
 ...
 *
 * TR: DAC ise tersini yapar: dijital değerleri analog gerilime çevirir.
 *     V_out = Digital_Value × V_ref / (2^n - 1)
 *     Yaygın kullanım: ses çıkışı, dalga formu üretimi, kontrol döngüleri.
 *
...
// TR: Yapılandırılabilir çözünürlük ve Vref ile SAR ADC simülasyonu
...
struct ADCConfig {
    ...
    double vref = 3.3;              // Reference voltage — TR: Referans gerilim
    uint32_t sample_rate_hz = 1000000;  // 1 MSPS — TR: 1 milyon örnekleme/saniye
    bool differential = false;       // single-ended vs differential
                                     // TR: tek uçlu / farksal giriş seçimi
...
class ADCSimulator {
    ADCConfig config_;
    std::mt19937 rng_{42};  // fixed seed for reproducibility — TR: tekrarlanabilirlik için sabit tohum
    std::normal_distribution<double> noise_{0.0, 0.5};  // ±0.5 LSB noise — TR: kuantalama gürültüsü

    // EN: Simulated analog input voltages (up to 16 channels)
    // TR: Simüle edilmiş analog giriş gerilimleri (16 kanala kadar)
...
public:
    ...
    // TR: Dijital değeri tekrar gerilime çevir (ADC okumasını yorumlamak için)
    ...
    // TR: Çoklu örnekleme ortalaması (gürültü azaltma için aşırı örnekleme)
...
// TR: Yaygın otomotiv sensör transfer fonksiyonları

struct SensorCalibration {
    std::string name;
    std::string unit;
    double v_min;       // voltage at physical minimum — TR: fiziksel minimumdaki gerilim
    double v_max;       // voltage at physical maximum — TR: fiziksel maksimumdaki gerilim
    double phys_min;    // physical minimum — TR: fiziksel minimum değer
    double phys_max;    // physical maximum — TR: fiziksel maksimum değer

    // EN: Linear transfer function: V → Physical
    // TR: Doğrusal transfer fonksiyonu: gerilim → fiziksel değer
    ...
    // EN: Physical → Voltage
    // TR: Fiziksel değer → gerilim (DAC çıkışı hesaplaması için)
    ...
    // TR: Gerilimin geçerli aralıkta olup olmadığını kontrol et
    //     (açık/kısa devre tespiti — otomotivde sensör hatası teşhisi için kritik)
    bool is_valid(double voltage) const {
        constexpr double margin = 0.1;  // 100 mV margin — TR: 100 mV tolerans marjı
...
public:
    ...
    // TR: Bir periyotluk dalga formu üret, gerilim örneklerini döndür
...
// TR: Aşırı örnekleme ADC çözünürlüğünü etkin olarak artırır.
//     Ekstra bit = log2(örnekleme_oranı) / 2
//     4x = +1 bit, 16x = +2 bit, 64x = +3 bit, 256x = +4 bit
//     12-bit ADC + 256x örnekleme = etkin 16-bit çözünürlük (yavaş ama hassas)
...
int main() {
    ...
    // ─── Demo 2: Automotive Sensor Reading ──────────────────────────────
    {
        ...
        // TR: Otomotiv sensörlerini tanımla (her biri farklı gerilim-fiziksel aralığa sahip)
        ...
        // TR: Simüle edilmiş sensör gerilimlerini ayarla
    ...
    // ─── Demo 5: DAC Waveform Generation ────────────────────────────────
    {
        ...
        // TR: Sinüs dalgası üret (1 periyot, 20 örnek — DAC çıkışı simülasyonu)
        auto sine = wg.generate_sine(20, 1.5, 1.65);

        std::cout << "  Sine wave (Amp=1.5V, Offset=1.65V):\n  ";
        // EN: ASCII art waveform
        // TR: ASCII dalga formu görselleştirmesi
    ...
    // ─── Demo 7: ADC+DAC Loopback ───────────────────────────────────────
    {
        ...
        // TR: Basit oransal kontrol simülasyonu (P kontrol — hata × Kp = çıkış)
        double current_temp = 60.0;  // starting temperature
        double kp = 0.05;            // proportional gain — TR: oransal kazanç
        ...
        for (int step = 0; step < 8; ++step) {
            ...
            // TR: Sıcaklığın hedefe doğru yükseldiğini simüle et
