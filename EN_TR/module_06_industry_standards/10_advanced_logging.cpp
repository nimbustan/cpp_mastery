 ...
 * @brief Advanced Logging — İleri Seviye Günlükleme
 *
 * @details
 * =============================================================================
 * [THEORY: Production Logging Requirements / TEORİ: Üretim Günlükleme Gereksinimleri]
 * =============================================================================
 *
 ...
 *     └─────────────────────┴──────────────────────────────────────────────┘
 *
 * TR: Üretim günlükleme std::cout'un çok ötesindedir. Gereksinimler:
 *
 *     1. LOG SEVİYELERİ: Debug < Info < Warning < Error < Critical
 *     2. THREAD GÜVENLİĞİ: Birden fazla thread eşzamanlı günlükleme
 *     3. GÜNLÜK ROTASYONU: Diski doldurma — boyut/zamana göre döndür
 *     4. YAPILANDIRILMIŞ LOGLAMA: Makine-ayrıştırılabilir (JSON) + insan-okunabilir
 *     5. ASENKRON LOGLAMA: Sıcak yolları G/Ç için bloke etme
 *     6. KAYNAK KONUMU: Dosya, satır, fonksiyon — otomatik olarak
 *     7. ÇIKIŞ HEDEFLERİ: Konsol, dosya, syslog, ağ — yapılandırılabilir
 *
 *     ┌─────────────────────┬──────────────────────────────────────────────┐
 *     │ Kütüphane           │ Açıklama                                     │
 *     ├─────────────────────┼──────────────────────────────────────────────┤
 *     │ spdlog              │ Hızlı, header-only, asenkron. STANDART.      │
 *     │ glog (Google)       │ LOG(INFO), CHECK(), VLOG(). Thread-güvenli.  │
 *     │ Boost.Log           │ Tam özellikli, yapılandırılabilir. Ağır.     │
 *     │ plog                │ Header-only, taşınabilir, küçük.             │
 *     │ Custom (bu dosya)   │ Sıfır bağımlılık öğrenme implementasyonu.    │
 *     └─────────────────────┴──────────────────────────────────────────────┘
 *
 * =============================================================================
 * [THEORY: spdlog — The Industry Standard / TEORİ: spdlog — Endüstri Standardı]
 * =============================================================================
 *
 ...
 *
 * TR: spdlog hızlı referans (CMake: FetchContent veya find_package):
 *
 *     // Dönen dosya sink'i ile logger oluştur
 *     auto logger = spdlog::rotating_logger_mt(
 *         "ecu_log",                      // Logger adı
 *         "logs/ecu.log",                 // Dosya yolu
 *         1024 * 1024 * 5,                // 5 MB maks boyut
 *         3                               // 3 döndürülmüş dosya tut
 *     );
 *
 *     // Format argümanları ile logla (fmtlib-tarzı)
 *     logger->info("Sensor {} reading: {:.2f}C", sensor_id, temp);
 *     logger->warn("Temperature above threshold: {}C", temp);
 *     logger->error("Sensor {} communication failed", sensor_id);
 *
 ...
 *     spdlog::set_pattern("[%Y-%m-%d %H:%M:%S.%e] [%l] [thread %t] %v");
 *
 *     // Asenkron loglama (G/Ç için arka plan threadi)
 *     spdlog::init_thread_pool(8192, 1);  // Kuyruk boyutu, 1 işçi thread
 *     auto async_logger = spdlog::create_async<spdlog::sinks::rotating_file_sink_mt>(
 *         "async_ecu", "logs/ecu.log", 5*1024*1024, 3);
 *
...
// TR: Konsol çıktısı için ANSI renk kodları
...
struct LogEntry {
    ...
    // TR: İnsan okunabilir metin olarak biçimlendir
    ...
    // TR: JSON olarak biçimlendir (yapılandırılmış günlükleme)
...
// TR: Asenkron logger — günlükler kuyrukta tamponlanır, arka plan thread'i yazar
class AsyncLogger {
public:
...
// TR: Kaynak konumu ile kolaylık makrosu
...
// TR: Dönen dosya logger'ı — dosya maksimum boyutu aştığında döner
...
private:
    ...
    void rotate() {
        ...
        // TR: Mevcut dönmüş dosyaları kaydır: .2 → .3, .1 → .2
...
int main() {
    ...
    // ─── Demo 1: Log Levels & Formatting ────────────────────────────────
    {
        ...
        // TR: Tüm günlük seviyelerini renklerle göster
    ...
    // ─── Demo 2: Async Logger ───────────────────────────────────────────
    {
        ...
        // TR: Birden fazla thread eşzamanlı günlükleme
        ...
        // TR: İşçi thread'e işleme zamanı ver
    ...
    // ─── Demo 3: Log Rotation ───────────────────────────────────────────
    {
        ...
        auto log_path = fs::temp_directory_path() / "ecu_rotating.log";
        {
            ...
            // TR: Dönen logger: dosya başına 200 bayt, 3 dönmüş dosya tut
