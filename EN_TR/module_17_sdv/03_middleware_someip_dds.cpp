 ...
 * @brief Middleware: SOME/IP, DDS, iceoryx — Automotive Communication
 *
 * @details
 * =============================================================================
 * [THEORY: Why Middleware? / TEORİ: Neden Middleware?]
 * =============================================================================
 *
 ...
 *                                └──────────┘
 *
 * TR: Middleware, düşük seviye iletişimi (soket, paylaşımlı bellek) soyutlar
 *     ve dağıtık sistemler için servis tabanlı iletişim sağlar.
 *
 *     Middleware Olmadan          Middleware İle
 *     ┌──────────┐               ┌──────────┐
 *     │  Uygulama│               │  Uygulama│
 *     │ socket() │               │ proxy->  │
 *     │ connect()│               │  call()  │
 ...
 *                                └──────────┘
 *
 *     Ham soket kullanımı hata eğilimli ve taşınamaz, middleware soyutlar.
 *
 * =============================================================================
 * [THEORY: SOME/IP Header Format / TEORİ: SOME/IP Başlık Formatı]
 * =============================================================================
 *
 * EN: Byte: 0       4       8       12      16
 *        ┌───────────────┬───────────────┐
 ...
 *
 * TR: SOME/IP başlık formatı:
 *     - Message ID (4 bayt): Service ID (16-bit) + Method ID (16-bit)
 *     - Length (4 bayt): payload + 8 bayt
 *     - Request ID (4 bayt): Client ID + Session ID
 *     - Protocol Version, Interface Version, Message Type, Return Code
 *     Mesaj tipleri: REQUEST (istek), NOTIFICATION (bildirim),
 *     RESPONSE (yanıt), ERROR (hata).
 *
 * =============================================================================
 * [THEORY: DDS — Data Distribution Service / TEORİ: DDS — Veri Dağıtım Servisi]
 * =============================================================================
 *
 * EN:
 *     ┌─────────────────────────────────────────────┐
 ...
 *     └─────────────────────────────────────────────┘
 *
 * TR: DDS (Data Distribution Service), OMG standardı publish/subscribe
 *     middleware'idir. DataWriter konuya (topic) yazar, DataReader okur.
 *     QoS politikaları: Reliability (güvenilirlik), Durability (kalıcılık),
 *     Deadline, History, Lifespan — her topic bağımsız ayarlanabilir.
 *     ADAS ve robotik uygulamalarda yaygın (Fast-DDS, Cyclone DDS).
 *
 * =============================================================================
 * [THEORY: Middleware Comparison / TEORİ: Ara Katman Karşılaştırması]
 * =============================================================================
 *
 * EN:
 *     ┌──────────┬───────────────┬───────────────┬──────────────┬──────────┐
 ...
 *     └──────────┴───────────────┴───────────────┴──────────────┴──────────┘
 *
 * TR: Middleware karşılaştırması:
 *     ┌──────────┬───────────────┬───────────────┬───────────────┬──────────┐
 *     │ Özellik  │ SOME/IP       │ DDS           │ iceoryx       │ gRPC     │
 *     ├──────────┼───────────────┼───────────────┼───────────────┼──────────┤
 *     │ Desen    │ İstek/Yan+Olay│ Pub/Sub       │ Pub/Sub       │ İst/Yan  │
 *     │ Transfer │ TCP/UDP       │ UDP multicast │ Paylaş. bellek│ HTTP/2   │
 *     │ Keşif    │ SOME/IP-SD    │ Dahili (DDP)  │ RouDi daemon  │ El ile   │
 *     │ Gecikme  │ ~100µs        │ ~50µs         │ <1µs (0kopya) │ ~500µs   │
 *     │ Standart │ AUTOSAR       │ OMG           │ Eclipse       │ Google   │
 *     │ C++ API  │ vsomeip       │ Fast-DDS      │ iceoryx2      │ grpc++   │
 *     │ Kullanım │ Klasik oto    │ ADAS, robotik │ SoC içi IPC   │ Bulut    │
 *     └──────────┴───────────────┴───────────────┴───────────────┴──────────┘
 *
...
// TR: SOME/IP başlığı (16 byte)
struct SomeIpHeader {
    uint16_t    service_id;
    uint16_t    method_id;
    uint32_t    length;         // payload + 8 bytes — TR: yük + 8 bayt
    uint16_t    client_id;
    uint16_t    session_id;
...
// TR: SOME/IP Servis Keşfi (basitleştirilmiş)
struct ServiceEntry {
    uint16_t service_id;
...
// TR: QoS (Hizmet Kalitesi) politikaları
...
struct QoSPolicy {
    ...
    uint32_t    deadline_ms = 0;   // 0 = no deadline — TR: 0 = son tarih yok
...
// TR: DataWriter — bir topic üzerinde örnek yayınlar
class DataWriter {
    std::string                               topic_;
    QoSPolicy                                qos_;
    uint64_t                                 seq_ = 0;
...
// TR: DataReader — bir topic'e abone olur
class DataReader {
    std::string                               topic_;
    QoSPolicy                                qos_;
...
// TR: DDS Participant — writer ve reader'ları yönetir
class DomainParticipant {
    std::string name_;
    std::vector<DataWriter*>                writers_;
    std::vector<DataReader*>                readers_;
    // EN: Topic → readers mapping for routing
    // TR: Konu → okuyucu eşlemesi — yayınlanan veriyi ilgili DataReader'lara yönlendirir
...
// TR: Simüle edilmiş paylaşımlı bellek segmenti
...
public:
    ...
    // TR: Bir chunk "ödünç al" — yayıncı pointer alır, yazar, yayınlar
    struct Chunk {
        uint8_t* data;
        size_t   size;
        size_t   offset;
...
// TR: iceoryx Yayıncı (sıfır kopyalama)
class Publisher {
    std::string         topic_;
...
// TR: iceoryx Abone (sıfır kopyalama okuma)
class Subscriber {
    std::string          topic_;
    SharedMemorySegment& shm_;
...
// TR: RouDi (Yönlendirme ve Keşif daemon'u) — basitleştirilmiş
class RouDi {
    SharedMemorySegment shm_;
    ...
    void Publish(const std::string& topic, const std::vector<uint8_t>& data) {
        ...
        for (auto& sub : subscribers_) {
            if (sub.topic == topic) {
                ...
                // TR: BEST_EFFORT için %20 kayıp simülasyonu — gerçek ağda UDP paket kaybını yansıtır
...
int main() {
    ...
    std::cout << "--- Demo 1: SOME/IP — Request/Response + Notification ---\n";
    {
        ...
        // TR: GetVehicleSpeed için SOME/IP isteği oluştur (service=0x1234, method=0x0001)
        ...
        // TR: Sunucu işler ve hız=120 km/h ile yanıt verir (pozitif yanıt, MsgType=0x80)
        ...
        // TR: Olay bildirimi (hız değişti) — MsgType=NOTIFICATION ile abone olan istemcilere gönderilir
    ...
    std::cout << "\n--- Demo 3: DDS — Topic Pub/Sub with QoS ---\n";
    {
        ...
        // TR: 3 hız değeri yayınla — DataWriter aracılığıyla topic'e veri yazar
        ...
        // TR: Okuyucu durumunu kontrol et — son alınan örneği tampondan çıkar
    ...
    std::cout << "\n--- Demo 4: iceoryx — Zero-Copy Shared Memory IPC ---\n";
    {
        ...
        // TR: 1KB'lık nokta bulutu yayınlama simülasyonu — paylaşımlı belleğe kopyalanır
        struct PointCloud {
            uint32_t num_points;
            float    min_range;
            float    max_range;
        ...
        // TR: Abone okur — sıfır kopya (paylaşımlı bellek işaretçisi ile doğrudan erişim)
        size_t sz = 0;
