 ...
 * @details
 * =============================================================================
 * [THEORY: SDV — Software-Defined Vehicle / TEORİ: Yazılım Tanımlı Araç]
 * =============================================================================
 *
 ...
 *     └──────────────────────┘          └──────────────────────────┘
 *
 * TR: SDV, aracı bir yazılım platformu olarak ele alır. Özellikler
 *     donanım değişikliği olmadan yazılım ile tanımlanır ve güncellenir.
 *
 *     Geleneksel ECU Mimarisi:       SDV Mimarisi:
 *     ┌──────────────────────┐      ┌──────────────────────────┐
 *     │ ECU1 (Motor)         │      │ HPC (Merkezi Bilgisayar) │
 *     │ ECU2 (Fren/ABS)      │      │ ┌─────────────────────┐  │
 *     │ ECU3 (ADAS)          │ →→→  │ │ ADAS│Kokpit│Gövde   │  │
 *     │ ECU4 (Gövde/BCM)     │      │ └─────────────────────┘  │
 *     │ ECU5 (Bilgi-Eğlence) │      │ + Bölge Kontrolörleri    │
 *     │ ... (70-150 ECU)     │      │   ZC1  ZC2  ZC3  ZC4     │
 *     └──────────────────────┘      └──────────────────────────┘
 *
 * =============================================================================
 * [THEORY: Domain → Zonal Architecture Transition / TEORİ: Alan → Bölgesel Mimari Geçişi]
 * =============================================================================
 *
 ...
 *
 * TR: Domain (alan) tabanlı mimari: her fonksiyon grubunun ayrı kontrolörü
 *     vardır (Powertrain, Şasi, Gövde, ADAS) — CAN/LIN ile bağlanır.
 *     Zonal mimari: araç fiziksel bölgelere ayrılır (ön-sol, arka-sağ vb.),
 *     her bölgede bir Zone Controller bulunur. Tüm mantık merkezi HPC'de
 *     çalışır, ZC'ler sensör/aktüatör I/O birimi olarak görev yapar.
 *     Avantajlar: kablo ağırlığı ↓, yazılım güncellenebilirliği ↑, maliyet ↓.
 *
 * =============================================================================
 * [THEORY: SOA — Service-Oriented Architecture in Automotive / TEORİ: SOA — Otomotivde Servis Odaklı Mimari]
 * =============================================================================
 *
 * EN:
 *     ┌───────────────────────────────────────────────────────┐
 ...
 *     └────────────┘   └───────────┘  └────────────┘
 *
 * TR: SOA (Servis Yönelimli Mimari), araç içi yazılımı bağımsız servislere
 *     böler. Servisler SOME/IP, DDS veya iceoryx middleware üzerinden
 *     birbirini keşfeder ve iletişim kurar. Tüketiciler (Dashboard, ADAS)
 *     servis sağlayıcılardan (VehicleSpeed, Radar) veri alır.
 *     Avantaj: gevşek bağlama, bağımsız güncelleme, yeniden kullanılabilirlik.
 *
 * =============================================================================
 * [THEORY: Vehicle Signal Specification (VSS) / TEORİ: Araç Sinyal Spesifikasyonu (VSS)]
 * =============================================================================
 *
 ...
 *
 * TR: COVESA VSS, araç sinyallerini standart ağaç yapısında tanımlar.
 *     Örnekler: Vehicle.Speed (km/h), Vehicle.Chassis.SteeringWheel.Angle,
 *     Vehicle.Powertrain.FuelSystem.Level, Vehicle.ADAS.AEB.IsEnabled.
 *     Tüm OEM'ler ortak sinyal isimlendirmesi kullanarak birlikte çalışabilirlik sağlar.
 *
 * =============================================================================
 * [THEORY: C++ Role in SDV / TEORİ: SDV'de C++'ın Rolü]
 * =============================================================================
 *
 * EN:
 *     ┌─────────────────────────────────────────────────────────┐
 ...
 *     │ Katman              │ Dil         │ Neden                   │
 *     ├─────────────────────┼─────────────┼─────────────────────────┤
 *     │ ML Çıkarım          │ C++ / CUDA  │ Gerçek zamanlı gecikmesi│
 *     │ Sensör Füzyonu      │ C++         │ Deterministik zamanlama │
 *     │ Middleware          │ C++         │ Performans + SOA        │
 *     │ AUTOSAR Adaptive    │ C++14/17    │ ara::* API'leri         │
 *     │ Gerçek zamanlı serv.│ C++         │ RTOS entegrasyonu       │
 *     │ HMI / Qt            │ C++ / QML   │ GPU render              │
 *     │ Sürücü/HAL          │ C           │ Çekirdek gereksinimi    │
 *     └─────────────────────┴─────────────┴─────────────────────────┘
 *
...
// TR: Geleneksel ve SDV mimarisindeki ECU türleri
...
struct ECUNode {
    ...
    uint32_t            compute_gflops;  // processing power — TR: işlem gücü (GFLOPS)
};

// EN: Demonstrate domain→zonal consolidation
// TR: Domain→zonal konsolidasyonunu göster
...
// TR: VSS veri türleri — COVESA VSS standardını yansıtır
using SignalValue = std::variant<bool, int32_t, uint32_t, float, std::string>;

struct VSSNode {
    std::string  path;         // e.g. "Vehicle.Speed" — TR: örn. "Vehicle.Speed" sinyal yolu
...
// TR: Servis arayüzü — tüm servisler bunu uygular
class IService {
public:
...
// TR: SOA Servis Kaydı — merkezi servis dizini
...
public:
    ...
    // TR: Hız değişikliğini simüle et ve event yayınla
...
// TR: Gösterge Paneli Servisi (tüketici / gözlemci)
...
// TR: Veri Kaydedici Servisi (gözlemci)
...
int main() {
    ...
    std::cout << "\n--- Demo 2: Vehicle Signal Specification (VSS) Tree ---\n";
    {
        ...
        // EN: Update signals
        // TR: Sinyalleri güncelle — VSS ağacındaki değerleri variant ile ayarla
    ...
    std::cout << "\n--- Demo 3: SOA — VehicleSpeed + Dashboard + Logger ---\n";
    {
        ...
        // TR: Hız olaylarına abone ol — Observer deseni ile hız değiştiğinde bildirim alınır
        ...
        // TR: Kayıt defteri üzerinden servisleri sorgula — istek/yanıt şeklinde metot çağrısı
