 ...
 * @brief CAN FD & CAN XL — Modern CAN Protocols — Modern CAN Protokolleri
 *
 * @details
 * =============================================================================
 * [THEORY: CAN FD — Flexible Data-rate / TEORİ: CAN FD — Esnek Veri Hızı]
 * =============================================================================
 *
 ...
 *
 * TR: CAN FD, klasik CAN'ı iki önemli iyileştirme ile genişletir:
 *     1) Daha yüksek veri hızları: arbitrasyon 500 kbps, veri fazı 8 Mbps'ye kadar
 *     2) Daha büyük veri yükü: 64 byte'a kadar (Klasik CAN'da 8 byte)
 *
 *     ┌─────────────────────────┬──────────────┬──────────────┬────────────────┐
 *     │ Özellik                 │ Klasik CAN   │ CAN FD       │ CAN XL         │
 *     ├─────────────────────────┼──────────────┼──────────────┼────────────────┤
 *     │ Maks Veri (byte)        │ 8            │ 64           │ 2048           │
 *     │ Maks Bit Hızı (veri)    │ 1 Mbps       │ 8 Mbps       │ 20 Mbps        │
 *     │ Arbitrasyon             │ ID tabanlı   │ ID tabanlı   │ ID + öncelik   │
 *     │ CRC Alanı               │ CRC-15       │ CRC-17/CRC-21│ CRC-32         │
 *     │ Geriye Uyumluluk        │ —            │ Klasik CAN   │ CAN FD+Klasik  │
 *     │ ISO Standardı           │ 11898-1:2003 │ 11898-1:2015 │ CiA 610-1      │
 *     │ Tipik Kullanım          │ Gövde, Motor │ ADAS, GW     │ Eth köprüsü    │
 *     └─────────────────────────┴──────────────┴──────────────┴────────────────┘
 *
 *     CAN FD Çerçeve formatı (anahtar alanlar):
 *     ┌─────┬──────┬─────┬─────┬─────┬──────────┬──────────────┬─────┬─────┐
 *     │ SOF │ Arb  │ IDE │ FDF │ BRS │ ESI      │ DLC → Veri   │ CRC │ EOF │
 *     │ 1b  │ 11/29│ 1b  │ 1b  │ 1b  │ 1b       │ 0-64 byte    │17/21│ 7b  │
 *     └─────┴──────┴─────┴─────┴─────┴──────────┴──────────────┴─────┴─────┘
 *     FDF = FD Format (1=FD, 0=Klasik) — FD çerçevesi olduğunu belirtir
 *     BRS = Bit Rate Switch (1=veri fazında yüksek hız) — hız geçişi kontrolü
 *     ESI = Error State Indicator (0=aktif, 1=pasif) — hata durumu bildirimi
 *
 * =============================================================================
 * [THEORY: CAN XL — Extended Length / TEORİ: CAN XL — Genişletilmiş Uzunluk]
 * =============================================================================
 *
 ...
 *
 * TR: CAN XL, CAN ve Ethernet dünyalarını birleştirir:
 *     - 2048 byte'a kadar veri yükü (CAN FD'nin 32 katı)
 *     - 20 Mbps'ye kadar veri hızı
 *     - SDU Type alanı ile Ethernet çerçeve tünellemesi (0x05 = Ethernet)
 *     - Öncelik tabanlı arbitrasyon (0 = en yüksek öncelik)
 *     - Zonal mimari omurgası (domain → zone geçişi) için tasarlanmıştır
 *
 * =============================================================================
 * [THEORY: DBC File Format — CAN Database / TEORİ: CAN Veritabanı]
 * =============================================================================
 *
 ...
 *
 * TR: DBC dosyaları CAN mesajları içindeki sinyal düzenini tanımlar.
 *     Araçlar: Vector CANdb++, PEAK Symbol Editor, Kayak, cantools (Python).
 *
 *     Örnek DBC girişi:
 *       BO_ 0x123 EngineData: 8 Engine_ECU
 *         SG_ RPM : 0|16@1+ (0.25,0) [0|16383.75] "rpm" Gateway
 *         SG_ Temp : 16|8@1+ (1,-40) [-40|215] "degC" Gateway
 *
 *     Format: SG_ <ad> : <başlat_bit>|<uzunluk>@<byte_sırası><işaret>
 *             (<faktör>,<offset>) [<min>|<maks>] "<birim>" <alıcılar>
 *     byte_sırası: 1=Intel (little-endian), 0=Motorola (big-endian)
 *     Fiziksel = ham_değer × faktör + offset
 *
 * =============================================================================
 * [THEORY: SocketCAN — Linux CAN Interface / TEORİ: SocketCAN — Linux CAN Arayüzü]
 * =============================================================================
 *
 ...
 *
 * TR: SocketCAN, Linux çekirdeğinin CAN alt sistemidir. Sağladıkları:
 *     - CAN için standart soket API'si (PF_CAN adres ailesi)
 *     - Sanal CAN arayüzü (vcan) — donanımsız test için
 *     - CAN filtreleri (mask + id) — seçici alım: (alınan_id & mask) == (filtre_id & mask)
 *     - Klasik CAN, CAN FD ve ham çerçeve desteği
 *
 *     Kurulum: sudo modprobe vcan
 *              sudo ip link add dev vcan0 type vcan
 *              sudo ip link set up vcan0
 *
 * @note Compile: g++ -std=c++20 -Wall -Wextra -Wpedantic -Wshadow -Wconversion 01_can_fd_and_xl.cpp -o 01_can_fd_and_xl
 *       SocketCAN only works on Linux (linux/can.h). Simulation works everywhere.
 *
 * @date 2026
...
// TR: CAN FD için DLC → gerçek byte sayısı dönüşümü
...
// TR: Klasik CAN frame (maks 8 byte)
struct ClassicCANFrame {
    uint32_t id       = 0;     // 11-bit (standart) veya 29-bit (genişletilmiş) arbitrasyon ID'si
    bool     extended = false; // IDE: 0=standart (11-bit), 1=genişletilmiş (29-bit)
    bool     rtr      = false; // Remote Transmission Request — veri yerine uzak istek
    uint8_t  dlc      = 0;    // Data Length Code (0-8) — veri alanı byte sayısı
...
// TR: CAN FD frame (64 byte'a kadar, çift bit hızı)
struct CANFDFrame {
    uint32_t id       = 0;
    bool     extended = false;
    bool     brs      = false; // Bit Rate Switch: veri fazında yüksek hız (arbitrasyon 500k, veri 2-8M)
    bool     esi      = false; // Error State Indicator: 0=aktif (sağlıklı), 1=pasif (hata sayıcı yüksek)
    uint8_t  dlc      = 0;    // DLC 0-15 (dlc_to_len ile 0-64 byte'a eşlenir)
...
// TR: CAN XL frame (2048 byte'a kadar, öncelik tabanlı)
struct CANXLFrame {
    uint8_t  priority    = 0;    // 0 = en yüksek öncelik (CAN ID yerine ayrı alan)
    uint32_t acceptance_field = 0; // Kabul filtresi alanı (CAN ID'nin yerini alır)
    uint8_t  sdu_type    = 0;    // Service Data Unit türü (0x05 = tünellenmiş Ethernet)
    uint16_t data_length = 0;    // Gerçek veri uzunluğu (1-2048 byte)
...
//                        = 0x302899
// TR: CAN FD, ≤16 byte için CRC-17, >16 byte için CRC-21 kullanır.
...
uint32_t crc17_canfd(const uint8_t* data_bytes, size_t len) {
    ...
    // TR: Eğitim amaçlı basitleştirilmiş bit-bit CRC-17 hesaplama
    uint32_t crc = 0;
...
// TR: CAN FD veri uzunluğuna göre otomatik CRC seçimi
...
// TR: CAN bit bazlı arbitrasyon kullanır — en düşük ID kazanır (dominant='0' baskındır).
...
// TR: Aynı anda yarışan N ECU için bit bazlı arbitrasyon simülasyonu
ArbitrationEntry simulate_arbitration(std::vector<ArbitrationEntry>& entries) {
    // EN: Sort by ID (lowest wins — dominant)
    // TR: ID'ye göre sırala (en düşük kazanır — dominant '0' baskındır)
...
// TR: DBC "sinyal" tanımı, CAN ham baytlarından fiziksel değerin nasıl çıkarılacağını belirtir.

struct DBCSignal {
    std::string name;
    uint8_t  start_bit   = 0;     // çerçevedeki bit konumu (LSB pozisyonu)
    uint8_t  bit_length  = 0;     // bit sayısı (sinyal genişliği)
    ...
    // TR: CAN verisinden ham değeri çıkar (Intel / little-endian byte sırası)
    uint64_t extract_raw(const uint8_t* frame_data, uint8_t /*frame_len*/) const {
        ...
        // TR: Basitlik için Intel (little-endian) byte sırası çıkarımı
    ...
    // TR: Ham değeri fiziksel değere dönüştür
    double to_physical(uint64_t raw) const {
        if (is_signed) {
            // EN: Sign-extend if MSB is set
            // TR: MSB set ise işaret uzatması yap (negatif değerler için)
    ...
    // TR: Tam çıkarım hattı (ham → fiziksel tek adımda)
...
// TR: CAN denetleyicileri CPU yükünü azaltmak için donanım filtreleri kullanır.
//     Filtre geçiş koşulu: (alınan_id & mask) == (filtre_id & mask)

struct CANFilter {
    uint32_t filter_id;
    uint32_t mask;  // 1 = eşleşmeli, 0 = önemsiz (don't care)
...
// TR: CAN düğümleri TEC (Gönderim Hata Sayacı) ve REC (Alım Hata Sayacı) tutar.
//     Durum makinesi: Error Active → Error Passive → Bus-Off
//     - Error Active: TEC < 128 VE REC < 128 (normal çalışma)
//     - Error Passive: TEC ≥ 128 VEYA REC ≥ 128 (kısıtlı katılım)
//     - Bus-Off: TEC ≥ 256 (düğüm bağlantıyı keser, kurtarma gerekir)

enum class CANErrorState {
    ErrorActive,   // Normal — aktif hata çerçeveleri gönderebilir
    ErrorPassive,  // Kısıtlı — sadece pasif hata çerçeveleri
    BusOff         // Bağlantı kesildi — bus-off kurtarma gerekli
};
...
class CANErrorManager {
    uint16_t tec_ = 0;  // Transmit Error Counter (Gönderim Hata Sayacı)
    uint16_t rec_ = 0;  // Receive Error Counter (Alım Hata Sayacı)
...
public:
    ...
    // TR: Gönderim hatasında çağrılır (ISO 11898: her hatada TEC += 8)
    void transmit_error() {
    ...
    // TR: Bus-off kurtarma: 128 kez 11 ardışık recessive bit algılanmalı
...
// TR: 4 ECU'lu basit bir araç CAN ağı simülasyonu:
//     Motor ECU → RPM, soğutma sıcaklığı, gaz kelebeği pozisyonu yayınlar
//     ABS ECU → tekerlek hızları yayınlar
//     HVAC ECU → kabin sıcaklığı, fan hızı yayınlar
//     Gateway ECU → tüm mesajları alır ve yönlendirir

struct ECU {
    std::string name;
    std::vector<CANFDFrame> tx_queue;   // gönderilecek mesajlar
    std::vector<CANFDFrame> rx_buffer;  // alınan mesajlar
    std::vector<CANFilter> filters;     // kabul filtreleri (donanım filtresi simülasyonu)
    ...
    bool accept(uint32_t msg_id) const {
        if (filters.empty()) return true;  // filtre yok = hepsini kabul et (gateway modu)
...
// TR: CAN bus — ECU'ları bağla ve iletişimi yönet
class CANBus {
...
public:
    ...
    // TR: Kuyruklanmış tüm frameleri ilet (arbitrasyon: en düşük ID önce)
    void transmit_cycle() {
        // EN: Collect all pending frames with source info
        // TR: Bekleyen tüm frameleri kaynak bilgisiyle topla
        struct PendingFrame {
            CANFDFrame frame;
            ECU* source;
        };
        ...
        // TR: Arbitrasyon — ID'ye göre sırala (en düşük kazanır — dominant baskınlığı)
        ...
        // TR: Her frame'i gönderen hariç tüm düğümlere ilet (broadcast bus)
...
// TR: SocketCAN kod referansı (sadece Linux'ta linux/can.h ile derlenir)
...
int main() {
    ...
    // ─── Demo 1: Frame Structures ───────────────────────────────────────
    {
        ...
        // TR: Klasik CAN — Motor devir mesajı (8 byte sınırı)
        ...
        // TR: CAN FD — ADAS sensör füzyon verisi (>8 byte gerektirir)
        CANFDFrame fd_frame;
        fd_frame.id = 0x200;
        ...
        // TR: CAN XL — Tünellenmiş Ethernet frame (SDU_ETHERNET=0x05)
        CANXLFrame xl_frame;
        xl_frame.priority = 1;
    ...
    // ─── Demo 2: CAN FD CRC Calculation ─────────────────────────────────
    {
        ...
        // TR: Kısa mesaj (≤8 byte) → CRC-17 kullanılır
        ...
        // TR: Uzun mesaj (>16 byte) → CRC-21 kullanılır
        CANFDFrame long_msg;
        ...
        // TR: Veri değiştiğinde CRC'nin de değiştiğini doğrula (bütünlük kontrolü)
    ...
    // ─── Demo 4: DBC Signal Extraction ──────────────────────────────────
    {
        ...
        // TR: DBC sinyal tanımlarıyla mesajı oluştur
        ...
        // TR: RPM sinyali: başlat_bit=0, uzunluk=16, Intel byte sırası, işaretsiz
        //     physical = raw * 0.25 + 0
        DBCSignal rpm_sig;
        rpm_sig.name = "RPM";
        rpm_sig.start_bit = 0;
        ...
        // TR: Sıcaklık sinyali: başlat_bit=16, uzunluk=8, offset=-40
        //     physical = raw * 1 + (-40)
        DBCSignal temp_sig;
        ...
        // TR: Motor yükü: başlat_bit=24, uzunluk=8
        DBCSignal load_sig;
        ...
        // TR: Simüle CAN verisi: RPM=3000→ham=12000, Sıcak=90°C→ham=130, Yük=75%→ham=191
    ...
    // ─── Demo 5: CAN Filter ────────────────────────────────────────────
    {
        ...
        // TR: Filtre: sadece 0x0C0-0x0CF kabul et (motor mesajları)
    ...
    // ─── Demo 6: Error State Machine ────────────────────────────────────
    {
        ...
        // TR: Tekrarlanan gönderim hatalarını simüle et (her hata TEC += 8)
        ...
        // TR: Bus-off kurtarma girişimi (128 recessive bit dizisi gerekli)
    ...
    // ─── Demo 7: 4-ECU Car Network Simulation ──────────────────────────
    {
        ...
        // TR: ABS sadece motor ve tekerlek hızı mesajlarıyla ilgilenir
        ...
        // TR: HVAC sadece kabin sıcaklık talebiyle ilgilenir (tam eşleşme)
        ...
        // TR: Motor ECU devir (3000 rpm) ve soğutma sıvısı sıcaklığı (90°C) iletir
        {
            CANFDFrame rpm_msg;
            rpm_msg.id = 0x0C0;
            rpm_msg.dlc = 8;
        ...
        // TR: ABS ECU tekerlek hızlarını iletir (4 tekerlek × 2 byte)
        {
        ...
        // TR: HVAC kabin sıcaklığı ve fan hızı iletir
        {
        ...
        // TR: Bir bus döngüsü çalıştır (arbitrasyon + iletim + filtreleme)
        bus.transmit_cycle();
        ...
        // TR: Gateway'in tüm mesajları aldığını doğrula (filtresiz = hepsini kabul)
        ...
        // TR: ABS filtresi doğrulama — motor (0x0C0) var, HVAC (0x300) yok olmalı
        ...
        // TR: HVAC filtresi doğrulama — motor veya tekerlek hızı olmaması gerek
