 ...
 * @brief Design Patterns: Structural — Adapter, Decorator, Facade, Composite, Proxy — Yapısal
 * Kalıplar
 *
 * @details
 * =============================================================================
 * [THEORY: Structural Patterns Overview / TEORİ: Yapısal Kalıplar]
 * =============================================================================
 ...
 * components together.
 *
 * TR: Yapısal kalıplar, sınıfların ve nesnelerin daha büyük yapılar oluşturmak için NASIL
 * BİRLEŞTİRİLDİĞİ ile ilgilenir. Karmaşık ilişkileri basitleştirir, uyumsuz arayüzleri uyarlar
 * ve mevcut kodu değiştirmeden yeni davranış ekler. Bileşenleri birbirine yapıştıran "mimari
 * bağlayıcılar" olarak düşünün.
 *
 * =============================================================================
 * [THEORY: Adapter — Make Incompatible Interfaces Work Together / TEORİ: Adapter — Uyumsuz
 * Arayüzleri Birlikte Çalıştır]
 * =============================================================================
 ...
 *
 * TR: Adapter kalıbı, uyumsuz bir arayüze sahip mevcut sınıfı, istemcinin beklediği arayüze
 * uyacak şekilde sarar. AB fişlerini ABD prizlerine dönüştüren güç adaptörü gibi. Uyarlanan
 * sınıf HİÇBİR değişiklik gerektirmez — adaptör çeviriyi halleder.
 *
 * =============================================================================
 * [THEORY: Decorator — Add Behavior Dynamically / TEORİ: Decorator — Davranışı Dinamik Olarak
 * Ekle]
 * =============================================================================
 ...
 *
 * TR: Decorator kalıbı, orijinal sınıfı değiştirmeden yeni davranış EKLEMEK için bir nesneyi
 * sarar. Dekoratörler sarılan nesneyle aynı arayüzü paylaşır, böylece üst üste yığılabilir
 * (zincirlenir). Örnek: bir veri akışına şifreleme, sıkıştırma ve loglama ekleme — her dekoratör
 * öncekini sarar.
 *
 * =============================================================================
 * [THEORY: Facade — Simplified Interface to Complex System / TEORİ: Facade — Karmaşık Sisteme
 ...
 *
 * TR: Facade kalıbı, birçok sınıftan oluşan karmaşık bir alt sisteme BASİTLEŞTİRİLMİŞ bir arayüz
 * sağlar. İstemciler 10 farklı nesneyi yönetmek yerine, perde arkasında her şeyi orkestra eden
 * TEK bir facade ile konuşurlar. Oyun motorlarında, medya çerçevelerinde ve kütüphane
 * sarmalayıcılarda yaygındır.
 *
 * =============================================================================
 * [THEORY: Composite — Treat Individual and Group Objects Uniformly / TEORİ: Composite — Tekil
 * ve Grup Nesneleri Aynı Şekilde Ele Al]
 * =============================================================================
 ...
 * a leaf or a branch.
 *
 * TR: Composite kalıbı, nesneleri ağaç yapılarına birleştirmenize ve tekil nesnelerle
 * kompozisyonları aynı arayüz aracılığıyla AYNI ŞEKİLDE ele almanıza olanak tanır. Bir `Dosya`
 * ve bir `Klasör` (dosya ve alt klasörler içeren) her ikisi de `getSize()` uygular. İstemci
 * yaprak mı dal mı olduğunu umursamaz.
 *
 * =============================================================================
 * [CPPREF DEPTH: Multiple Inheritance for Mixin-Style Decorator Chains / CPPREF DERİNLİK:
 * Mixin-Tarzı Dekoratör Zincirleri için Çoklu Kalıtım]
 * =============================================================================
 ...
 *
 * TR: C++ çoklu kalıtımı destekler, bu da davranışın birden fazla küçük sınıftan kalıtımla
 * birleştirildiği mixin-tarzı dekoratörleri mümkün kılar. Her mixin TEK bir yetenek ekler. Sanal
 * kalıtım, mixin'ler ortak bir temeli paylaştığında elmas problemini çözer. Pratikte, daha iyi
 * esneklik için kalıtım-tabanlı yerine bileşim-tabanlı dekoratörleri (yapıcı ile sarma) tercih
 * edin. cppreference.com/w/cpp/language/derived_class
 *
 * =============================================================================
 * [CPPREF DEPTH: std::shared_ptr for Shared Composite Ownership / CPPREF DERİNLİK: Paylaşımlı
 * Composite Sahipliği için std::shared_ptr]
 * =============================================================================
 ...
 *
 * TR: Composite kalıp uygulamalarında, çocuk düğümler birden fazla ebeveyn arasında
 * paylaşılabilir (örn. sembolik bağlantılar, DAG yapıları). Birden fazla sahibin nesneyi canlı
 * tuttuğu paylaşımlı sahiplik için `std::shared_ptr` kullanın. Dairesel referansları kırmak için
 * `std::weak_ptr` kullanın. Katı ağaç hiyerarşileri (tek ebeveyn) için sıfır maliyetli
 * `std::unique_ptr` tercih edilir. cppreference.com/w/cpp/memory/shared_ptr
 *
...
// TR: Değiştiremeyeceğimiz eski kütüphane (üçüncü taraf).
...
// TR: Sistemimizin beklediği arayüz.
class IDataParser {
public:
...
// TR: Adaptör, eski sınıfı bizim arayüzümüze uyacak şekilde sarar.
...
// TR: Temel dekoratör — bir IMessage sarar.
...
// TR: Karmaşık alt sistem sınıfları (Gösterge paneli alt sistemleri).
...
// TR: Facade — Tüm gösterge panelini başlatmak için TEK metod.
...
int main() {
  ...
  // TR: Yığın: Düz -> Sıkıştırılmış -> Şifrelenmiş -> Loglanmış
