 ...
 * @brief Embedded & Automotive Design Patterns — Gömülü ve Otomotiv Tasarım Kalıpları
 *
 * @details
 * =============================================================================
 * [THEORY: GoF vs Embedded Patterns / TEORİ: GoF ve Gömülü Kalıplar Farkı]
 * =============================================================================
 *
 ...
 *
 * TR: Klasik GoF tasarım kalıpları masaüstü/sunucu ortamını varsayar: heap tahsisi ucuzdur,
 *     sanal dağıtım maliyeti ihmal edilebilir ve bellek boldur. Gömülü ve otomotiv
 *     sistemlerde bu varsayımlar KIRILIR:
 *
 *     ┌────────────────────────┬────────────────────────┬────────────────────────┐
 *     │ Varsayım               │ Masaüstü/Sunucu        │ Gömülü/Otomotiv        │
 *     ├────────────────────────┼────────────────────────┼────────────────────────┤
 *     │ Heap tahsisi           │ Hızlı, bol             │ Yasak veya maliyetli   │
 *     │ Sanal dağıtım          │ ~ns maliyet, sorunsuz  │ Öngörülebilirlik önemli│
 *     │ Bellek                 │ GB mevcut              │ KB-MB (MCU: 32-512KB)  │
 *     │ İstisnalar             │ Yaygın hata yönetimi   │ Genellikle devre dışı  │
 *     │ RTTI (typeid/dyn_cast) │ Mevcut                 │ Genellikle devre dışı  │
 *     │ Dinamik konteyner      │ vector, map serbestçe  │ Sadece statik tamponlar│
 *     │ Thread modeli          │ std::thread, pthreads  │ RTOS görevleri, ISR'ler│
 *     └────────────────────────┴────────────────────────┴────────────────────────┘
 *
 *     Bu dosya, kaynak kısıtlı, gerçek zamanlı ve güvenlik-kritik ortamlara
 *     (AUTOSAR, RTOS, bare-metal) özel tasarlanmış kalıpları kapsar.
 *
 * =============================================================================
 * [THEORY: Memory Pool Pattern / TEORİ: Bellek Havuzu Kalıbı]
 * =============================================================================
 *
 ...
 *
 * TR: Güvenlik-kritik gömülü sistemlerde (ASIL-B ve üzeri), dinamik bellek tahsisi
 *     (malloc/new) başlatma sonrası ya YASAKTIR ya da ciddi şekilde kısıtlıdır.
 *     Nedenler:
 *     1. Parçalanma: tekrarlanan alloc/free belleği parçalar → sonunda OOM
 *     2. Belirsiz zamanlama: malloc'un en kötü durum süresi sınırsızdır
 *     3. Sanal bellek yok: MCU'larda MMU yok, OOM = hard fault
 *     4. MISRA C++ Kuralı 21.3: "init aşamasından sonra dinamik bellek kullanılmamalı"
 *
 *     Bellek Havuzu, başlangıçta sabit bir blok tahsis eder ve nesneleri oradan sunar:
 *     ┌─────────────────────────────────────────────────────────┐
 *     │ Pool<Sensör, 16>                                        │
 *     │ ┌────┬────┬────┬────┬────┬────┬────┬────┬─── ···        │
 *     │ │ S0 │ S1 │BOŞ │ S3 │BOŞ │BOŞ │ S6 │BOŞ │               │
 *     │ └────┴────┴────┴────┴────┴────┴────┴────┴─── ···        │
 *     │ Serbest liste: [2] → [4] → [5] → [7] → ···              │
 *     └─────────────────────────────────────────────────────────┘
 *     Placement new ile önceden tahsis edilmiş depolamada nesne oluşturur.
 *
 * =============================================================================
 * [THEORY: Active Object Pattern / TEORİ: Aktif Nesne Kalıbı]
 * =============================================================================
 *
 ...
 *
 * TR: Aktif Nesne kalıbı, METOT ÇAĞRISINI METOT YÜRÜTMESINDEN ayırır.
 *     Bir metodu doğrudan çağırmak yerine (çağıranı bloklar), istek bir mesaj
 *     kuyruğuna yerleştirilir. Özel bir thread istekleri sırayla işler.
 *
 *     ┌──────────┐    enqueue()    ┌─────────────────┐   dequeue()   ┌──────────┐
 *     │ Çağıran  │ ──────────────→ │  Mesaj Kuyruğu  │ ────────────→ │  Worker  │
 *     │ (İstemci)│                 │ (thread-güvenli)│               │  Thread  │
 *     └──────────┘                 └─────────────────┘               └──────────┘
 *
 *     Gömülü/otomotivde neden kullanılır?
 *     • AUTOSAR Runnable'lar esasen Aktif Nesnelerdir (OS zamanlayıcısı tarafından tetiklenir)
 *     • ISR → Task iletişimi: ISR kuyruğa ekler, task güvenle kuyruğundan alır
 *     • Belirleyici sıralama: mesajlar FIFO işlenir
 *     • Thread güvenliği: yalnızca worker thread paylaşılan duruma erişir
 *
 * =============================================================================
 * [THEORY: Embedded State Machine / TEORİ: Gömülü Durum Makinesi]
 * =============================================================================
 *
 ...
 *
 * TR: Durum makineleri gömülü sistemlerde her yerdedir. Her ECU mod kontrolcüsü,
 *     iletişim protokol yığını ve çevre birimi sürücüsü temelde bir FSM'dir.
 *
 *     İki ana uygulama yaklaşımı:
 *     ┌──────────────────────┬────────────────────────────────────────────────┐
 *     │ Yaklaşım             │ Özellikler                                     │
 *     ├──────────────────────┼────────────────────────────────────────────────┤
 *     │ Switch-Case FSM      │ Basit, satır içi, hızlı. Ölçeklemesi zor.      │
 *     │ Tablo Güdümlü FSM    │ Veri güdümlü, genişletilebilir, MISRA uyumlu.  │
 *     │                      │ Durum × Olay → {Aksiyon, SonrakiDurum}         │
 *     └──────────────────────┴────────────────────────────────────────────────┘
 *
 *     Tablo güdümlü FSM'ler otomotivde tercih edilir (AUTOSAR BSW dahili olarak kullanır):
 *     - Geçiş tablosu const veridir → FLASH'ta saklanır, RAM'da değil
 *     - Durum/olay eklemek dağıtım mantığını değiştirmez
 *     - UML durum diyagramlarından otomatik oluşturulması kolaydır
 *     - MISRA C++ 2023 uyumlu (karmaşık kontrol akışı yok)
 *
 * =============================================================================
 * [THEORY: Double Buffer & Hardware Proxy / TEORİ: Çift Tampon ve Donanım Vekili]
 * =============================================================================
 *
 ...
 *
 * TR: Çift Tampon: Bir taraf veri yazarken diğer taraf eşzamanlı okuyorsa kullanılır.
 *     İki tampon rol değiştirir — okuyucu A tamponu kullanırken, yazıcı B tamponunu doldurur.
 *     Takas'ta roller tersine döner. ISR arka tampona yazar; ana döngü ön tamponu okur.
 *     Takas atomik ise (tek pointer/indeks takası) kilit gerekmez.
 *
 *     Donanım Vekili: Ham register erişimini OOP arayüzüyle sarar.
 *     Şu yerine: *(volatile uint32_t*)0x40020000 |= (1 << 5);
 *     Şunu yazarsın: gpio.set_pin(5);
 *     CRTP + volatile ile MMIO register'ları üzerinde sıfır maliyetli soyutlama sağlar.
 *
 ...
 *
 * TR: Statik diziden tahsis yapan sabit boyutlu nesne havuzu. Heap yok, parçalanma yok.
 *     O(1) tahsis/serbest bırakma için serbest liste (indekslerle tek bağlı) kullanır.
 *     placement new nesneleri yerinde oluşturur; açık destructor çağrısı yıkar.
 *
 *     MISRA uyumlu: init sonrası dinamik tahsis yok, belirleyici zamanlama.
 *     RTOS görevleri için uygun, spinlock ile sarılırsa ISR güvenli.
 */
...
class MemoryPool {
    ...
    // TR: Her yuva bir union: ya T nesnesi ya da serbest liste indeksi tutar.
    //     Boşken 'next_free' sonraki uygun yuvayı gösterir.
    //     Doluyken 'storage' inşa edilmiş T'yi tutar.
    struct Slot {
    ...
    std::size_t free_head_;     // EN: Head of the free list / TR: Serbest listenin başı
    std::size_t active_count_;  // EN: Number of allocated objects / TR: Tahsis edilmiş nesne sayısı

public:
    MemoryPool() : free_head_(0), active_count_(0) {
        // EN: Initialize free list: slot[0] → slot[1] → ··· → slot[N-1] → SENTINEL
        // TR: Serbest listeyi başlat: slot[0] → slot[1] → ··· → slot[N-1] → NÖBETÇİ
        for (std::size_t i = 0; i < N; ++i) {
            slots_[i].next_free = i + 1;  // EN: Last element points to N (sentinel) / TR: Son eleman N'yi gösterir (nöbetçi)
    ...
    ~MemoryPool() {
        ...
        // TR: Hâlâ yaşayan tüm nesneleri yık — kaynak sızıntısını önle
        for (std::size_t i = 0; i < N; ++i) {
    ...
    // TR: Kopyalanamaz, taşınamaz (ham depolamaya sahip)
    MemoryPool(const MemoryPool&) = delete;
    MemoryPool& operator=(const MemoryPool&) = delete;

    /// EN: Construct a T in the pool. Returns pointer or nullptr if full.
    /// TR: Havuzda bir T oluştur. Doluysa nullptr döndürür.
    ...
    void deallocate(T* ptr) {
        ...
        if (!found || !slots_[idx].occupied) return;  // EN: Not from this pool / TR: Bu havuzdan değil

        ptr->~T();                            // EN: Explicit destructor call / TR: Açık destructor çağrısı
        slots_[idx].occupied = false;
        slots_[idx].next_free = free_head_;   // EN: Push to front of free list / TR: Serbest listenin başına ekle
        free_head_ = idx;
        --active_count_;
    }
 ...
 *
 * TR: Kilitsiz eşzamanlı üretici/tüketici için çift tampon.
 *     Üretici "arka" tampona yazar; tüketici "ön" tamponu okur.
 *     swap() iki tampon indeksini atomik olarak değiştirir.
 *
 *     Gerçek kullanım: DMA, arka tamponu ADC örnekleriyle doldururken ana döngü
 *     ön tamponu işler. DMA tamamlama kesmesinden sonra → swap().
 */
template<typename T, std::size_t N>
class DoubleBuffer {
    std::array<T, N> buffer_a_{};
    std::array<T, N> buffer_b_{};
    std::atomic<int> active_{0};  // EN: 0=A is front, 1=B is front / TR: 0=A ön, 1=B ön

public:
    /// EN: Get read-only reference to the front (consumer) buffer.
    /// TR: Ön (tüketici) tamponuna salt okunur referans al.
    ...
    /// TR: Arka (üretici) tamponuna yazılabilir referans al.
    ...
    /// TR: Ön ve arka tamponları atomik olarak takas et. Üretici yazmayı bitirdikten sonra çağır.
    void swap() {
 ...
 *
 * TR: Aktif Nesne, görevleri özel bir thread'de asenkron olarak çalıştırır.
 *     Çağıranlar std::function<void()> mesajları kuyruğa ekler; worker thread FIFO işler.
 *     Thread güvenli: kuyruk mutex + koşul değişkeniyle korunur.
 *
 *     Bunun temeli:
 *     - AUTOSAR Runnable'lar (paylaşılan durum olmadan OS-zamanlanmış fonksiyonlar)
 *     - CAN mesaj dağıtıcıları (ISR kuyruğa ekler, task işler)
 *     - Ertelenmiş yürütme ile Command kalıbı
 */
class ActiveObject {
...
public:
    ...
    ~ActiveObject() {
        ...
        // TR: Worker'a durmasını sinyal ver ve bitmesini bekle
        {
    ...
    // TR: Kopyalanamaz (bir thread'e sahip)
    ActiveObject(const ActiveObject&) = delete;
    ActiveObject& operator=(const ActiveObject&) = delete;

    /// EN: Enqueue a task for asynchronous execution.
    /// TR: Asenkron yürütme için bir görev kuyruğa ekle.
    ...
    /// TR: Tüm bekleyen görevler işlenene kadar bekle.
    void flush() {
        // EN: Enqueue a synchronization task and wait on its completion
        // TR: Bir senkronizasyon görevi kuyruğa ekle ve tamamlanmasını bekle
        std::mutex m;
...
private:
    void run() {
        while (true) {
            ...
            task();  // EN: Execute outside the lock / TR: Kilidin dışında çalıştır
        }
    }
};
 ...
 *
 * TR: Geçiş tablosunun const veri olduğu genel tablo güdümlü FSM.
 *     Durum × Olay → { guard(), aksiyon(), sonraki_durum }
 *     Dağıtım döngüsü sabittir; uygulama başına yalnızca tablo değişir.
 *
 *     AUTOSAR Temel Yazılım (BSW) modüllerinde kullanılan kalıp:
 *     - BswM (BSW Mod Yöneticisi)
 *     - ComM (İletişim Yöneticisi)
 *     - EcuM (ECU Durum Yöneticisi)
 */

// EN: ECU operating states / TR: ECU çalışma durumları
enum class EcuState : uint8_t {
    STARTUP,      // EN: Power-on initialization / TR: Güç açılış başlatma
    NORMAL,       // EN: Normal operation mode / TR: Normal çalışma modu
    DIAGNOSTIC,   // EN: Diagnostic session active / TR: Teşhis oturumu aktif
    SLEEP,        // EN: Low-power sleep mode / TR: Düşük güç uyku modu
    COUNT         // EN: Number of states (sentinel) / TR: Durum sayısı (nöbetçi)
};

// EN: Events that trigger state transitions / TR: Durum geçişlerini tetikleyen olaylar
enum class EcuEvent : uint8_t {
    INIT_COMPLETE,     // EN: Initialization finished / TR: Başlatma tamamlandı
    DIAG_REQUEST,      // EN: Diagnostic request received / TR: Teşhis isteği alındı
    DIAG_TIMEOUT,      // EN: Diagnostic session timed out / TR: Teşhis oturumu zaman aşımı
    ...
    COUNT              // EN: Number of events (sentinel) / TR: Olay sayısı (nöbetçi)
};

// EN: Convert enum to string for logging / TR: Log için enum'u string'e dönüştür
...
// TR: Koruma fonksiyonları — geçiş için doğru olması gereken koşullar
...
// TR: Geçiş tablosu — tablo güdümlü FSM'nin ÇEKİRDEĞİ
//     İndeksleme: transition_table[durum][olay]
...
// TR: Fonksiyon işaretçileri constexpr olmadığından geçiş tablosu çalışma zamanında başlatılır
...
void init_transition_table() {
    ...
    // TR: NO_TRANSITION ile doldur (işlenmeyen olaylarda mevcut durumda kal)
    for (std::size_t s = 0; s < STATE_COUNT; ++s)
        for (std::size_t e = 0; e < EVENT_COUNT; ++e)
            transition_table[s][e] = NO_TRANSITION;

    // EN: STARTUP state transitions / TR: STARTUP durumu geçişleri
    transition_table[0][0] = {guard_always_true, action_start_normal, EcuState::NORMAL};

    // EN: NORMAL state transitions / TR: NORMAL durumu geçişleri
    ...
    // EN: DIAGNOSTIC state transitions / TR: DIAGNOSTIC durumu geçişleri
...
EcuState fsm_dispatch(EcuState current, EcuEvent event) {
    ...
    // TR: Geçiş tanımlı değilse mevcut durumda kal
    if (t.next_state == EcuState::COUNT) return current;

    // EN: Check guard condition / TR: Koruma koşulunu kontrol et
    if (t.guard && !t.guard()) return current;

    // EN: Execute action / TR: Aksiyonu çalıştır
 ...
 *
 * TR: Ham bellek eşlemeli register erişimini tip güvenli OOP arayüzüyle sarar.
 *     CRTP kullanır, böylece türetilmiş sınıflar register temel adresini ve düzenini belirtir.
 *     volatile, derleyicinin register okuma/yazmaları optimize etmemesini sağlar.
 *
 *     Gerçek MCU'da base_address ör. 0x40020000 (STM32'de GPIO port A) olurdu.
 *     Burada statik dizi ile simüle ediyoruz ama arayüz aynı.
 */
template<typename Derived>
class RegisterBase {
protected:
    // EN: Read a 32-bit register at given offset from base
    // TR: Tabandan verilen offset'teki 32-bit register'ı oku
    ...
    // TR: Verilen offset'teki register'a 32-bit değer yaz
 ...
 *
 * TR: Simüle edilmiş GPIO portu — Donanım Vekili kalıbını gösterir.
 *     Üretim kodunda regs_ kaldırılır ve reg_ptr()
 *     reinterpret_cast<volatile uint32_t*>(BASE_ADDRESS + offset) döndürür.
 */
class GpioPort : public RegisterBase<GpioPort> {
    // EN: Register offsets (matching STM32 GPIO layout)
    // TR: Register offset'leri (STM32 GPIO düzenine uygun)
    static constexpr std::size_t MODER  = 0;  // EN: Mode register / TR: Mod register'ı
    static constexpr std::size_t ODR    = 1;  // EN: Output data register / TR: Çıkış veri register'ı
    static constexpr std::size_t IDR    = 2;  // EN: Input data register / TR: Giriş veri register'ı
    static constexpr std::size_t BSRR   = 3;  // EN: Bit set/reset register / TR: Bit ayarla/sıfırla register'ı

    // EN: Simulated register file (in production: MMIO at fixed address)
    // TR: Simüle register dosyası (üretimde: sabit adreste MMIO)
...
public:
    ...
    /// TR: Pin'i çıkış olarak yapılandır (2 mod bitini 01 yap)
    void configure_output(uint8_t pin) {
        ...
        mode |=  (0x1u << shift);    // EN: Set to output (01) / TR: Çıkışa ayarla (01)
        write_reg(MODER, mode);
    }

    /// EN: Set pin HIGH using the bit set/reset register (atomic on real HW)
    /// TR: Pin'i YÜKSEK yap — bit ayarla/sıfırla register kullan (gerçek HW'da atomik)
    void set_pin(uint8_t pin) {
        assert(pin < 16);
        write_reg(BSRR, 1u << pin);    // EN: Write to lower 16 bits = SET / TR: Alt 16 bite yaz = AYARLA
        set_bits(ODR, 1u << pin);       // EN: Also update ODR for simulation / TR: Simülasyon için ODR'yi de güncelle
    }
    ...
    void clear_pin(uint8_t pin) {
        assert(pin < 16);
        write_reg(BSRR, 1u << (pin + 16));  // EN: Write to upper 16 bits = RESET / TR: Üst 16 bite yaz = SIFIRLA
        clear_bits(ODR, 1u << pin);
    }

    /// EN: Read pin state from output data register
    /// TR: Çıkış veri register'ından pin durumunu oku
    ...
    /// TR: Tüm çıkış pin'lerini bit maskesi olarak oku
 ...
 *
 * TR: Yukarıdaki kalıpları gerçekçi bir ECU senaryosunda birleştirir:
 *     - CAN mesaj nesneleri için MemoryPool
 *     - Sensör okumaları için DoubleBuffer
 *     - Asenkron görev işleme için ActiveObject
 *     - ECU durum yönetimi için Tablo Güdümlü FSM
 *     - Donanım göstergesi (LED) için GpioPort
 */
struct CanMessage {
    uint32_t id;
    uint8_t dlc;
...
int main() {
    ...
    // ─── Demo 2: Double Buffer ───────────────────────────────────────────
    {
        ...
        // TR: ISR/DMA simülasyonu: arka tampona yaz
        ...
        // TR: Tamponları takas et — üreticinin verisi artık okunabilir
        sensor_buf.swap();
        std::cout << "  [Swap] Buffers exchanged\n";

        // EN: Consumer reads from front buffer
        // TR: Tüketici ön tampondan okur
    ...
    // ─── Demo 3: Active Object ───────────────────────────────────────────
    {
        ...
        // TR: 5 görev kuyruğa ekle — worker thread'de asenkron yürütülür
        for (int i = 1; i <= 5; ++i) {
            worker.enqueue([i] {
        ...
        // TR: Flush, devam etmeden önce tüm kuyruktaki görevlerin tamamlanmasını sağlar
        worker.flush();
    ...
    // ─── Demo 5: Hardware Proxy ──────────────────────────────────────────
    {
        ...
        // TR: Pin 0, 5, 13'ü çıkış olarak yapılandır (STM32 Nucleo gibi: LED pin 5'te)
        ...
        // TR: Gerçek donanım LED'lerini kontrol eder gibi pin'leri ayarla/temizle
        gpio.set_pin(0);
        gpio.set_pin(5);
        gpio.set_pin(13);
    ...
    // ─── Demo 6: Integrated ECU Scenario ─────────────────────────────────
    {
        ...
        // EN: Setup components / TR: Bileşenleri hazırla
        ...
        // TR: Faz 1: Init tamamlandı → NORMAL, LED AÇIK
        ecu_task.enqueue([&] {
        ...
        // TR: Faz 2: CAN mesajı al, havuzdan tahsis et
        ecu_task.enqueue([&] {
        ...
        // TR: Faz 3: Çift tampon ile sensör verisini güncelle
        ecu_task.enqueue([&] {
            auto& back = sensor_buf.back();
        ...
        // TR: Faz 4: Uyku isteği → LED KAPALI
        ecu_task.enqueue([&] {
