 ...
 *
 * - [capture_list] : HOW to grab outside variables (see next theory). - (parameters)   : Input
 * arguments, just like a normal function. - mutable        : (Optional) Lets you modify
 * captured-by-value copies. - -> return_type : (Optional) Explicit return type. Usually
 * auto-deduced. - { body }       : The actual code.
 *
 ...
 *
 * - [yakalama_listesi] : Dış değişkenleri NASIL yakalayacağınız. - (parametreler)     : Girdi
 * argümanları, normal fonksiyon gibi. - mutable            : (Opsiyonel) Değer ile yakalanan
 * kopyaları değiştirmenize izin verir. - -> dönüş_tipi      : (Opsiyonel) Açık dönüş tipi.
 * Genelde otomatik çıkarılır. - { gövde }          : Gerçek kod.
 *
