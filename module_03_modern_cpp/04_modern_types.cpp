 ...
 * geldi.
 *
 * 1. std::optional<T> : Contains a value of type "T" OR it is empty (nullopt). (İçinde "T" tipi
 * bir değer YA VARDIR YA DA YOKTUR - nullopt). 2. std::variant<T, Y> : A type-safe version of
 * C's "union". Holds only ONE type at a time but allocates memory equal to the BIGGEST mapped
 * type (RAM saver). (C'deki "union"ın güvenli halidir. Aynı anda tek tip tutar, RAM cimrisidir.)
 * 3. std::any : Holds "anything" (Like variables in Python or JS). Heavy overhead. ("İçine ne
 * atarsan at" tipidir. Maliyetlidir, zorda kalınmadıkça kullanılmaz.) 4. std::string_view : A
 * lightweight purely observational "window" to look at a string WITHOUT copying it! (Bir metni
 * kopyalamadan -yeni RAM tüketmeden- SADECE BAKMA penceresidir.)
 *
 ...
 * parameter: references and top-level const/volatile are stripped. This means: • `const int& r =
 * n; auto a = r;`  → a is `int`, NOT `const int&`. • `auto x = {1,2,3};` deduces
 * `std::initializer_list<int>` (special rule). • `auto x{42};` in C++17 deduces `int` (changed
 * from init-list in C++11). `decltype(auto)` preserves the EXACT type including references: •
 * `decltype(auto) y = r;` → y is `const int&`. Parenthesized id-expressions change decltype
 * semantics: • `decltype(x)` → declared type of x. • `decltype((x))` → always an lvalue
 * reference (T&), because (x) is an lvalue expression, not an id-expression! This subtle
 * difference is a common source of bugs in return type deduction with `decltype(auto)` —
 * returning `(localVar)` gives a dangling reference.
 *
 ...
 * referanslar ve üst-düzey const/volatile atılır. • `const int& r = n; auto a = r;`  → a tipi
 * `int`'tir, `const int&` DEĞİL. • `auto x = {1,2,3};` → `std::initializer_list<int>` çıkarılır.
 * • `auto x{42};` → C++17'de `int` (C++11'de init-list'ti, değişti). `decltype(auto)`
 * referanslar dahil TAM türü korur: • `decltype(auto) y = r;` → y tipi `const int&`. Parantezli
 * ifadeler decltype semantiğini değiştirir: • `decltype(x)`   → x'in bildirim türü. •
 * `decltype((x))` → her zaman lvalue referans (T&)! Çünkü (x) bir lvalue ifadesidir,
 * id-expression değildir. Bu ince fark, `decltype(auto)` dönüş türüyle `(localVar)` döndürürken
 * sarkan referans (dangling reference) hatasına yol açar.
 *
 * @date 2026
