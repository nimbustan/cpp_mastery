 ...
 * =============================================================================
 * EN: - L-Value (Left Value): Has a permanent memory address, a name, and can sit on the left of
 * an equal sign. (`int x = 5;` -> 'x' is an L-Value). - R-Value (Right Value): A temporary,
 * nameless object born to die. (`5` or the unassigned return of `factory()` is an R-Value). -
 * L-Value Reference (`&`): Classic reference. - R-Value Reference (`&&`): C++11 feature.
 * Specifically catches TEMPORARY, DYING objects so you can "Steal/Move" their internals!
 *
 * TR: - L-Value (Left Value): RAM'de yer edinen, bir ismi (adresi) olan ve eşittirin solunda
 * olabilen nesne. (`int x = 5;` -> 'x' bir L-Value'dir). - R-Value (Right Value): Geçici, ismi
 * olmayan, şipşak yaratılıp ölen nesne. (`5` veya `factory()` fonksiyonunun havada kalan dönüşü
 * R-Value'dir). - L-Value Reference (`&`): Klasik C++ referansı (Sadece kalıcıları tutar). -
 * R-Value Reference (`&&`): C++11 Devrimi! GEÇİCİ, ÖLECEK olan nesneleri yakalar ve onların
 * donanımlarını "çalmanızı/taşımanızı" sağlar!
 *
 * =============================================================================
