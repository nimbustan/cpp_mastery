/**
 * @file 05_structured_bindings_optional.cpp
 * @brief EN: Structured Bindings, std::optional, std::variant, and std::any — Modern C++17
 * Vocabulary Types
 * TR: Yapısal Bağlamalar, std::optional, std::variant ve\n *        std::any — Modern C++17
 * Sözcük Türleri
 *
 * @details [THEORY]
 *
 ...
 * +==============================================================================+
 *
 * EN: Structured bindings allow decomposing an object into its constituent elements using the
 * syntax: auto [a, b, c] = expression; They work with std::pair, std::tuple, std::array, and any
 * aggregate type (struct/class with all public non-static data members). In automotive ECU
 * software, structured bindings dramatically simplify iteration over maps of sensor data, DTC
 * records, and calibration tables.
 *
 * TR: Yapısal bağlamalar, bir nesneyi bileşenlerine ayırmayı sağlar: auto [a, b, c] = ifade;
 * sözdizimi kullanılır. std::pair, std::tuple, std::array ve tüm üyeleri public olan herhangi
 * bir aggregate türle (struct/class) çalışır. Otomotiv ECU yazılımında, sensör haritaları, DTC
 * kayıtları ve kalibrasyon tablolarının dolaşımını büyük ölçüde basitleştirir.
 *
 * +==============================================================================+ |            
 * std::optional<T> (C++17)                            |
 * +==============================================================================+
 *
 * EN: std::optional<T> represents a value that may or may not be present. It is a safer
 * alternative to raw pointers, sentinel values (-1, NaN), or out-parameters for expressing "no
 * result". Key API: has_value(), value(), value_or(default), operator*, operator bool. In
 * automotive: a sensor may be offline, a DTC lookup may find nothing, a calibration parameter
 * may be unconfigured — all modeled by optional.
 *
 * TR: std::optional<T>, var olabilecek veya olmayabilecek bir değeri temsil eder. Ham
 * işaretçiler, sentinel değerler (-1, NaN) veya çıkış parametreleri yerine "sonuç yok" ifade
 * etmek için daha güvenli bir alternatiftir. Temel API: has_value(), value(),
 * value_or(varsayılan), operator*, operator bool. Otomotifte: sensör çevrimdışı, DTC araması
 * sonuçsuz, kalibrasyon parametresi yapılandırılmamış olabilir — hepsi optional ile modellenir.
 *
 * +==============================================================================+ |            
 * std::variant / std::any (C++17)                               |
 * +==============================================================================+
 *
 * EN: std::variant<T1,T2,...> is a type-safe union — it holds exactly one of the listed types at
 * any time. Access via std::get<T>, std::get<I>, or std::visit() with a visitor (often the
 * "overloaded" lambda pattern). std::any is a type-safe void* — it can hold any
 * copy-constructible type and is retrieved via std::any_cast<T>. Throws std::bad_any_cast on
 * mismatch.
 *
 * TR: std::variant<T1,T2,...> tür-güvenli birleşimdir — listelenen türlerden her zaman tam
 * olarak birini tutar. std::get<T>, std::get<I> veya std::visit() ile ziyaretçi deseni
 * kullanılarak erişilir. std::any, tür-güvenli void* karşılığıdır — kopyalanabilir herhangi bir
 * türü tutar, std::any_cast<T> ile alınır. Uyumsuzlukta std::bad_any_cast fırlatır.
 *
 * EN: When to use which: optional  → "might not exist" (nullable value semantics) variant   →
 * "one of several known types" (closed type set) any       → "truly dynamic type" (open type
 * set, use sparingly)
 *
 * TR: Hangisi ne zaman kullanılır: optional  → "olmayabilir" (null olabilen değer semantiği)
 * variant   → "bilinen birkaç türden biri" (kapalı tür kümesi) any       → "gerçekten dinamik
 * tür" (açık tür kümesi, dikkatli kullanılmalı)
 *
 * [CPPREF DEPTH: std::optional — No Heap, No Exceptions, Just Value Semantics / CPPREF DERİNLİK:
 * std::optional — Heap Yok, İstisna Yok, Saf Değer Semantiği]
 * =============================================================================
 * EN: `std::optional<T>` stores `T` in-place using aligned storage — there is NO heap
 * allocation. Its size equals sizeof(T) + 1 byte boolean flag + padding for alignment. For
 * example: • `optional<int>` is typically 8 bytes (4 int + 1 flag + 3 pad). • `optional<char>`
 * is typically 2 bytes (1 char + 1 flag). `optional<optional<T>>` is valid and semantically
 * different from `optional<T>`: the outer layer can be engaged holding a disengaged inner
 * optional — two distinct "empty" states. Accessing a disengaged optional via `operator*` or
 * `operator->` is undefined behavior (no exception thrown!). Use `value()` for checked access —
 * it throws `std::bad_optional_access`. C++23 adds monadic operations inspired by functional
 * programming: • `and_then(f)` — flat-map: f returns optional<U> • `transform(f)` — map: f
 * returns U, wrapped in optional<U> • `or_else(f)` — called only when disengaged These enable
 * clean chaining: `opt.and_then(parse).transform(validate)` without nested if-checks.
 *
 * TR: `std::optional<T>`, T'yi hizalı depolama (aligned storage) ile yerinde saklar — heap
 * tahsisi YOKTUR. Boyutu = sizeof(T) + 1 bayt bayrak + hizalama dolgusu. Örneğin: •
 * `optional<int>` genelde 8 bayttır (4 int + 1 bayrak + 3 dolgu). • `optional<char>` genelde 2
 * bayttır. `optional<optional<T>>` geçerlidir ve `optional<T>`'den farklıdır: dış katman, boş
 * bir iç optional tutabilir — iki ayrı "boş" durum. Boş bir optional'a `operator*` ile erişmek
 * tanımsız davranıştır (istisna FIRLATMAZ!). Kontrollü erişim için `value()` kullanılır —
 * `std::bad_optional_access` fırlatır. C++23 fonksiyonel programlamadan esinlenen monadik
 * işlemler ekler: • `and_then(f)` — f, optional<U> döner (flat-map) • `transform(f)` — f, U
 * döner, optional<U> olarak sarılır (map) • `or_else(f)` — yalnızca boşken çağrılır Böylece iç
 * içe if kontrolü olmadan zincirleme yapılabilir: `opt.and_then(parse).transform(validate)`.
 *
 * @date 2026
