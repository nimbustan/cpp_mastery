 ...
 * @brief API Design in Modern C++
 *
 * @details
 * =============================================================================
 * [THEORY: Principles of Good C++ API Design]
 * =============================================================================
 *
...
public:
    ...
    // EN: Move-only (because of unique_ptr)
    ECUDiagnostics(ECUDiagnostics&& other) noexcept;
    ECUDiagnostics& operator=(ECUDiagnostics&& other) noexcept;

    // EN: Public API — stable ABI
    void run_diagnostics();
    int get_dtc_count() const;
    std::string get_summary() const;

private:
    struct Impl;                       // EN: Forward declaration
    std::unique_ptr<Impl> pimpl_;      // EN: Hidden implementation
};
...
double convert_to_celsius(double value, tags::Celsius) {
    return value;  // EN: Already Celsius
}
