 ...
 * =============================================================================
 * EN: - USE `friend` when two classes are tightly coupled BY DESIGN (e.g., an iterator and its
 * container, a serializer and its data class). - DO NOT use `friend` to bypass poor
 * encapsulation. If you need many friends, your design is probably wrong — add proper
 * public/protected API. - `static` is great for counters, caches, and singletons. Beware of
 * initialization order across translation units (the "static init fiasco").
 *
 * TR: - `friend` kullanın: İki sınıf TASARIM GEREĞİ sıkı bağlıysa (örn. bir yineleyici ve
 * konteyneri, bir serileştirici ve veri sınıfı). - `friend` KULLANMAYIN: Zayıf kapsüllemeyi
 * atlatmak için. Çok fazla friend'e ihtiyacınız varsa tasarımınız muhtemelen yanlıştır — uygun
 * public/protected API ekleyin. - `static`, sayaçlar, önbellekler ve tekil nesneler (singleton)
 * için harikadır. Çeviri birimleri arasında başlatma sırası konusunda dikkatli olun ("static
 * init fiasco" sorunu).
 *
