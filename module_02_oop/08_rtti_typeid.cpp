 ...
 * dispatch: 1. Enum-based type tags — manual but fast O(1) switch dispatch. 2. Visitor pattern
 * (double dispatch) — type-safe, no RTTI needed. 3. std::variant + std::visit — compile-time
 * checked, zero overhead. 4. CRTP (Curiously Recurring Template Pattern) — static polymorphism.
 ...
 * dönüşür. RTTI tabanlı dağıtıma alternatifler: 1. Enum tabanlı tür etiketleri — manuel ama
 * hızlı O(1) switch. 2. Visitor kalıbı (çift dağıtım) — tür güvenli, RTTI gerekmez. 3.
 * std::variant + std::visit — derleme zamanında kontrollü, sıfır ek yük. 4. CRTP — statik
 * polimorfizm. Otomotivde (MISRA C++), gerçek zamanlı ve gömülü alanlarda RTTI, öngörülemeyen
