 ...
 * vtable at runtime. CRTP resolves the call at compile time: - No vtable pointer (8 bytes saved
 * per object on 64-bit) - No indirect branch (better branch prediction, inlineable by compiler)
 * - No heap allocation required for polymorphic behavior Trade-off: CRTP types are not
 * runtime-polymorphic (no base-class pointer).
 * TR: Sanal fonksiyonlar nesne başına bir vtable işaretçisi ve çalışma zamanında vtable
 * üzerinden dolaylı çağrı gerektirir. CRTP çağrıyı derleme zamanında çözer: - vtable işaretçisi
 * yok (64-bit'te nesne başına 8 bayt tasarruf) - Dolaylı dal yok (daha iyi dal tahmini,
 * derleyici tarafından inline edilebilir) - Polimorfik davranış için yığın tahsisi gerekmez
 ...
 * ===============================================================================
 * EN: - Mixins: inject reusable functionality (logging, counting, serialization) into unrelated
 * classes without virtual overhead. - Static interfaces: enforce that derived classes implement
 * certain methods, checked at compile time via static_cast. - Expression templates: build lazy
 * evaluation trees (e.g., Eigen library).
 * TR: - Mixin'ler: sanal maliyet olmadan ilgisiz sınıflara yeniden kullanılabilir işlevsellik
 * (loglama, sayma, serileştirme) enjekte eder. - Statik arayüzler: türetilmiş sınıfların belirli
 * metotları uygulamasını derleme zamanında zorlar. - İfade şablonları: tembel değerlendirme
 * ağaçları oluşturur.
 *
 * ===============================================================================
