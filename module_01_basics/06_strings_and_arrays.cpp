 ...
 * =============================================================================
 * EN: C++ inherited NULL-terminated character arrays from C (`char str[]`). These are dangerous:
 * no bounds checking, manual memory management, easy buffer overflows. Modern C++ uses
 * `std::string` from `<string>`: - Automatic memory management (grows/shrinks as needed). - Safe
 * concatenation with `+` operator. - Rich API: `.length()`, `.substr()`, `.find()`,
 * `.compare()`, etc.
 *
 * TR: C++ dilinden NULL sonlandırmalı karakter dizilerini miras aldı (`char str[]`). Bunlar
 * tehlikelidir: sınır kontrolü yok, manuel bellek yönetimi, kolay buffer overflow. Modern C++
 * `<string>` kütüphanesindeki `std::string` kullanır: - Otomatik bellek yönetimi (ihtiyaca göre
 * büyür/küçülür). - `+` operatörü ile güvenli birleştirme. - Zengin API: `.length()`,
 * `.substr()`, `.find()`, `.compare()`, vb.
 *
 * =============================================================================
 * [THEORY: C-Style Arrays vs std::array / TEORİ: C Dizileri vs std::array]
 * =============================================================================
 * EN: C-style arrays (`int arr[5]`) decay to pointers when passed to functions, losing size
 * information. `std::array<int, 5>` (C++11) wraps a fixed-size array with bounds checking
 * (`.at()`) and knows its own size (`.size()`). For dynamic sizes, use `std::vector`.
 *
 * TR: C-tarzı diziler (`int arr[5]`) fonksiyonlara geçildiğinde pointer'a dönüşür ve boyut
 * bilgisini kaybeder. `std::array<int, 5>` (C++11) sabit boyutlu diziyi sınır kontrolü (`.at()`)
 * ve kendi boyutunu bilme (`.size()`) ile sarar. Dinamik boyut için `std::vector` kullanın.
 *
 * =============================================================================
 * [CPPREF DEPTH: Small String Optimization (SSO) / CPPREF DERİNLİK: Küçük String Optimizasyonu
 * (SSO)]
 * =============================================================================
 * EN: Most `std::string` implementations apply SSO: strings shorter than ~15-22 characters are
 * stored INSIDE the object (on the stack), avoiding heap allocation entirely. Longer strings
 * trigger a `new` allocation. This is crucial for embedded systems where heap usage is
 * expensive.
 *
 * TR: Çoğu `std::string` implementasyonu SSO uygular: ~15-22 karakterden kısa string'ler objenin
 * İÇİNDE (stack'te) saklanır, heap tahsisinden tamamen kaçınılır. Uzun string'ler `new` ile
 * heap'te yer açar. Heap kullanımının pahalı olduğu gömülü sistemlerde bu kritiktir.
 *
