 ...
 * ===============================================================================
 *
 * --- 1. What Are Ranges? / Aralık Nedir? ------------------------------------
 ...
 * nesneyle değiştirir.
 *
 * --- 2. Views — Lazy, Non-Owning Wrappers / Görünümler ---------------------
 * EN: A View is a lightweight, non-owning wrapper around a range. Views are cheap to copy and
 * never own the underlying data. Key views: • filter(pred)    — keep elements satisfying
 * predicate • transform(fn)   — apply function to each element • take(n)         — first N
 * elements • drop(n)         — skip first N elements • reverse          — reverse iteration
 * order
 * TR: Bir Görünüm (View), aralık etrafındaki hafif, sahiplik almayan bir sarmalayıcıdır.
 * Kopyalaması ucuzdur ve alttaki veriye sahip olmaz: • filter(pred)    — koşulu sağlayan
 * elemanları tut • transform(fn)   — her elemana fonksiyon uygula • take(n)         — ilk N
 * eleman • drop(n)         — ilk N elemanı atla • reverse          — ters sırada yinele
 *
 * --- 3. Pipe Syntax / Boru Sözdizimi ---------------------------------------
 * EN: The pipe operator `|` chains views left-to-right, reading like a processing pipeline on a
 ...
 * TR: Boru operatörü `|`, görünümleri soldan sağa zincirler; fabrika bandındaki bir işleme hattı
 ...
 *
 * --- 4. Why Ranges Matter / Aralıklar Neden Önemli -------------------------
 * EN: • Eliminate temporary containers between algorithm stages • Improve readability —
 * declarative, not imperative • Lazy evaluation — compute only what is consumed • Composable —
 * small pieces snap together like LEGO bricks
 * TR: • Algoritma aşamaları arasındaki geçici konteynerleri ortadan kaldırır • Okunabilirliği
 * artırır — bildirimsel, zorunlu değil • Tembel değerlendirme — yalnızca tüketilen hesaplanır •
 * Bileşenleştirilebilir — küçük parçalar LEGO gibi birleşir
 *
 * --- 5. C++17 Workaround Strategy / C++17 Geçici Çözüm Stratejisi ---------
 * EN: Under C++17 we lack <ranges>, so we emulate the pattern: 1. Template helper functions:
 ...
 * TR: C++17'de <ranges> olmadığından, aşağıdaki stratejiyle öykünüyoruz: 1. Şablon yardımcı
 ...
 * In C++17 (pre-ranges), similar laziness can be achieved with: • Custom iterator pairs wrapping
 * filter/transform logic • Pipeline builder classes chaining `std::function` stages •
 ...
 * C++17'de (ranges öncesi) benzer tembellik şu yollarla sağlanabilir: • Filtre/dönüşüm mantığını
 * saran özel iteratör çiftleri • `std::function` aşamalarını zincirleyen boru hattı sınıfları •
