 ...
 * @details
 * =====================================================================
 * [THEORY: Ordered vs Unordered — Red-Black Tree vs Hash Table / TEORİ: Sıralı vs Sırasız —
 * Kırmızı-Siyah Ağaç vs Hash Tablosu]
 * =====================================================================
 *
 * ───── Ordered Containers (Sıralı Konteynerler) ─────
 * EN: std::set, std::map, std::multiset, std::multimap Backed by Red-Black Tree (self-balancing
 * BST). ✓ Always sorted by key ✓ O(log N) insert, find, erase ✓ Supports ordered traversal,
 * lower_bound, upper_bound ✗ Slower than hash for pure lookup
 *
 * TR: std::set, std::map, std::multiset, std::multimap Kırmızı-Siyah Ağaç (kendi kendini
 * dengeleyen BST) ile desteklenir. ✓ Her zaman anahtara göre sıralı ✓ O(log N) ekleme, bulma,
 * silme ✓ Sıralı gezinme, lower_bound, upper_bound destekler ✗ Saf arama için hash'ten yavaş
 *
 * ───── Unordered Containers (Sırasız Konteynerler) ─────
 * EN: std::unordered_set, std::unordered_map, std::unordered_multiset/map Backed by Hash Table
 * (bucket array + linked lists). ✓ O(1) average insert, find, erase ✓ Fastest for pure key
 * lookup ✗ No ordering guarantee ✗ Worst case O(N) if many hash collisions ✗ Requires hash
 * function + equality operator
 *
 * TR: std::unordered_set, std::unordered_map, std::unordered_multiset/map Hash Tablosu (kova
 * dizisi + bağlı listeler) ile desteklenir. ✓ Ortalama O(1) ekleme, bulma, silme ✓ Saf anahtar
 * araması için en hızlı ✗ Sıralama garantisi yok ✗ Çok çarpışma varsa en kötü O(N) ✗ Hash
 * fonksiyonu + eşitlik operatörü gerektirir
 *
 ...
 * =============================================================================
 * EN: `std::map` / `std::set` are red-black trees (self-balancing BST): • All operations O(log
 * n) — insert, find, erase. • Iteration yields elements in SORTED order by key. • Iterators are
 * stable: insert/erase never invalidates other iterators. `std::unordered_map` /
 * `std::unordered_set` use hash tables: • Average O(1) for insert/find/erase, worst-case O(n)
 * (all collide). • No ordering guarantee — iteration order is implementation-defined. • Load
 * factor = size / bucket_count. When load_factor > max_load_factor (default 1.0), a REHASH
 * occurs: new bucket array allocated, all elements re-hashed — O(n) and ALL iterators are
 * invalidated! Custom hash: specialize `std::hash<T>` or pass as template argument:
 * `std::unordered_map<K, V, MyHash, MyEqual>` A good hash distributes uniformly. Combine fields
 * with: `seed ^= std::hash<T>{}(val) + 0x9e3779b9 + (seed<<6) + (seed>>2);` C++17
 * `try_emplace()` avoids constructing the value if key exists — more efficient than `emplace()`
 * for expensive-to-construct values.
 *
 * TR: `std::map` / `std::set` kırmızı-siyah ağaçtır (dengeli BST): • Tüm işlemler O(log n) —
 * ekleme, arama, silme. • İterasyon elemanları anahtara göre SIRALI verir. • İteratörler
 * kararlıdır: ekleme/silme diğer iteratörleri bozmaz. `std::unordered_map` /
 * `std::unordered_set` hash tablosu kullanır: • Ortalama O(1), en kötü O(n) (tüm anahtarlar
 * çakışırsa). • Sıralama garantisi YOKTUR — gezinme sırası uygulama tanımlıdır. • Yük faktörü =
 * size / bucket_count. max_load_factor (varsayılan 1.0) aşılınca REHASH olur: yeni kova dizisi,
 * tüm elemanlar yeniden hash'lenir — O(n) ve TÜM iteratörler geçersiz olur! Özel hash:
 * `std::hash<T>` özelleştirmesi veya şablon argümanı: `std::unordered_map<K, V, MyHash,
 * MyEqual>` İyi bir hash fonksiyonu düzgün dağılım sağlar. Alanları birleştirmek için: `seed ^=
 * hash(val) + 0x9e3779b9 + (seed<<6) + (seed>>2);` C++17 `try_emplace()` anahtar varsa değer
 * oluşturmaktan kaçınır — pahalı nesnelerde `emplace()`'ten daha verimlidir.
 *
 * @date 2026
