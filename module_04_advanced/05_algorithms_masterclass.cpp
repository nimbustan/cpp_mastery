 ...
 *
 * Konu: STL Algorithms — Sorting, Searching, Modifying, Numeric & Partitioning STL Algoritmaları
 * — Sıralama, Arama, Değiştirme, Sayısal ve Bölümleme
 *
 * Standart: C++17 Derleme: g++ -std=c++17 -Wall -Wextra -Wpedantic 05_algorithms_masterclass.cpp
 *
 * @details
 * =====================================================================
 ...
 * mismatch, equal, search          │
 * ├─────────────────────────────────────────────────────────────┤ │ Modifying      │ copy, move,
 * transform, fill, generate,    │ │                │ replace, remove, unique, reverse, rotate  
 * │ ├─────────────────────────────────────────────────────────────┤ │ Sorting        │ sort,
 * stable_sort, partial_sort,          │ │                │ nth_element, is_sorted               
 * │ ├─────────────────────────────────────────────────────────────┤ │ Binary Search  │
 * lower_bound, upper_bound, equal_range,    │ │                │ binary_search (requires sorted
 * range!)     │ ├─────────────────────────────────────────────────────────────┤ │ Partitioning  
 * │ partition, stable_partition, is_partitioned│
 * ├─────────────────────────────────────────────────────────────┤ │ Numeric        │ accumulate,
 * inner_product, partial_sum,   │ │                │ iota, reduce (C++17), transform_reduce    
 * │ ├─────────────────────────────────────────────────────────────┤ │ Min/Max        │ min, max,
 * minmax, min_element,            │ │                │ max_element, minmax_element, clamp
 * (C++17) │ └─────────────────────────────────────────────────────────────┘
 *
 ...
 * =============================================================================
 * EN: STL algorithms provide strict worst-case complexity guarantees: • `std::sort`         —
 * O(n log n) comparisons. Typically introsort (quicksort + heapsort fallback), guaranteed
 * worst-case O(n log n). • `std::stable_sort`  — O(n log²n) in-place, or O(n log n) if extra
 * memory is available (merge sort with buffer). • `std::nth_element`  — O(n) average
 * (introselect). After the call, element at nth position is what it would be in a sorted range.
 * • `std::partial_sort` — O(n log k) where k = middle - first. Exception safety: algorithms use
 * `std::move_if_noexcept` internally. If the element's move constructor is noexcept, it moves;
 * otherwise it copies to maintain the strong exception guarantee. This is why marking move
 * constructors `noexcept` is critical for performance. Algorithms that write to output iterators
 * (e.g., `std::transform`) provide the basic guarantee: no leaks, but output may be partial.
 * `std::for_each` returns its function object — useful for stateful functors accumulating
 * results during traversal.
 *
 * TR: STL algoritmaları katı en-kötü-durum karmaşıklık garantisi sunar: • `std::sort`         —
 * O(n log n) karşılaştırma. Genelde introsort (quicksort + heapsort yedek), en kötü durum O(n
 * log n) garantili. • `std::stable_sort`  — Yerinde O(n log²n), veya ek bellekle O(n log n)
 * (tamponlu merge sort). • `std::nth_element`  — ortalama O(n) (introselect). Çağrı sonrası n.
 * pozisyondaki eleman, sıralı dizideki yerindedir. • `std::partial_sort` — O(n log k), k =
 * middle - first. İstisna güvenliği: algoritmalar dahili olarak `std::move_if_noexcept`
 * kullanır. Elemanın taşıma yapıcısı noexcept ise taşır; değilse güçlü istisna garantisini
 * korumak için kopyalar. Bu nedenle taşıma yapıcılarını `noexcept` işaretlemek performans için
 * kritiktir. Çıkış iteratörlerine yazan algoritmalar (ör. `std::transform`) temel garanti sunar:
 * sızıntı olmaz, ama çıktı kısmi kalabilir.
 *
 * @date 2026
