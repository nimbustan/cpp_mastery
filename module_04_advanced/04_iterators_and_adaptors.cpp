 ...
 * =====================================================================
 *
 * ───── 1. Input Iterator (Giriş İteratörü) ─────
 * EN: Read-only, single-pass, forward-only. Like reading sensor data from a CAN bus — once you
 * read a frame, it's gone. You can only move forward and read each value exactly once. Supports:
 * ++it, *it (read), it == it2, it != it2
 * TR: Salt okunur, tek geçişli, sadece ileri. CAN bus'tan sensör verisi okumak gibi — bir kare
 ...
 *
 * ───── 2. Output Iterator (Çıkış İteratörü) ─────
 * EN: Write-only, single-pass, forward-only. Like writing commands to an actuator — you push
 * data out, you can't read it back. Supports: ++it, *it = val (write)
 * TR: Salt yazılır, tek geçişli, sadece ileri. Bir aktüatöre komut yazmak gibi — veri
 * gönderirsiniz, geri okuyamazsınız. Destekler: ++it, *it = val (yazma)
 *
 * ───── 3. Forward Iterator (İleri İteratörü) ─────
 * EN: Read+Write, multi-pass, forward-only. Like iterating over a linked list of ECU diagnostic
 * trouble codes (DTCs). Supports: Everything Input + multi-pass guarantee
 * TR: Okuma+Yazma, çok geçişli, sadece ileri. ECU arıza kodları (DTC) bağlı listesinde gezinmek
 * gibi. Destekler: Input'un tamamı + çok geçiş garantisi
 *
 * ───── 4. Bidirectional Iterator (Çift Yönlü İteratör) ─────
 * EN: Forward + backward traversal. Like scrolling through a vehicle's event log — you can go
 * forward and backward through entries. Supports: Everything Forward + --it Used by: std::list,
 * std::set, std::map
 * TR: İleri + geri gezinme. Araç olay günlüğünde gezinmek gibi — ileri ve geri gidebilirsiniz.
 * Destekler: Forward'ın tamamı + --it Kullanan: std::list, std::set, std::map
 *
 * ───── 5. Random Access Iterator (Rastgele Erişim İteratörü) ─────
 * EN: Full pointer arithmetic. Like accessing any sensor value in an array by index — instant
 * O(1) jump to any position. Supports: Everything Bidirectional + it+n, it-n, it[n], it<it2 Used
 * by: std::vector, std::deque, std::array, raw pointers
 * TR: Tam pointer aritmetiği. Bir dizideki herhangi bir sensör değerine indeksle erişmek gibi —
 * herhangi bir konuma anında O(1) atlama. Destekler: Bidirectional'ın tamamı + it+n, it-n,
 * it[n], it<it2 Kullanan: std::vector, std::deque, std::array, ham pointerlar
 *
 * =====================================================================
 * [HIERARCHY / HİYERARŞİ]
 * =====================================================================
 ...
 *
 * Her üst kategori, altındakilerin tüm yeteneklerini içerir. std::sort → Random Access
 * gerektirir std::find → Input yeterlidir std::reverse → Bidirectional gerektirir
 * =====================================================================
 *
 * [CPPREF DEPTH: Iterator Categories and Their Guarantees / CPPREF DERİNLİK: İteratör
 * Kategorileri ve Garantileri]
 * =============================================================================
 * EN: The C++ standard defines 5 iterator categories, each a superset of the previous: 1.
 * InputIterator   — single-pass read  (==, !=, *, ++) 2. OutputIterator  — single-pass write 
 * (*it = val, ++) 3. ForwardIterator — multi-pass read/write (can revisit elements) 4.
 * BidirectionalIterator — adds backward traversal (--) 5. RandomAccessIterator  — O(1) jump:
 * it+n, it-n, it[n], <, > `std::distance(first, last)` is O(1) for RandomAccess iterators
 * (pointer subtraction) but O(n) for all others (incremental ++). `std::advance(it, n)`
 * similarly dispatches on category. Iterator invalidation rules vary critically by container: •
 * vector: insert/push_back may invalidate ALL iterators (realloc). • deque: insert at front/back
 * invalidates iterators but not refs. • list/set/map: insert NEVER invalidates existing
 * iterators. C++20 introduces `std::contiguous_iterator` — guarantees elements are adjacent in
 * memory (e.g., vector, array, string).
 *
 * TR: C++ standardı 5 iteratör kategorisi tanımlar, her biri öncekinin üst kümesidir: 1.
 * InputIterator   — tek geçişli okuma  (==, !=, *, ++) 2. OutputIterator  — tek geçişli yazma 
 * (*it = val, ++) 3. ForwardIterator — çok geçişli okuma/yazma (tekrar ziyaret edilebilir) 4.
 * BidirectionalIterator — geri gidiş eklenir (--) 5. RandomAccessIterator  — O(1) atlama: it+n,
 * it-n, it[n], <, > `std::distance(first, last)` RandomAccess için O(1)'dir (işaretçi çıkarma),
 * diğerleri için O(n)'dir (sıralı ++). İteratör geçersizleşme kuralları konteynere göre değişir:
 * • vector: insert/push_back TÜM iteratörleri geçersiz kılabilir (realloc). • deque: baş/sona
 * ekleme iteratörleri geçersiz kılar ama referansları değil. • list/set/map: ekleme mevcut
 * iteratörleri ASLA geçersiz kılmaz. C++20 `std::contiguous_iterator` ekler — elemanların
 * bellekte bitişik olduğunu garanti eder (ör. vector, array, string).
 *
 * @date 2026
