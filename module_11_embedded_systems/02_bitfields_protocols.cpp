 ...
 * =============================================================================
 * EN: The C++ standard leaves several bitfield aspects implementation-defined: (1) Layout
 * ordering (MSB-first or LSB-first within a byte), (2) Whether bitfields can span storage unit
 * boundaries, (3) Alignment of bitfield storage units. This means bitfield layout is NOT
 * portable across compilers! For cross-platform protocol parsing, prefer explicit bit
 * manipulation (`& mask`, `>> shift`) over bitfields. cppreference.com/w/cpp/language/bit_field
 *
 * TR: C++ standardı birçok bit alanı yönünü uygulamaya bağlı bırakır: (1) Düzen sıralaması (bir
 * bayt içinde MSB-önce veya LSB-önce), (2) Bit alanlarının depolama birimi sınırlarını geçip
 * geçemeyeceği, (3) Bit alanı depolama birimlerinin hizalaması. Bu, bit alanı düzeninin
 * derleyiciler arasında TAŞINABİLİR OLMADIĞI anlamına gelir! Platformlar arası protokol
 * ayrıştırma için bit alanları yerine açık bit manipülasyonu (`& maske`, `>> kaydırma`) tercih
 * edin. cppreference.com/w/cpp/language/bit_field
 *
 * =============================================================================
