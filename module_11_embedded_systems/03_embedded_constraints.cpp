 ...
 * allocation (malloc/new) is BANNED because: (1) Heap fragmentation can exhaust memory
 * unpredictably. (2) Allocation time is non-deterministic — bad for real-time systems. (3) Heap
 * metadata overhead wastes scarce RAM. (4) Memory leaks in long-running firmware are
 * catastrophic. Instead, all memory is statically allocated at compile time using fixed-size
 * arrays, stack variables, and memory pools (placement new).
 *
 * TR: Güvenlik-kritik gömülü sistemlerde (otomotiv, tıbbi, havacılık) dinamik bellek tahsisi
 * (malloc/new) YASAKLIDIR çünkü: (1) Yığın parçalanması belleği öngörülemez şekilde tüketebilir.
 * (2) Tahsis süresi belirleyici değildir — gerçek zamanlı sistemler icin kötü. (3) Yığın meta
 * veri ek yükü kıt RAM'i israf eder. (4) Uzun süre çalışan donanım yazılımında bellek
 * sızıntıları felakettir. Bunun yerine, tüm bellek derleme zamanında sabit boyutlu diziler,
 * yığıt değişkenleri ve bellek havuzları (placement new) kullanılarak statik olarak tahsis
 * edilir.
 *
 * =============================================================================
 * [THEORY: constexpr — Compile-Time Computation / TEORİ: constexpr — Derleme Zamanı Hesaplama]
 * =============================================================================
 * EN: `constexpr` forces computation to happen at compile time, producing zero runtime overhead.
 * In embedded, this means: - CRC/checksum lookup tables computed at compile time. - Baud rate
 * divisor calculations resolved before flashing. - Trigonometric lookup tables baked into
 * firmware ROM. C++17 allows `constexpr if` for compile-time branching, and C++20 allows
 * `consteval` to GUARANTEE compile-time-only execution.
 *
 * TR: `constexpr` hesaplamayı derleme zamanında gerçekleşmeye zorlar, sıfır çalışma zamanı ek
 * yükü üretir. Gömülü sistemlerde bu şu anlama gelir: - CRC/sağlama toplamı arama tabloları
 * derleme zamanında hesaplanır. - Baud hızı bölücü hesaplamaları flaşlamadan önce çözümlenir. -
 * Trigonometrik arama tabloları donanım yazılımı ROM'una gömülür. C++17 derleme zamanı dallanma
 * için `constexpr if`'e izin verir ve C++20 yalnızca derleme zamanı yürütmeyi GARANTİ etmek için
 * `consteval`'e izin verir.
 *
 * =============================================================================
 ...
 * =============================================================================
 * EN: `placement new` constructs an object at a specific memory address without allocating.
 * Syntax: `new (address) Type(args);` This is essential for: (1) Memory pools with fixed-size
 * blocks. (2) DMA buffer management. (3) Constructing objects on stack-allocated aligned
 * buffers. You must call the destructor manually: `obj->~Type();`
 *
 * TR: `placement new` tahsis yapmadan belirli bir bellek adresinde nesne oluşturur. Sözdizimi:
 * `new (adres) Tip(argümanlar);` Bu, şunlar için gereklidir: (1) Sabit boyutlu bloklara sahip
 * bellek havuzları. (2) DMA tampon yönetimi. (3) Yığıt-tahsisli hizalanmış tamponlarda nesne
 * oluşturma. Yıkıcıyı manuel olarak çağırmanız gerekir: `obj->~Type();`
 *
 * =============================================================================
 * [CPPREF DEPTH: constexpr Specifier Rules and Limits / CPPREF DERİNLİK: constexpr Belirleyici
 * Kuralları ve Sınırları]
 * =============================================================================
 * EN: A `constexpr` function must: (1) have a literal return type, (2) not contain goto, asm, or
 * non-literal variables (pre-C++20). C++14 relaxed most restrictions (loops, multiple
 * statements). C++20 allows virtual constexpr, try-catch, dynamic_cast. `consteval` (C++20)
 * guarantees the function is ONLY called at compile time — if forced at runtime, it's a
 * compilation error. `constinit` ensures static storage duration variables are initialized at
 * compile time. cppreference.com/w/cpp/language/constexpr
 *
 * TR: Bir `constexpr` fonksiyon: (1) literal dönüş tipine sahip olmalı, (2) goto, asm veya
 * literal olmayan değişkenler içermemeli (C++20 öncesi). C++14 çoğu kısıtlamayı gevşetti
 * (döngüler, çoklu ifadeler). C++20 sanal constexpr, try-catch, dynamic_cast'e izin verir.
 * `consteval` (C++20) fonksiyonun YALNIZCA derleme zamanında çağrıldığını garanti eder.
 * `constinit` statik depolama süresine sahip değişkenlerin derleme zamanında başlatılmasını
 * sağlar. cppreference.com/w/cpp/language/constexpr
 *
 * =============================================================================
 * [CPPREF DEPTH: Placement New and Object Lifetime / CPPREF DERİNLİK: Placement New ve Nesne
 * Yaşam Süresi]
 * =============================================================================
 * EN: Placement new does NOT allocate memory — it only calls the constructor at a given address.
 * The programmer is responsible for: (1) ensuring proper alignment via `alignas` or
 * `std::aligned_storage`, (2) manually calling the destructor before reusing memory, (3) NOT
 * calling `delete` on placement-new objects. `std::launder` (C++17) is needed after placement
 * new to access the object through old pointers when the type has const/reference members.
 * cppreference.com/w/cpp/language/new#Placement_new
 *
 * TR: Placement new bellek TAHSİS ETMEZ — yalnızca verilen adreste yapıcıyı çağırır. Programcı
 * şunlardan sorumludur: (1) `alignas` veya `std::aligned_storage` ile uygun hizalamayı sağlama,
 * (2) belleği yeniden kullanmadan önce yıkıcıyı manuel çağırma, (3) placement-new nesnelerinde
 * `delete` ÇAĞIRMAMAK. `std::launder` (C++17) const/referans üyeleri olan türlerde eski
 * işaretçiler aracılığıyla nesneye erişmek için placement new sonrası gereklidir.
 * cppreference.com/w/cpp/language/new#Placement_new
 *
 * @date 2026
